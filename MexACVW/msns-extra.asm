...
personality_f   db      %00000100	; Only 1 allowed to be set unless in      ;* 0 Tuner Studio ignmode (= %00000100, $3, 3T)
...
outputpins_f    db      %00110010                                             ;* 1 Tuner Studio ???????? (= %00110010, $32, 50T)
;               bits=    76543210
...
feature1_f	    db      %01001000                                               ;* 11 Tuner Studio ??????        (= %01001000,
...
; CrankingPW2	equ $40      Fire PW2 during Cranking?   OFF|| ON^              ;* Tuner Studio inj2cr bit6      (= 1 (On))
...
Trig1_f		    db       1T	    ; wheel decoding                                 ;* 25 Tuner Studio trig1      (= %00000001, $1, 1T)
Trig2_f		    db       19T	;  "                                             ;* 26 Tuner Studio trig2      (= %00010011, $13, 19T)
Trig3_f		    db       0T	    ;  "                                             ;* 27 Tuner Studio trig3      (= 0)
Trig4_f		    db       0T	    ;  "                                             ;* 28 Tuner Studio trig4      (= 0)
Trig1ret_f	    db       8T	    ;  "                                             ;* 29 Tuner Studio trig1ret   (= %00001000, $8, 8T
Trig2ret_f	    db       28T	;  "                                             ;* 30 Tuner Studio trig2ret   (= %00011100, $1C, 28T)
...
feature3_f	    db      %10000001   ; (46)                                        ;* 46 Tuner Studio ????????   (= %10000001, $81, 129T)
...
feature5_f	    db      %00000000   ; Yet another feature bit (76)                             ;* 76 Tuner Studio ?????                    ( = %00000000, $0, 0T)
...
BarroHi_f	    db      116T	    ; Barometric Correction Max Limit in KPa (82                 ;* 82 Tuner Studio BarroHi      (= %01110100, $74, 116T)
BarroLow_f	    db      67T	        ; Barometric Correction Lower Limit in KPa                   ;* 83 Tuner Studio BarroLow     (= %01000011, $43, 67T)
...
NumTeeth_f	    db      36T	        ; Number of teeth for wheel decoder                          ;* 90 Tuner Studio numteeth     (= %00100100, $24, 36T)
...
dwellcrank_f	db       45T	; cranking dwell in 0.1ms                                         ;* 110 Tuner Studio dwellcrank (= %00101101, $2D, 45T)
dwellrun_f	    db       30T	; running  dwell in 0.1ms  (111)                                  ;* 111 Tuner Studio dwellrun   (= %00011110, $1E, 30T)
...
feature8_f	    db      %01000000  ; More feature bits (116)                                      ;* 116 Tuner Studio ????        (= %01000000, $0, 0T)
...
tachconf_f      db      4T         ; tach output config (159)                                     ;* 144 Tuner Studio tachconf bits0:2 (= %00000100 (X5/JS3))
...
tpsdotrate:
	            db	    10T		    ; These next 4 are delta points for TPSdot                    ;* 161 Tuner Studio taeRates4   (= %00001010, $A, 10T)
				                    ; V/Sec
				                    ; these were hard coded points, now users can
				                    ; select what values
				                    ; they want where. *0.1960784 MAX=25.5
	            db	    39T		    ; So 40 = 0.8V/0.1Sec or 8V/Sec as we check it                ;* 162 Tuner Studio NOT USED (= %00100111, $27, 39T)
				                    ; every 0.1Sec
	            db	    78T		    ;                                                             ;* 163 Tuner Studio NOT USED (= %01001110, $4E, 78T)
	            db	    150T		; (179)                                                       ;* 164 Tuner Studio NOT USED (= %10010110, $96, 150T)
mapdotrate_f:
...
EGODELTA_f	    db      0T	    ; EGODELTA                                  ;* 146 Tuner Studio egoDelta1  (= 0 (open loop only, 1 = default)
EGOLIMIT_f	    db      15T	    ; EGOLIMIT                                  ;* 147 Tuner Studio egoLimit1  (= 15T default))
REQ_FUEL_f1	    db      104T	; REQFUEL (148)                             ;* 148 Tuner Studio reqfuel1   (= %01101000, $68, 104T)
...
INJOPEN_f1	    db      9T	    ; INJOPEN                                   ;* 151 Tuner Studio injOpen1   (= %00001001, $9, 9T)
...
BATTFAC_f1	    db      20T	    ; BATTFAC                                   ;* 155 Tuner Studio battFac1   (= %00010000, $14, 20T)
rpmk_f1		    db      $0B	    ; RPMK[0]                                   ;* 156 Tuner Studio rpmk1 (16 bit) (= %1011, $B, 11T)
		        db      $B8	    ; RPMK[1]                                   ;* 157 Tuner Studio rpmk1          (= %10111000, $B8, 184T)
RPMRANGEVE_f1:
...
KPARANGEVE_f1:
	            ...
	            db      7T	    ; KPARANGEVE[7]                             ;* 177
	            db      78T	    ; KPARANGEVE[8]                             ;* 178
	            db      85T	    ; KPARANGEVE[9]                             ;* 179
	            db      93T	    ; KPARANGEVE[10]                            ;* 180
	            db      100T	; KPARANGEVE[11]                            ;* 181

config11_f1	    db      49T	    ; Config11 (originally 113T for 8 cyl) (182);* 182 Tuner Studio Config1 (= %00110001, $31, 49T)
;  Bit 0-1 = MAP Type
;            00 = MPX4115AP
;            01 = MPX4250AP
;            10 = MPXH6300A
...
config12_f1	    db      58T	; Config12 (originally 112T for 8 injectors);* 183 Tuner Studio Config2 (= %00111010, $3A, 58T)
...
config13_f1	    db      10T	    ; Config13                                  ;* 184 Tuner Studio Config3 (= %00001010, $A, 10T)
...
EGOrpm_f	    db      50T	    ; RPMOXLIMIT                                ;* 185 Tuner Studio egoRPM1 (= 50T (5000RPM, prohibit closed loop , default is 13T 1300 RPM)
FASTIDLEbg_f	db      234T	;                                           ;* 186 Tuner Studio fastIdleT1
O2targetV_f	    db      26T	    ; VOLTOXTARGET (187)                        ;* 187 Tuner Studio egoSwitch1
feature14_f1    db      %00000001   ; (188)  ; allows EGOigncount to be on page1 ;* 188 Tuner Studio egoIgnCount  = %00000001
...
KPARANGEST_f1:
	db      20T,27T,35T,42T,49T,56T,64T,71T,78T,85T,93T,10frd0T  ;(last byte 167) ;* 156 Tuner Studio mapBins3
				; KPARANGEST[0-b]

;; org $d3a8  ; stick them up out of the way at known values
TriggAngle_f	db      227T	; TriggAngle (60 deg)     (168)  *0.352            ;* 168 Tuner Studio TrigAng   (= 227T) (80 deg)
...
CrankAngle_f	db      56T	    ; Cranking advance (10deg)  *0.352 -28.4           ;* 171 Tuner Studio CrankAng  (= 56T)(10 deg)
				                ;  min -10 max 80
SparkHoldCyc_f	db      3T	    ; SparkHoldCyc (hold spark x cycles on             ;* 172 Tuner Studio IgHold    (= 3) (trigger signals)
				                ; stall and restart)
SparkConfig1_f	db   %00001000	; SparkConfig1 (Normal trigger, trigger            ;* 173 Tuner Studio Trig_plus  (= %00001000 )
...
KPARANGEST_f2:
        db      30T,40T,50T,60T,70T,80T,90T,100T,110T,120T,130T,140T        ;* 156 Tuner Studio mapBins4  (N/A)					   ; KPARANGEST[0-b]
...
feature13_f       db      3T                                                             ;* 16 Tuner Studio ?????         (= %00000011, $3, 3T)
...
slowIdleTemp_f	  db      200T	; Feilding 2-Wire Idle control Slow Idle                 ;* 18 Tuner Studio slowIdleTemp  (= %11001000, $C8, 200T)
...
feature11_f4    db  %00010001       ; (42)                                                  ;* 42 Tuner Studio ????? (=%00010001, $11, 17T)
...
tpsflood_f	    db      $F3	 ; Throttle position for floodclear mode in RAW ADC          ;* 44 Tuner Studio tpsflood (= 243T)
