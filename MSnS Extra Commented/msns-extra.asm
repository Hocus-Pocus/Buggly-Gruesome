...
; Set up the port data-direction registers
  ...
  sta   portd
  bset  launch,ptdpue
  bset  NosIn,ptdpue	; Set all the inputs internal pull ups On
...
;is PTC4 an input?    - see also 'B' code section
  ...
  bit   #wd_2trigb      ;* wd_2trigb = 1. Bit 1 is zero, go to line 939 (TS says bit 0 ????)
  beq   norm_op_ddrc
...
; Set up the Real-time clock Timer (TIM2)
  ...
  MOV   #%00110011,t2sc	 ; Stop Timer so it can be set up, No overflow interrupt, stop,
					     ; reset, div / 8
...
;  mov     #92T,T2CH0L	 ; Channel 0 low, 92 = 0.1 ms
  ...
  mov   #%01010100,T2SC1 ; Channel 1 Output compare, interrupt enabled
							 
; edis? mov     #%01010000,T2SC1	; Channel 1 Output compare, interrupt enabled

  bclr  TOF,T2SC1		 ; clear any pending interrupt
  bclr  TOIE,T2SC1		 ; Disable timer interrupt until we are ready
...
; Set up SCI port
  ...
  lda   #$30		 ; This is 9615 baud w/ the osc frequency 8.0M - DJLH
  sta   scbr
...
;clear water outputs
  ...
  brset out3sparkd,feature2,w_no3   ;* out3sparkd Bit 4 is clear
  bclr  Output3,portd
...
;  sta   baro
  ...
  mov   #$10,dwellush		    ; } high speed dwell delay, default of 4.1ms
...
; real angle. Shouldn't happen if angles set correctly.
  ...
  bhi   init_crang		        ; trigger angle > 20, continue  ;* TriggAngle_f = 80/.352=227, go to line 1183
  bset  nextcyl,EnhancedBits4
	
init_crang:
  lda   CrankAngle_f                ;* CrankAngle_f = 10/.352=28
  sta   SparkAngle
  brset EDIS,personality,init_edis  ;* EDIS,personality is clear, go to line 1190 
...
; this won't work for next-cyl but will be ignored at low rpm anyway
  ...
  lda   SparkHoldCyc_f   ;* = 0
  bra   init_cont        ;* go to line 1205
...
init_cont:
  ...
  brset WHEEL,personality,init_wheel   ;* WHEEL = 1, go to line 1211
  bra   init_no_hold
...
; if 2nd trig rising and falling then store existing value of pin to monitor state
  ...
  lda   dtmode_f
  bit   #trig2risefallb  ;* trig2risefallb =0, go to line 1231
  beq   init_no_hold
...
; see if inverted or non-inv output and use a quick bit
  ...
  bit   #M_SC1InvSpark         ;* #M_SC1InvSpark = 1, go to line 1245
  bne   inspk_inv
...
inspk_done:
  lda   p8feat1_f
  bit   #                      ;* rotary2b = 0, go to line 1256
  beq   not_init_rot
...
; decide if we are doing multiple wasted spark outputs
  ...
  brset WHEEL,personality,wsp_init   ;* WHEEL = 1, go to line 1267
...
;If HEI set bypass to 0v
  ...
  brclr HEI7,personality,not_hei7_init  ;* HEI7 =0, go to line 1283
  bset  aled,portc
	
not_hei7_init:

  brclr DUALEDIS,personality,chk_out   ;* DUALEDIS =0, go to line 1288
  bset  EDIS,personality	            ; DUALEDIS implies EDIS

chk_out:
**** add in some sanity checks for outputs vs. code base ****
  lda   personality      ;* personality byte is not clear, go to line 1297
  bne   check_out_config
	
;  lda   outputpins		; assumes if personality zero then any outputs are error
;  beq   b_dc			; no personality, no outputs
                        ;wrong!  This prevents "Fuel only", just check for conflicts

  bra   check3  ;* go to line 1323 
...
block_neon:
  ...
  mov   #2,tmp4
  bset  config_error,feature2	       ; if MSNEON but haven't reused led17&19 then error
  jmp   done_checks
...
check_msns:
  ...
  mov   #3,tmp4
  bset  config_error,feature2	      ; if MSNS and haven't reused FIDLE or LED17 then error
  jmp   done_checks
...
;  brclr PWMidle,feature2,check4
  lda   feature13_f
  bit   #pwmidleb    ;* pwmidleb =1, go to line 1328
  beq   check4
  brclr REUSE_FIDLE,outputpins,check4  ;* REUSE_FIDLE =0, go to line 1334
  mov   #4,tmp4
  bset  config_error,feature2	     ; trying to use PWM idle and spark on FIDLE
  jmp   done_checks

check4:                              ; check we don't have Water and Fan control as both use X2
  lda   feature3_f
  bit   #WaterInjb   ;* WaterInjb =0, go to line 1346
  beq   check5
...
check5:
  brclr Nitrous,feature1,check6   ;* Nitrous =0, go to line 1357
  
  lda   feature3_f
  bit   #WaterInjb   
  beq   check6    ;* WaterInjb is clear go to line 1357
...
check6:                         ; 7pin HEI must have spark output B (LED19) defined. For bypass output
  brclr HEI7,personality,check7   ;* HEI7 =0, go to line 1365
...
; coilc is the pain - set if LED18=1 and LED18_2=1
  ...
  brclr wspk,EnhancedBits4,check8	    ; don't bother if we aren't doing multiple outputs ;* wspk = 0
  brclr WHEEL,personality,check8                                                           ;* wheel = 1
  brset out3sparkd,feature2,ck74	    ; 4th output                                       ;* out3sparkd = 0
  brclr REUSE_LED18,outputpins,ck72	    ; not 3rd output                                   ;* REUSE_LED18 is clear, go to line 1389
...
ck71:
  ...
  beq   ck7err               
  bra   check7b				; passed all checks  ;* go to line 1406
...
check7b:	                    ; can't use FIDLE for spark if doing wasted spark
  brclr REUSE_FIDLE,outputpins,check7c  ;*REUSE-FIDLE = 0, go to line 1413
...
; first check for dual dizzy feature
  ...
  lda   feature6_f
  bit   #dualdizzyb                 ;* dualdizzyb = 0, go to line 1429
  bne   check8                      ; if dual dizzy then only 2 outputs anyway
  lda   trig4_f				        ; if trig4 pt set must have spark o/p d
  beq   ck7c3                       ;* trig4_f is clear go to line 1424
  brclr out3sparkd,feature2,ck7err	; 4th output
	
ck7c3:
  lda   trig3_f				        ; if trig3 pt set must have spark o/p c
  beq   check8                      ;* trig3_f is clear go to line 1430
  brclr REUSE_LED18,outputpins,ck7err
  brclr REUSE_LED18_2,outputpins,ck7err

check8:
  bra   check9    ;*  go to line 1468
  
; Do rotary2 output checks, must have led17,18,19 set to spark and two wheel triggers
...
check9:
  brclr REUSE_FIDLE,outputpins,check10  ;* REUSE_FIDLE = 0, go to line 1474
...
; count how many ignition types and if more than one give an error
  ...
  clra
  brclr MSNS,personality,check10a  ;* MSNS = 0, go to line 1482 
  
  inca
	
check10a:
  brclr MSNEON,personality,check10b ;* MSNEON = 0, go to line 1486 
  inca
...
check10c:
  brclr EDIS,personality,check10d ;* Edis = 0, go to line 1494 
  inca
	
check10d:
  brclr TFI,personality,check10e ;* = 0, go to line 1494
   inca
	
check10e:
  brclr HEI7,personality,check10f ;*  HE17 = 0, go to line 1502
  inca
	
check10f:
  cmp   #1
  bls   check11  ;* Accu A = 1, go to line 1510 
  mov   #14T,tmp4
  bset  config_error,feature2
  bra   done_checks
...
; Fire up the ADC, and perform three conversions to get the baro value, IAT and the clt temp
...
***************************************************************************
  ...
  bne  Two_Primes
;  brset PrimeTwice,feature6,Two_Primes	; Are we firing priming pulses twice?
  inc  TCSparkCut			     ; Add 1 to prime counter so it only does it once
  
Two_Primes:					     ; using spark cut byte to cut down bytes
  lda  feature11_f4
...
PrimeNow:
  ...
  lda  TCSparkCut
  cmp  #02T				       ; Have we reached prime pulse count limit?
  blo  Prime_Not_Done
...
Prime_Not_Done:
  ...
  beq  PrimeTable_P            ; Prime table ;* go to line 1595
...
PrimeTable_P:                   ; Interpolate from CLT, same curve as cranking PW.
  jsr   crankingModePrime
  lda   tmp6
  bra   prime

NotPrimed:					   ; If were here we must be priming late
  lda   secl
  cmp   #02T				   ; Have we been powered up for 2 secs?
...
prime:
  ...
  bset  sched1,squirt
  bset  inj1,squirt
  brclr CrankingPW2,feature1,CalcRunningParameters ; can skip prime on second channel
  sta   pw2
  clr   pwrun2
  bset  sched2,squirt
  bset  inj2,squirt
...
config_error1:
  bset  IMASK,INTSCR			; disable interrupts for IRQ (the ignition i/p)
  bclr  running,engine
  bset  fuelp,porta
;  bclr  wled,portc
...
dead_end:
  bset  fuelp,porta
  lda   tmp4
  beq   skip_err_msg			; if zero then don't try to send
...
; find start address or error message
  ...
  clr   tmp4				; wipe error code so we only send it once
...
CalcRunningParameters:
  clrh
  brset  OneShotBArro,EnhancedBits2,bEnd_of_Baro	; Only do this once as we may change the baro value
  ...
  bne    ConsBar
  lda    config13_f1
  bit    #c13_cs                ; Are we doing Alpha_n?      ;* bit2 = 0
  beq    OneShot_Bar            ; No so one shot baro only!  ;* go to line 1753
...
non_baro:
  bset  OneShotBArro,EnhancedBits2  ; only do this once not doing any baro, so use fixed Pambient and 100% baro correction
...
OneShot_Bar:
  ...
  bra   DoBaroCorr                   ;* go to line 1761
...
Do_Baro:
  ...
  lda   config11_f1
  and   #$03                    ; What MAP sensor? ;* bit 0-1 = 1 (4250) go to line 1783
  beq   do_baro4115
...
; do_baro_4250:
  ...
  sta   Pambient
  lda   BAROFAC4250,x
  bra   DoneBaroCorr     ;* go to line 1804
...
; now convert map ADC count into internal kpa
  ...
  ldx   map
  lda   config11_f1    ;* bit 0-1 = 1 (4250) 
  and   #$03
  cbeqa #1T,do_kpa4250  ;* go to line 1819
  cbeqa #2T,do_kpa6300
  cbeqa #3T,do_kpa6400
  bra   do_kpa4115
...
Donekpa:
  ...
  beq   Do_AirDens      ;* go to line 1854
...
;******** CHECK IF CORRECTING AIR DENSITY *****************
  ...
  beq   NormAirDen           ; If no then do normal air density           ;* go to line 1944
...
MATRel:
  sta   tmp4
  jsr   tableLookup          ; Find the lookup place for coolant corr in table
  clrh
  ldx   tmp5
...
; 50% re-written in 026i with aim of better odd-fire averaging
  ...
  ldhx   rpmph
  bne    dorpmCalc		 ; If zero then jump over calculation - prevent divide by zero
...
; If odd-fire is set (bit zero of Config13), then average RPM values
  ...
  lda     config13_f1
  and     #$01         ;* bits 1-2 = $02
  beq     NO_ODD_FIRE  ; go to line 2010
...
NO_ODD_FIRE:
  lda    tmp12
  beq    rpmCalcFast		; If we have only 8-bit denominator then use native divide
...
CalcPWs:
  lda   rpm
  cmp   crankRPM_f		                ; Check if we are cranking, ;* crankRPM_f = 3
  bhi   runIt
  brset cant_crank,EnhancedBits2,runIt 	; don't allow reentry to crank mode while running
  
crankIt:
  jsr   crankingMode
  jmp   checkRPMsettings :*  go to line 3818
...
Warm_Done_Now:
  ...
  bclr   wled,portc		                    ; not when crank sim or if LED re-used as IRQ indicator
...
Table_ASEStuff:
  ...
  jsr    tableLookup	; This finds the bins when the temperatures are set
  clrh
  ldx    tmp5
  lda    ASEVTbl_f,x
  sta    liY2
  decx
  lda    ASEVTbl_f,x    ; This finds the values for the ase percentage for the temperature
  sta    liY1
  mov    coolant,liX
  jsr    LinInterp		; tmp6 contains amount of ase enrichment in percent for this temperature
  clr    liX1
  lda    AWC_f1
  sta    liX2
  lda    tmp6			; Use the value from the interpolated table rather than the normal value
  sta    liY1
  clr    liY2
  clr    liX
  lda    feature10_f5
...
TAE_CALC:
  ...
  bit    #KpaDotSetb
  beq    tps_dotty
;  brclr  KpaDotSet,feature4,tps_dotty	; If not in KPA dot mode jump past KPa settings
  bit    #KpaDotBoostb
  beq    No_Boost_Chk
;  brclr  KpaDotBoost,feature4,No_Boost_Chk; Are we going to stop accel in boost?
  lda    kpa
  cmp    #100T
  bhi    TAE_CHK_JMP1		           ; If KPa above 100 then no accel deccel enrichment
...
AE_CHK:
  ...
  beq    tps_ThreshCheck
  lda    tmp31
  cmp    MAPthresh_f		          ; Are we above the Accel threshold for MAP?
  bhs    AE_SET
...
tps_ThreshCheck:
  lda    tmp31
  cmp    TPSthresh_f1		          ; Are we above the Accel threshold for TPS?
  bhs    AE_SET
  brclr  TPSAEN,ENGINE,acc_done_led   ; If we are not in AE mode then jump to end
  jmp    TAE_CHK_TIME
  
AE_SET:
  brset   PSAEN,ENGINE,AE_COMP_SHOOT_AMT
...
NormalBased_AE:
  ...
  bit    #KpaDotSetb
  beq    tps_FirstElem
  lda    MAPAQ_f	    ; start out using first element - will determine actual next time around
  bra    Store_TEA1

tps_FirstElem:
  lda    TPSAQ_f1		; start out using first element - will determine actual next time around
...
RPMBackAE:
  ...
  bset   TPSAEN,ENGINE
  bclr   TPSDEN,ENGINE
  brset  REUSE_LED19,outputpins,acc_done_led	; LED already used in NEON as coilb
  bset   aled,portc
...
AE_COMP_SHOOT_AMT:
  ...
  mov    #205T,liX2		; 165 + 40 degrees (because of offset in lookup table)
  lda    TPSACOLD_f1
  sta    liY1			; This is the amount at coldest
  clr    liY2			; no enrichment addon at warm temperature
  lda    coolant
  sta    liX
  jsr    LinInterp
...
Decay_AE_Aw:
  lda    AccelDecay_f
  cmp    TPSACCEL		    ; Only do interpolated Decay if Accel is higher than target point
  bhs    TAE_DONEJMP
  sta    liY2			    ; Load in the Decay PW value in mS at the end of the timer
  clr    lix1			    ; Acceltimer Start point for linear interpolater.
  lda    TPSASYNC_f1
  sta    lix2			    ; Stick the max time in lix2 for linear interpolater.
  lda    Decay_Accel
  sta    liY1			    ; Load in the actual maximum PW we calculated fo the Accel to interpolate from
  lda    TPSaclk
...
TDE:
  lda    feature6_f
  bit    #NoDecelBoostb		             ; Have we selected to use Decel all the time?
  beq    NormDecel
  lda    kpa
  cmp    DecelKpa_f
  bhi    TAE_DONE		                 ; If KPa above user defined amount then no decel enrichment
...
;  brclr   WaterInj,feature3,no_ego_w_chk
  brset  water,porta,SKIP_ALL_O2	       ; if water injection on skip both O2 checks
  
no_ego_w_chk:
  brset  NosSysOn,EnhancedBits,SKIP_ALL_O2 ; If NOS running then no O2 checks
  ...
  bit    #KPaTpsOpenb
  beq    throttle_check
;  brclr  KPaTpsOpen,feature3,throttle_check ; 0 = throttle do throttle check  1 = KPa
  lda    kpaO2_f			                 ; In KPa mode so is it higher than setpoint?
  beq    SETAFR_UP			                 ; If its zero dont check it as no open loop
  cmp    kpa
...
throttle_check:
  lda	 tpsO2_f		; Throttle position setpoint check for open loop
  beq    SETAFR_UP		; If its zero dont check it as no open loop
  cmp	 tps			; Load in TPS
  blo	 SKIP_ALL_O2
  bra    SETAFR_UP
...
CheckVE1:
  brset useVE3,EnhancedBits,Re_CheckTarg  ; Are we are using VE3 at present if so check if targets needed?
  bra   SETAFRTABLE

CheckVE3:
  brset useVE3,EnhancedBits,SETAFRTABLE	 ; If were not running in VE3 then no targets

SETAFRNORMAL:				             ; Normal setting for AFR
  lda    O2targetV_f
  sta    afrTarget

SETAFRTABLE:				             ; AFR Table value is already in afrTarget
  lda    ego
  sta    tmp32			                 ; Make tmp32 = the ego raw adc in narrow band or non AFR target mode
...
; Check if rich/lean
  ...
  clr    EGOcount
  lda    kpa			; See if we need to load in a new Ego Limit
  cmp    EgoLimitKPa_f
  bhi    New_EgoLim
...
EgoLim_Done:
  lda    config13_f1	; Check if Narrow-band (bit=0) or DIY-WB (bit=1)
  bit    #c13_o2		; Use BIT instead of brset because outside of zero-page
...
EGO_DONE:
  ...
  bit    #SecondO2b
  bne    DO_Second_Ego
;  brset  SecondO2,feature4,DO_Second_Ego	; Do we have a second O2 sensor?
...
Check2VE1:
  brset  useVE3,EnhancedBits,Re_Check2Targ	; Are we are using VE3 at present if so check if target needed?
  bra    SETAFRTABLE2

Check2VE3:
  brset  useVE3,EnhancedBits,SETAFRTABLE2	; If were not running in VE3 then no targets
...
AFTERAFRSET2:
  ...
  sta    tmp31			    ; We can re-use this as its reset every time
  bra    EgoLim2_Done
...
EgoLim2_Done:
  ...
  lda    config13_f2		; Check if Narrow-band (bit=0) or DIY-WB (bit=1)
  bit    #c13_o2			; Use BIT instead of brset because outside of zero-page
...
WARMACCEL_COMP:
  ...
  clr    tmp13			    ; tmp13 is zero
  jsr    Supernorm		    ; multiply and divide by 100 result in tmp10:tmp11
  ...
  bit    #alt_i1ge		    ; if 0 then set it to 100T to remove GammaE.
  bne    ld_ve_1
  mov    #100T,GammaE
  bra    ld_ve_1Done
...
ld_ve_1Done:
  ...
  brset  hybridAlphaN,feature1,skip_loadcontcomp ; if hybrid then skip AN bypass
  lda    config13_f1
  bit    #c13_cs
...
BATT_CORR_PW:
  ...
  clr    liY2			; Screws up slope, but gives reasonable result.
...
CALC_FINAL:
  ...
  beq    PW_Done		; If no calculated pulse, then don't open at all.
...
ld_ve2_Done:
  mov    EGOcorr2,tmp10		                    ; closed-loop correction percent into tmp10
  clr    tmp11			                        ; remainder is zero
  brset  hybridAlphaN,feature1,skip_loadccomp2	; if hybrid then skip AN bypass
  lda    config13_f2
  bit    #c13_cs
...
***************************************************************************
  ...
  lda    config11_f1  ;* config11_f1 = %00110001
  and    #$03                       ;* %00000011
  cmp    #$02			; Are we using Turbo Map sensor?
  blo    CALC_FINAL2    ; skip if 0 or 1  ;* go to line 3779
...
CALC_FINAL2:
  ...
  lda    tmp11			; From required fuel, above.
  beq    PW2_Done			; If no calculated pulse, then don't open at all.
  add    tmp6			; from batt correction
  bcs    MaxPulse2
  add    TPSACCEL
  bcs    MaxPulse2
...
PW2_calc:
  clr    tmp2
  lda    DTmode_f
  bit    #alt_i2t2		   ; if inj2 is not driven from table1 then skip
  bne    pw2_table2
...
; Do all the rpm related stuff here.
  ...
  brclr  ShiftLight,feature2,ShiftLightDone  ;* ShiftLight = 0, go to line 3865
...
; Hard Cut Rev and Launch checks
  ...
  brclr   LaunchControl,feature2,LaunchDone ; Is Launch selected? ;* LaunchControl = 0, go to line 3922
  
; Changes to launch system for 025y - JSM

  brset   Launch,portd,Reset_VL		; Button not pressed so reset variable bit
  lda     VlaunchLimit
...
; ***Over Boost Protection**********************************
  ...
  cmp    #101T
  blo    BoostP_Done	      ; If set to less than 100KPa then no boost protection  ; * go to line 3945
  cmp    kpa				  ; Is the kpa higher than the boost safety high limit?
  bhi    BoostP_Done
  lda    feature5_f
  bit    #BoostCutb
  beq    B_SparkFuel           ; Spark Cut Mode?
  lda    SparkCutCnt
  cmp    SparkCutBNum_f		   ; Have we sparked more than the user defined number?
  bhi    B_SparkFuel		   ; Yes so dont cut any more sparks
...
; implement fuel cut from rev limiter soft limiter
  ...
  brset   RevLimHSoft,RevLimBits,Chk_Rev_Cuts  ;* RevLimHSoft = 0
...
checkHighLimit:
  lda    revLimit_f                             ;* revLimit = 250T
  beq    checkRevsOk	; Zero means no limit
  cmp    rpm
  bhs    checkRevsOk	; We have not hit any   ;* go to line 4009 
						; Rev limits
...
Chk_Rev_Cuts:
  ...
  beq    FuelCut_C
  lda    SparkCutCnt			; We are in spark cut only mode so how many sparks are we at?
  cmp    SparkCutNum_f			; User defined spark number
  bhi    Fuelcut_C			    ; If spark count higher than number dont set spark cut bit
...
Chk_Cuts:
  ...
  lda    SparkCutCnt			; We are in spark cut mode so how many sparks are we at?
  cmp    SparkCutNLC_f			; User defined spark number for Launch
  bhi    SparkFuel_LC			; If spark count higher than number dont set spark cut bit
  bset   sparkCut,RevLimBits	; Set sparkcut bit
...
cutChannels:
  ...
  mov    tmp1,pwcalc1
  mov    tmp2,pwcalc2
  jmp    spark_lookup			; In fuel cut mode so return with zeros
  
checkRevsOk:
  brclr  Traction,EnhancedBits2,No_Traction_On
  lda    TCSparkCut             ;* TCSSparkCut = 0
  beq    No_Traction_On			; If zero then no spark cut  ;* go to line 4017
  cmp    SparkCutCnt			; In traction mode do we cut sparks
  bls    No_Traction_On
  bset   sparkCut,RevLimBits    ; Set sparkcut bit

No_Traction_On:
  brset  OverRun,EnhancedBits,cutChannels; If Over run fuel cut on cut fuel ;* OverRun = 0

; Add in the NOS and Staged PW's here

  lda    feature5_f     feature5_f = %00110011
  bit    #stagedeither  stagedeither = %1100
  bne    Add_to_PWCALC
;  brset    staged,feature5,Add_to_PWCALC     ; If in Staged mode Add to PW1+2
;  brset    stagedMode,feature5,Add_to_PWCALC ; If in Staged mode Add to PW1+2
  brset  Nitrous,feature1,Add_to_PWCALC       ; If NOS System selected add to PW1+2 ;* Nitrous = 0
  brset  crank,engine,No_TCAccel
  lda    tmp1
  add    TCAccel
  sta    tmp1
  lda    tmp2
  add    TCAccel			; Add in the traction control enrichments
  sta    tmp2
  
No_TCAccel:
  mov    tmp1,pwcalc1
  mov    tmp2,pwcalc2
  jmp    spark_lookup   ;* go to line 4108
...
Dont_Nos_PW1:
  brclr  REStaging,EnhancedBits,No_Staging      ; Staging not running so dont add PW Staging
  lda    pw_staged
  sta    tmp1
...
Staging_2_PW:
  brclr  REStaging,EnhancedBits,No_PW2_Staging	; Staging Mode not running so no PW2
  lda    pw_staged2
  sta    tmp2
  bra    Staging_Done_PW
  
No_PW2_Staging:
  clr    tmp2  				; In Staging Mode but not running so PW2 = 0
...
Staging_Done_PW:
  ...
  lda    feature4_f
  bit    #DtNosb
  beq    Calc_PWs_DONE      ; In DT mode so do we add NosPW to PW2?
...
spark_lookup:
  lda    personality_f   ; Are we using a spark mode? ;*personality = %00000100
  beq    No_Personality
  lda    page
  cmp    #3
  bne    fixed_fl
...
fixed_fl:
  lda    FixedAngle_f  ;* -10 for use map, 56T for 10 degrees
  
fxr_c:
  cmp    #$03
  blo    NOT_FIXED	    ; Added this as earlier MT didnt send a perfect 00T for -10 (use map)
;; sta    SparkAngle	 ; else use this fixed advance
  jmp    CALC_DELAY
  
NOT_FIXED:
  brclr  LaunchOn,RevLimBits,Not_LC_in               ;* LaunchOn = 0, go to line 4141
...
Not_LC_in:
  ...
  cmp	 #$03
  blo	 use_spark_table      ;* go to line 4190
...
use_spark_table:
  brclr  RevLimSoft,RevLimBits,STTABLELOOKUP  ;*RevLimSoft = 0, go to line 4227
...
; variable is MAP, not kpa
  ...
  lda    feature9_f
  bit    #MassAirFlwb    ;* MassAirFlwb = 0
  beq    SD_ALPHa_N      ; Are we using a MAF on pin X7?  ;* go to line 4241
  lda    o2_fpadc        ; Using MAF thats on pin X7
  sta    kpa_n
  bra    ST_STEP_1

SD_ALPHa_N:
  lda    config13_f1	; Check if in speed-density or Aplha-N mode
  bit    #$04			; Use BIT instead of brset because outside of zero-page ;* bit2 = 0
  beq    Kpa_n_Kpa		; Branch if the bit is clear  ;* go to line 4249
  lda    tps            ; Alpha_N Mode
  sta    kpa_n			; Added so as KPa can be used elsewhere in code
  bra    ST_STEP_1

Kpa_n_Kpa:              ; Speed Den Mode
  lda    kpa
  sta    kpa_n			; Added so as KPa can be used else where in code
...
ST2_STEP_1:
  ...
  beq    LookUp_Done                        ;* go to line 4359
...
LookUp_Done:
  jmp    LookUp_Finished   ;* go to line 4431
...
Not_ST1:
  lda    page
  cmp    #3      ;* branch if page is not = 3
  bne    trim_fl
  lda    TrimAngle_r
  bra    trim_c
  
trim_fl: 
  lda    TrimAngle_f    ; TrimAngle_f = 0
...
trim_c: 
  ...
  lda    #$00			; Negative trim over to high advance, clamp to 0
  bra    TRIM_DONE
...
TRIM_DONE:
  ...
  lda    CrankAngle_f		         ; Update spark angle for User Interface crankAngle_f = 56T
  bra    TRIM_DONE2                  ;* go to line 4469
...
TRIM_DONE2:
;  bmi    store_spark	            ; Check if result negative (i.e. > 10ATDC)
;  lda    #0			            ; Clamp to minimum (surely safer?)

store_spark:
  brset  crank,engine,store_spark2	; if we are cranking skip to the save
  add    CltIatAngle                ;* CltIatAngle = 0
  add    KnockAngleRet              ;* KnockAngleRet = 0
  add    NitrousAngle               ;* NitrousAngle = 0
  clc				                ; Clear carry bit **
  add    TCAngle
  bcc    Store_Spark_Ang		    ; Did we over flow with the traction angle? **
        lda     #28T			    ; Yes so limit angle to 0 deg **
		
Store_Spark_Ang:
  brclr  LaunchOn,RevLimBits,store_spark2  ;* LaunchOn = 0 go to line 4492
...
CALC_DELAY:
  ...
  brclr  nextcyl,EnhancedBits4,this_cyl  ;* next_cyl = 0, go to line 4523
...
this_cyl:
  stx    SparkAngle
  lda    TriggAngle_f   ;* TriggAngle_f - 227T
  sub    SparkAngle
  add    #28T
  sta    DelayAngle
  bra    CheckSoftLimit  ;* go to line 4595
...
;  brclr   multispark,feature4,NOT_MULTI
  lda    feature4_f		      ; this allow multi spark on/off while running
  bit    #multisparkb
  beq    NOT_MULTI
  lda    rpm
  cmp    edisms_f
...
CheckSoftLimit:
  bclr   LaunchOn,RevLimBits	                    ; Clear the Soft Launch Rev Limit bit
  brclr  LaunchControl,feature2,Magnus_revlimiters  ; Is Launch selected?  ;* LaunchControl = 0, go to line 4619
  brset  Launch,portd,Magnus_revlimiters	        ; Button not pressed so reset variable bit
  lda    tps			                            ; Is throttle in right place?
  cmp	 LC_Throttle_f
...
Magnus_revlimiters:
  ...
  beq    SRevLimOnDone		              ; skip if zero  ;* go to line 4642
  cmp    rpm
  blo    SRevLimOn		                  ; rpm higher than limit
  bhi    SRevLimOff		                  ; rpm lower than limit
  brset  RevLimSoft,RevLimBits,SRevLimOn  ; at limit check current status
...
;  sta     SRevLimCoolLeft
  lda    SRevLimTimeLeft		              ; Check if time left = counting down
  bne    SRevLimOnDone
  brset  RevLimHSoft,RevLimBits,SRevLimOnDone ; Check if soft limit has cut fuel
  lda    SRevLimHTime		                  ; Set delay time for soft limit to cut fuel
  sta    SRevLimTimeLeft
...
CheckOutputs:
  clrh
  brset  BoostControl,feature2,Out1DoneJMP; If Boost control used then no output1  ;* BoostControl = 0
  ...
  cmp    #06T
  beq    CLT1Source
  bra    Not_Temps1   ;* go to line 4698
...
Not_Temps1:
  ...
  beq    Out1Done	    ; No source = no check ;* go to line 4751
  lda    secl,x		    ; Get data
  sta    tmp31
  cmp    Out1Lim		; Check limit
  bhi    Out1On		    ; Above limit, set output
...
Hyster1:
  brclr  Output1On,Enhancedbits2,Out1Off   ; Is output 1 off? If so carry on as normal
  lda    Out1Lim
  sub    Out1Hys_f	                       ; Subtract Hysterisis for output1 from Out1 limit
  cmp    tmp31		                       ; Actual value
  bls    Out1Done	                       ; If actual value higher than Limit-Hysterisis then dont clear output
...
TractOut1:
  brset  Traction,EnhancedBits2,No_Upper_Lim1	; If traction Running set output
  bra    Out1Off

Out1On:
  lda    Out1UpLim_f	          ; Upper limit. Creates a window for output to work in
  beq    o_Upper_Lim1	          ; If zero no limit
  cmp    secl,x
  bhi    No_Upper_Lim1	          ; If higher than setpoint dont clear output
  lda    feature4_f
  bit    #InvertOutOneb
  bne    out1_set
...
Out1Done:
  ...
  cmp    #06T
  beq    CLT2Source
  bra    Not_Temps2               ;* go to line 4793
...
Not_Temps2:
  ...
  beq    Out2Done	    ; No source = no check ;* go to line 4851
  lda    secl,x		    ; Get data
  sta    tmp31
  cmp    Out2Lim		; Check limit
  bhi    Out2On		    ; Above limit, set output
...
Hyster2:
  brclr  Output2On,Enhancedbits2,Out2Off	; Is output 1 off? If so carry on as normal
  lda    Out2Lim
  sub    Out2Hys_f	              ; Subtract Hysterisis for output1 from Out1 limit
  cmp    tmp31		              ; Actual value
...
TractOut2:
  brset  Traction,EnhancedBits2,No_Upper_Lim2	; If traction Running set output
  bra    Out2Off

Out2On:
  lda    Out2UpLim_f	; Upper limit. Creates a window for output to work in
  beq    No_Upper_Lim2	; If zero no limit
  cmp    secl,x
  bhi    No_Upper_Lim2	; If higher than setpoint dont clear output
  lda    feature4_f
  bit    #InvertOutTwob
  bne    out2_set
...
*******************************************************************************
  ...
  brset  out3sparkd,feature2,out3done     ;* out3sparkd = 0, go to line 4933
  clrh
  lda     feature8_f
  bit     #Out1_Out3b	        ; Are we in Out1+ mode?
  beq     Norm_Out3_check
  brclr   Output1,porta,Out3Off	; If Output1 is off then don't do any checks for Out3
...
;  cmp    #01T
;  beq    Tract_Output3	         ; If source = 1 then traction to activate output
  cmp    #31T
  beq    Tract_Output3	        ; If source = 31 then traction to activate output
  cmp    #02
  beq    DEC_Output3	        ; If source = 2 then we are using decel to activate output
  cmp    #03T
  beq    ACEL_Output3	        ; If source = 3 then we are using accel to activate output
  cmp    #05T		            ; Are we using temperature?
  beq    IAT3Source
  cmp    #06T
  beq    CLT3Source
...
Out2_Out3:
  brclr  Output2,porta,Out3Off	; If Output2 on then turn output3 on
  bra    Out3On

Tract_Output3:
  brclr  Traction,EnhancedBits2,Out3Off	; If traction Running set output
  bra    Out3On
...
; for those using WATER INJECTION on X2
  ...
  brset  REUSE_LED18,outputpins,Out4Done	; being used as IRQ or COIL C       ;* REUSE_LED18 = 0, go to line 4998
  brclr  REUSE_LED18_2,outputpins,Out4Done	; Are we re using LED18 as output4?
  brset  LED18_FAN,outputpins,Out4Done	    ; Are we using it as fan control?
  clrh
  lda    Out4Source_f
  bit    #$0f		    ; Only use 5 bits of this byte
  beq    Out4Done	    ; No source = no check
;  cmp    #01T
;  beq    Tract_Output4	; If source = 1 then traction to activate output
  cmp    #31T
  beq    Tract_Output4	; If source = 31 then traction to activate output
  cmp    #02T
  beq    DEC_Output4	; If source = 2 then we are using decel to activate output
  cmp    #03T
  beq    ACEL_Output4	; If source = 3 then we are using accel to activate output
  cmp    #05T		    ; Are we using temperature?
  beq    IAT4Source
  cmp    #06T
  beq    CLT4Source
...
Tract_Output4:
  brclr  Traction,EnhancedBits2,Out4Off	; If traction Running set output
  bra    Out4On
...
***************************************************************************
  ...
  brset  X2_FAN,outputpins,DO_FAN_Check	    ; Are we using X2 as fan control?  ;* X2_FAN = 1, go to line 5015
  brset  REUSE_LED18,outputpins,fan_exit
  brclr  REUSE_LED18_2,outputpins,fan_exit
  brset  LED18_FAN,outputpins,DO_FAN_Check	; Are we using LED18 as fan control?
...
DO_FAN_Check:
  ...
  lda    coolant
  cmp    EfanOnTemp_f   ;* EfanOnTemp = 255T (295 degree F, no fan control)
  bhi    FAN_ON
  cmp    EfanOffTemp_f     ;* EfanOffTemp = 250T (290 degree F, no fan control)
  blo    FAN_OFF           ;* go to line 5025
  bra    FAN_DONE

FAN_OFF:
  brclr  X2_FAN,outputpins,No_FAN_Porta	 ; Are we using X2?  ;* X2_FAN = 1
  bclr   water,porta		              ; sharing X2 with water inj output
  
No_FAN_Porta:
  brclr  LED18_FAN,outputpins,FAN_DONE	  ; Are we using LED18?  ;* LED18_FAN = 0, go to line 5041
  bclr   wled,portc
  bra    FAN_DONE
...
Over_Run:
  lda    feature4_f
  bit    #OverRunOnb               ;* OverRunOnb = 0
  beq    Over_Run_Done             ;* go to line 5088
...
No_Over_Run:
  ...
  lda    #00T
  sta    OverRunTime		       ; Reset the over run timer once per over run
...
***************************************************************************
  ...
  beq    Water_Inj_Done         ;* go to line 5129

Water_Injection:			    ; we only get here if water inj is enabled
  brset  water,porta,ignore_iat ;If water on then dont check IAT again
...
*****************************************************************************
  ...
  beq    retard_endJmp       ;* go to line 5203
...
IatClt_Related:
  ...
  cmp    coolant			; Are we within 5 degrees F of setpoint for clt advance?
  blo    Advance_end		;
  lda    coolant			;
  cmp    cltAdvance_f		; Is the clt under the setpoint?
  blo    carryOn_Advance	; If so carry on with advance
  lda    #$00
  sta    CltIatAngle		; If not then it's in dead band so clear trimAngle
...
carryOn_Advance:
  ...
  beq    Advance_end	; If zero no Advance
  lsra				    ; Shift bit pattern to the right (Divide by 2)
  bcc    nota_carry		; Check if carry bit clear, skip increment
  inca				    ; otherwise, increment accumulator
  
nota_carry:
  sta    tmp31			; Stores half the cltDeg (used for checking division)
  lda    cltAdvance_f	; Load into the accumulator the top temperature limit
  sub    coolant		; How much cooler are we?
  clrh				    ; Zero out high 8 bits of 16-bit H:X register Accumulator contains low 8 bits
  ...
  pula
  cmp    tmp31			; See if the remainder is more than half of divisor
  blo    roundedAdvance
...
retard_endJmp:
  jmp    retard_end		; If Coolant advance running dont  ;* go to line 5261
					    ; check IAT retard
...
*****************************************************************************
  ...
  lsra				        ; Shift bit pattern to the right Divide by 2)
  bcc    no_carry			; Check if carry bit clear, skip increment
  inca				        ; otherwise, increment accumulator
...
no_carry:
  ...
  blo    clr_Retard			; If not reached make sure we clear the retard angle
  lda    airTemp			; Actual IAT Temp
  cmp    iatDanger_f		; Setpoint for start of retard
  blo    clr_Retard			; If not reached make sure we clear the retard angle
  sub    iatDanger_f		; How much higher are we? Leaves difference in accumulator
  clrh				        ; Zero out high 8 bits of 16-bit H:X register Accumulator contains low 8 bits
  ...
  pshh				        ; Transfer remainder to accumulator
  pula
  cmp    tmp32			    ; See if the remainder is more than half of divisor
  blo    roundedRetard
  incx				        ; It was a big remainder, round up.
...
roundedRetard:
  lda    #3T				; 1 degree
  mul				        ; X * A -> (X:A)
  sta    tmp31			    ; Store angle to retard, its an advance angle at the moment
  lda    #255T			    ;
  sub    tmp31			    ; (255-angle to retard) turns it into a retard angle
  cpx    #0T				; See if we overflowed, i.e., X != 0
...
***************************************************************************
  ...
  brset   REUSE_FIDLE,outputpins,idle_DoneJMP1  ;* REUSE_FIDLE = 0

IdleAdjust:
;  brset  PWMidle,feature2,idlePWM
  lda    feature13_f
  bit    #pwmidleb   ;* pwmidleb = 1
  bne    idlePWM     ;* go to line 5314
...
idleToggle:
  ...
  bls    idleFast		; Shouldn't there be some hysteresis here?  On the other hand, the
					    ; temp should never hover around here, so why bother?
idleSlow:
...
idlePWM:
  brset  istartbit,EnhancedBits6,Crank_PWM	; loop to stabilize on startup  ;* istartbit = 0
...
;  brset  crank,engine,jeskipAdjust	        ; Don't adjust idle during cranking
  lda    feature13_f     
  bit    #idle_warmupb                      ; idle_warmup = 1
  beq    idle_closedloop                   
...
; determine duty cycle by linear interpolation
  ...
  jsr    lininterp
  mov    liY,idleDC
  bra	 idle_closedloop   ;* go to line 5395

Idle_doneJMP1:
  jmp	 Idle_done          ;* go to line 5552
...
idle_closedloop:
;  brclridle_cl,feature7,Idle_doneJMP1
  lda    feature13_f
  bit    #idle_clb          ;* idle_clb = 0
  beq    idle_DoneJMP1      ;* go to line 5491
  lda    tps
...
Knock_Detection:
  ...
  beq    End_KnockJmp                    ; knock not enabled  ;* go to line 5655
  lda    feature8_f
  bit    #spkfopb
  bne    End_KnockJmp                    ; Spark output F enabled, incompatible
  clrh
  lda    rpm
  cmp    KnockRpmL_f		             ; Is the engine rpm too high for the knock sensor?
  bhi    Clr_KnockJmp		             ; If it is clear values, no more retard
  cmp    KnockRpmLL_f		             ; Is it running lower than the low rpm setpoint?
  blo    Clr_KnockJmp		             ; If so clear all values, no more retard
  lda    kpa
  cmp    KnockKpaL_f		             ; Is the boost above the limit for knock system?
  bhi    Clr_KnockJmp		             ; If it is clear knock values, no more retard.
  brset  Knocked,SparkBits,KnockTLeft	 ; If knock has been previously detected do timer
  brset  Advancing,RevLimBits,KnockALeft ; If we are advancing back
  brset  KnockIn,portd,End_KnockJmp	     ; If no knock on input then no knock
...
KnockTLeft:
  ...
  lda    KnockTimLft
  cmp    #00T
  beq    NoTimeLeft		                ; If timer counted down then add some advance
  jmp    End_KnockJmp		            ; End of Knock

KnockALeft:
  bclr   Knocked,SparkBits	            ; Clear Retard set bit as we are advancing
...
NoTimeLeft:
  ...
  lda    Boostknock_f
  beq    No_BoostKnock		     ; if no Boost Knock value then jump past checks
...
No_BoostKnock:
  lda    KnockAngle		         ; No Knock detected and time period over
  sub    KnockAdv_f		         ; so remove some retard

StoreKnock:
  cmp    #03T
  blo    ClearTime		         ; If retard is less than 1deg clear timer, we have finished
  cmp    #85T
  bhi    ClearTime		         ; If we are above 30 Degrees then somethings wrong so clear retard
  sta    KnockAngle
  jmp    Start_KnockTime
...
End_KnockJmp:
  jmp    End_Knock              ;* go to line 5708

ClearTime:				        ; No Knocks and retard back to start so clear everything.
  lda    #00T
...
Knocking_Still:
  ...
  bclr   Advancing,RevLimBits	; Clear the advance bit as we are in knock retard
  lda    KnockBoost
...
Store_Boost_Remove:
  sta    KnockBoost
  lda    KnockRet2_f		
  add    KnockAngle		        ; add the knock retard angle2 to knock angle
...
********************************************************************************
  ...
  beq   Traction_DoneJMP                     ;* go to line 5817

TractionSystem:
  brclr  running,engine,No_TC_Yet            ; Only use it if engine running
  brset  crank,engine,No_TC_Yet              ; Dont use it during cranking as we use sometraction bytes
  brset  startw,engine,No_TC_Yet	         ; only use Anti-Rev when after start enrichment over
  brset  WheelSensor,feature7,No_RPM_Thresh	 ; If using wheel sensors then no need to look at rpm
...
NO_TC_Loss:
  brset  Traction,EnhancedBits2,reset_TC_Yet ; Have we selected to wait till cycle counter timed out?
...
Carry_LC:				                    ; Acc contains result
  ...
  bmi    NO_TC_Loss		                    ; Drive wheels slower than undriven, no TC

; Interpolate to find allowable slip depending on vehicle speed
; (store allowed slip in tmp31)

  clr    liX1			                    ; Set minimum speed to 00
  lda    #127T			                    ; Set maximum speed for interpolater to half speed
  sta    liX2
  lda    TCSlipFac_f		                ; Slip allowed at minimum speed (00 liX1)
  sta    liY1
  lda    TCSlipFacH_f		                ; Slip allowed at half speed
  sta    liY2
  lda    o2_fpadc			                ; Actual speed were running at
  sta    liX
  jsr    LinInterp		                    ; Go and find out what slip is allowed at current speed
  sta    tmp31

; Find out if we are slipping over the amount allowed

  ldx    tmp32			                    ; Load x reg with speed undriven wheels should be (calculated from driven wheel)
  lda    tmp31			                    ; Multiply by the allowable difference factor (slip allowed)
  mul
...
Carry_Slip:
  add    o2_fpadc		                    ; Acc = speed of undriven wheel + slip allowed
  bcs    NO_TC_Loss		                    ; If we go over 255 then no traction
  cmp    tmp32			                    ; Compare to calculated speed of undriven wheel
  bhi    NO_TC_Loss		                    ; Were not over limit so no TC
  sta    tmp31			                    ; Store speed of undriven wheel + slip allowed
  lda    tmp32			                    ; Load calculated value of undriven wheel
  sub    tmp31
  sta    tmp32			                    ; Store amount of traction loss
  bra    VSSThresh_RJMP

Traction_DoneJMP:
  jmp    Traction_Done   ;* go to line 5975

; If were here weve had Anti-Rev working and rpm is stable so do we
; reset it yet or later?

reset_TC_Yet:
  brclr  TCcycleSec,feature7,Reset_TC_Now   ; Reset it now if stable rpm selected
  bra    Check_TC_Counter	                ; Not reseting on stable rpm so check cycle counter

RPM_Thresh:
  lda    rpm
  sub    rpmlast
  cmp    RPMthresh_f		                ; Have we increased rpm above the threshold?
  bhs    Thresh_Reach

Check_TC_Counter:
  lda    ASEcount		                    ; Use after start warmup counter as its only used for a few seconds on start up.
  cmp    TCCycles
  blo    Dont_Reset_Tract	                ; Only reset angle and accel enrich after nn cycles
  bclr   Traction,EnhancedBits2	            ; Clear the traction control bit
  lda    #00T
  sta    TCAngle
  sta    TCAccel
...
; the rest of the interpolaters
  ...
  ldhx   #rpmdotrate		                 ; Store address for finding rate of change
  sthx   tmp1
  ...
  clrh
  lda    tmp5			                     ; Put Address value from lookup into x reg
  tax
...
; Find percentage of loss: Undriven wheel/100 * loss of traction (tmp32)
  ...
  pshh
  pula
  cmp    #50T			                      ; Is remainder higher than half divisor?
  blo    Round_Slip_Per
  incx
...
Prime_Checked:
  brclr  BoostControl,feature2,no_boost    ;* BoostControl = 0, go to line 5990
  jsr    CalcBoostDC

no_boost:
  brclr  Nitrous,feature1,no_nitrous       ;* Nitrous = 0, go to line 5994
  jsr    EnableN2O

no_nitrous:
  lda    feature6_f
  bit    #VETable3b                        ;* VETable3b = 0
  beq    No_VE_Table_3                     ;* go to line 6000
  jsr    Check_VE3_Table

No_VE_Table_3:
  lda    feature3_f
  bit    #TargetAFRb                        ;* Target AFRb = 0
  beq    No_AFRTar_VE1                      ; Are we using the target afrs for table 1?  ;* go to line 6006
  jsr    AFR1_Targets                       ; Get Target AFR from table 1 for VE 1
  
NO_AfrTar_VE1:
  lda    feature6_f
  bit    #TargetAFR3b                       ;* TargetAFR3b = 0
  beq    No_AfrTar_VE3                      ;* go to line 6012
  jsr    AFR2_Targets                       ; Are we using the target afrs for table 3?
            		                        ; Get Target AFR from table 2 for VE 3
No_AfrTar_VE3:
  brset  running,engine,nospkoff            ; skip next check if not running then make sure all spark outputs are OFF
                                            ; this is a bandaid, but better safe than sorry
  jsr    turnallsparkoff                    ; subroutine to stop them all
  
nospkoff:
  lda    personality_f                      ;* personality_f = 1
  beq    No_misc_Spark                      ;* go to line 7090
...
; hasn't.
  ...
  brclr  checkbit,EnhancedBits5,troll_ck_done  ; ok oh dear, we've missed it
  lda    T2CNTL                                ; unlatch any previous read
  lda    T2CNTH
  sta    tmp1
  lda    T2CNTL
...
troll_ck_done:
  ...
  jmp    CalcRunningParameters                  ; Start main loop again  ;* go to line 1719
...
crankingMode:
   brclr running,engine,ExtraFuelCrank   ; We are stopped so do we add extra fuel whilst cranking?

crankingModePrime:
  bset   crank,engine		; Turn on cranking mode.
  bclr   startw,engine		; Turn off ASE mode.
  bclr   warmup,engine		; Turn off WUE mode.
  lda    tps			    ; ~70% comparison value for throttle - flood clear trigger
  cmp    tpsflood_f         ;* tpsflood_f = 243T
  blo    crankingPW
  clr    TCAccel

floodClear:
  clra				        ; Turn off pulses altogether.
  jmp    crankingDone       ;* go to line 6203
...
ExtraFuelCrank:
  ...
  beq    floodClear                         ; If Extra Cranking Fuel not selected then carry on as normal
  lda    tps
  cmp    tpsflood_f                         ;* tpsflood_f = 243T
  bhs    HighTPS                            ; Is the TPS higher than the floodclear value?
  brclr  NosDcOk,EnhancedBits,floodClear    ; No so go back to clearing PW ;* NosDcOk = 0, go to line 6095
...
; choose coolant, airtemp or average
  ...
  lda    feature11_f4
  bit    #matcrankb      ;* matcranfb = 0
  bne    crpwmat         ;* fall through
  
crpwclt:				    ; if cltcrank bit is 1 or 0
  lda    coolant
  bra    crpwint           ;*  go to line 6165
...
; Table look up for Cranking PW, liX already contains temperature to look for - PR
  ...
  mov    liX,tmp4		                     ; tmp4 holds the variable to look for in the lookup table
  ...
  sta    liY2
  decx
  lda    CrankPWs_f,x	                     ; This finds the values for the PW at the above temperatures
  sta    liY1
  mov    tmp10,liX
  jsr    LinInterp
...
;  sta    egtadc
  sta    tmp1
  brclr  CrankingPW2,feature1,no_crankpw2  ;* CrankingPW2 = 0, go to line 6210
...
CALC_STAGED_PW:
  ...
  lda    SCALEFAC_f		    ; load SCALEFAC constant into accumulator
  mul				        ; multipy the two together, 16-bit result in x:a
  txa				        ; transfer high byte of pw*ScaleFac to accumlator, overwriting existing lower byte.
...
; if the count is 0, change to 1 to avoid instant transition
; redundant	cmp	#00T
...
; now we have the amt to set pw_staged2 to add back the open time
...
continue_pw_1:
  ...
  add	 tmp6
  sta	 pw_staged
  cmp	 pw_staged2         ; if pw_staged2 is greater than pw_staged, we probably rounded, so use the original
			                ; pw_staged instead
  pulh
  bhi	 check_staged_on	; we're done here, go see if staging should be on or not.
...
cont_staged_off:
  ...
  clra
  sta	 stgTransitionCnt	; staged is off, clear the staging transition count
  sta	 pw_staged2
...
; boost control TABLE 3 for switching boost table on the run
; gets a byte from page8 or RAM.
; On entry X contains index.
; Returns byte in A
...
; rotary trailing split	for switching boost table on the run
; gets a byte from flash or RAM.
; On entry X contains index.
; Returns byte in A
...
CalcBoostDC:
  lda    kpa
  cmp    Pambient
  blo    boostZero		; If no boost sensed, don't burn up the solenoid. is this good or bad??
  lda    bcCtlClock		; RTC-based updates. Would it be better to use engine revs
...
;  brclr  BoostTable3,feature8,BoostT1
  ...
  bit    #BoostTable3b
  beq    BoostT1
  brclr  NosIn,portd,Table3_Boost       ; If using Boost Table 3 and input low then its time to use it
...
*****************************************************************************
; The real boost controller, compute delta.

  lda    bcSetPoint
  sub    KnockBoost		; Subtract boost target with knock detection boost value
  sta    bcSetPoint
...
*****************************************************************************
  ...
  beq    noBoostIAT			; If zero then dont go any further.
  lsra				        ; Shift bit pattern to the right (Divide by 2)
  bcc    no_carry_B			; Check if carry bit clear, skip increment
  inca				        ; otherwise, increment accumulator
...
no_carry_B:
  ...
  blo    noBoostIAT
  sub    iatBoostSt_f		; How much higher are we? Leaves difference in accumulator
  clrh				        ; Zero out high 8 bits of 16-bit H:X register
...
boostPos:
  ldx    bcPgain_f		; Proportional Gain in percent, 255=100%.
  mul				    ; returns in x:a
  stx    bcP			; just high byte bcP = abs(kpa-bcSetpoint) * (Pgain / 256)
  lda    kpa			; now calc 'kpadot' in here at same rate
  sub    kpalast
  bcc    kpadotPos
  nega
...
soin:   ; non inv
  ...
  brset  REUSE_FIDLE,outputpins,soin1   ;* REUSE_FIDLE = 0
  bclr   iasc,porta
  bra    soin2
...
soin2:
  brset   REUSE_LED17,outputpins,soin3  ;* REUSE_LED17 = 1, go to line 6936
  bclr    sled,portc
  bra     soin4
...
soin4:
  brset  REUSE_LED19,outputpins,soin5  ;* REUSE_LED19 = 1, go to line 6944
  bclr   aled,portc
  bra    soin6
...
soin5:
  bset   aled,portc
soin6:
  brclr  REUSE_LED18,outputpins,soin7   ;* REUSE_LED18 = 0, go t line 6952
...
soin7:
  bclr   wled,portc
soin8:
  brclr  out3sparkd,feature2,soin9   ;* out3sparkd = 0, go to line 6958
  bset   Output3,portd
  
soin9:
  lda	 feature8_f
  bit     #spkeopb    ;* = 0
  beq     soin10      ;* go to line 6964
  bset    pin10,portc
  
soin10:
  lda    feature8_f
  bit    #spkfopb     ;* spkfopb = 0
  beq    soin11       ;* go to line 6970
  bset   KnockIn,portd
...
save_dwell:             ; move calculation variable into variable used by CalcDwellspk
  ldhx   dwelltmpHms
...
;
hei7_spd:  ;moved from Sparktime - set/clr HEI7 output
  brclr  HEI7,personality,dwellornot    ;* HE17 = 0, go to line 7123
...
do_dwell_us:
  stx    dwelldms		; save corrected target dwell (in 0.1ms units)
...
; dwell control for 1,2,3,4, rotary2 outputs
  ...
  jmp    true_dwell_calc                      ;* go to line 7599
...
; save an un-mutilated copy for rotary
  ...
  ror    welltmpH	; Divide by 2 to get 50% dwell
  ror    welltmpL
...
; add off the accel factor (-ve)    dt = dt-1 + (-ddt) (predicted next period)
  ...
  brset  wspk,EnhancedBits4,cd1_start  ;* wspk = 1, go to line 7653
...
cd1_start:
; check to see if value we _would_ store in dwelldelay1 is negative ie. top bit set
...
;,dd2 = dd1 +dt-1 + ac + ac
  ...
  brclr  REUSE_LED18,outputpins,cd2_done    ; want 1 } spark c  ;*REUSE_LED18 = 0, go to line 7701  
...
;  sthx   dwelldelay3
;  sthx   dwelldelay4
 jmp    really_done_dwell  ;* go to line 7961

cd2_cont:

; check to see if value we _would_ store in dwelldelay2 is negative ie. top bit set
...
; check to see if value we _would_ store in dwelldelay3 is negative ie. top bit set
...
; check to see if value we _would_ store in dwelldelay4 is negative ie. top bit set
...
; finally we've calculated everything we need to for dwell and saved it away - phew!
  ...
  brset  rotary2,EnhancedBits5,rotary_split  ; are we doing rotary split  ;* roary2 = 0
  jmp    misc_spark_end                                                   ;* go to line 8154
...
fixspl_c:
  cmp    #$03
  blo    rs_STEP_1	; Added this as MT doesnt send a perfect 00T
...
sld2:
  ...
  blo    trail_simult
;  lda    dwelltmpXac
;  beq    trail_split         ; only do split if fast enough ;at slow speeds < 537rpm no trailing
...
$MACRO COILNEG
  ...
  brclr  TOY_DLI,outputpins,nils	; note, Toyota Multiplex only NON-inverted   ;* TOY_DLI = 0, go to line 8214
  brset  coilabit,coilsel,fcnita
  brset  coilbbit,coilsel,fcnitb
  brset  coilcbit,coilsel,fcnitc   ;* won't get here
...
fcnitc:   ;* won't get here
  bclr   coilb,portc
  bset   wled,portc
  bra    dslsa
  
rot2neg:   ;* won't get here
...
nils:					          ; normal sparking non inverted
  ...
  brset  coilcbit,coilsel,dslsc   ;* won't get here
  brset  coildbit,coilsel,dslsd   ;* won't get here
  brset  coilebit,coilsel,dslse   ;* won't get here
  brset  coilfbit,coilsel,dslsf   ;* won't get here
...
dslsc:
  bset   wled,portc		    ; Set spark on   ;* won't get here
  bra    cn_end
  
dslsd:
  bset   output3,portd		; Set spark on   ;* won't get here
  bra    cn_end
  
dslse:
  bset   pin10,portc		; Set spark on   ;* won't get here
  bra    cn_end
  
dslsf:
  bset   knockin,portd		; Set spark on   ;* won't get here
  bra    cn_end
  
dslsx:
  bset   iasc,porta   ;* won't get here
...
; note no Toyota, because never inverted - ??? is this right

  brset  coilabit,coilsel,ilsoa
  brset  coilbbit,coilsel,ilsob
  brset  coilcbit,coilsel,ilsoc   ;* won't get here
  brset  coildbit,coilsel,ilsod   ;* won't get here
  brset  coilebit,coilsel,ilsoe   ;* won't get here
  brset  coilfbit,coilsel,ilsof   ;* won't get here
  bra    fc_end			                 ; should never get here
  
rot2pos:   ;* won't get here
...
ilsoc:   ;* won't get here
  bclr   wled,portc
  bra    fc_end
  
ilsod:   ;* won't get here
  bclr   output3,portd
  bra    fc_end
  
ilsoe:   ;* won't get here
  bclr   pin10,portc
  bra    fc_end
  
ilsof:   ;* won't get here
  bclr   knockin,portd
  bra    fc_end
  
ilsox:   ;* won't get here
  bclr   iasc,porta
...
; macro only used after spark when mainloop will??? have had time to calc since trigger
  ...
  brset  wspk,EnhancedBits4,wastedwell   ;* wspk = 1, go to line 8398
...
dd_c:
  sthx   SparkOnLeftch	; Store time to keep output the same
  bra    jdd_end
  
dd_d:
  sthx   SparkOnLeftdh	; Store time to keep output the same
...
; no need to check for 2nd output, wouldn't have got here otherwise (wspk above)
  ...
  brclr  REUSE_LED18,outputpins,jwdwell2op    ; want 1 } spark c  ;* REUSE_LED18 = 0, go to line 8395 
...
; we've now set the 360deg wait, see if we can delay off previous spark (120deg)
  ...
  lda    dwelldelay1+1
  cmp    #2
  blo    wd3skip120   ; check if more than 0.2ms if less then dwell might get missed
...
; not enough time in 120deg period, see if 240deg will work
  ...
  lda    dwelldelay2+1
  cmp    #2
  blo    wd3end240   ; check if more than 0.2ms if less then dwell might get missed
...
; we've now set the 360deg wait, see if we can delay off previous spark (180deg)
  ...
  lda    dwelldelay1+1
  cmp    #2
  blo     wd2skip   ; check if more than 0.2ms if less then dwell might get missed
...
; we've now set the 360deg wait, see if we can delay off previous spark (90deg)
  ...
  lda    dwelldelay1+1
  cmp    #2
  blo    wd4skip90   ; check if more than 0.2ms if less, then dwell might get missed
...
; not enough time in 90deg period, see if 180deg will work
  ...
  lda    dwelldelay2+1
  cmp    #2
  blo    wd4skip180   ; check if more than 0.2ms if less then dwell might get missed
...
; not enough time in 180deg period, see if 270deg will work
  ...
  lda    dwelldelay3+1
  cmp    #2
  blo    wd4end270   ; check if more than 0.2ms if less then dwell might get missed
...
; we've now set the 360deg wait, see if we can delay off previous spark (72deg)
  ...
  lda    dwelldelay1+1
  cmp    #2
  blo    wd5skip72   ; check if more than 0.2ms if less, then dwell might get missed
...
wd5skip72:
  ...
  lda    dwelldelay2+1
  cmp    #2
  blo    wd5skip144   ; check if more than 0.2ms if less then dwell might get missed
...
; not enough time in 144deg period, see if 216deg will work
  ...
  lda    dwelldelay3+1
  cmp    #2
  blo    wd5skip216   ; check if more than 0.2ms if less then dwell might get missed
...
; not enough time in 216deg period, see if 288deg will work
  ...
  lda    dwelldelay4+1
  cmp    #2
  blo    wd5skip288   ; check if more than 0.2ms if less then dwell might get missed
...
; we've now set the 360deg wait, see if we can delay off previous spark (60deg)
  ...
  lda    dwelldelay1+1
  cmp    #5
  blo    wd6skip60   ; check if more than 0.2ms if less, then dwell might get missed
...
; not enough time in 60deg period, see if 120deg will work
  ...
  lda    dwelldelay2+1
  cmp    #5
  blo    wd6skip120   ; check if more than 0.2ms if less then dwell might get missed
...
; not enough time in 120deg period, see if 180deg will work
  ...
  lda    dwelldelay3+1
  cmp    #5
  blo    wd6skip180   ; check if more than 0.2ms if less then dwell might get missed
...
; not enough time in 180deg period, see if 240deg will work
  ...
  lda    dwelldelay4+1
  cmp    #5
  blo    wd6skip240   ; check if more than 0.2ms if less then dwell might get missed
...
; not enough time in 240deg period, see if 300deg will work
  ...
  lda    dwelldelay5+1
  cmp    #5
  blo    wd6skip300   ; check if more than 0.2ms if less then dwell might get missed
...
edis2_fire:
  brclr  REUSE_LED19,outputpins,go_inj_fire2	; if 2nd output notenabled then skip
...
; start 2nd SAW here and set timer to turn it off
  ...
  clr    coilsel
  bset   coilbbit,coilsel	; only support 2nd spark output assume that other outputs cannot get set
...
set_saw_on2:				; now set timer for SAW on period using sawh/l calculated in main loop
...
; if we are stalled don't increment these or we might skip the wheeldecoder
  ...
  inc     lowresH
					; otherwise done
TIMER_DONE:
   bclr    TOIE,T2SC0	; disable 0.1ms interrupt to prevent re-entry
   cli                  ; allow interrupts during the large chunk of code below. This
					    ; significantly reduces spark jitter. Without it there is
					    ; ~6deg at 9000rpm only really want IRQ to be allowed
...
; James will kill me if it does
  ...
  brclr   BoostControl,feature2,bcActDone   ;* BoostControl = 0, go to line 9301
...
bcActDone:
  ...
  brset  WHEEL,personality,wheel_irq    ;* Wheel = 1, go to line 9332
  lda    personality
  bne    no_wd_trig                  ; any other spark modes skip over 2nd irq bits
  jmp    INJ_FIRE_CTL		         ; skip this section if not controlling spark
...
; more bloat... check for second "reset" spark input
  ...
  brclr  wd_2trig,feature1,no_wd_trig   ;* wd_2trig = 0, go to line 9398
  lda    dtmode_f
  bit    #trig2risefallb
...
lowspdspk:
  brclr  rotary2,EnhancedBits5,chkindwell   ;* rotary2 = 0, go to line 9506
...
chkindwell:
  brset  indwell,EnhancedBits4,blssd	; if doing hi-res dwell then don't turn on coil now
...
Dont_ResetCnt:
  ...
  brset  sparkCut,RevLimBits,blssd	    ; If in spark cut mode jump past spark  ;* sparkCut = 0
  brset  invspk,EnhancedBits4,lsspk_inv ; check if noninv or inv spark          ;* invspk = 1, go to line 9529
...
CHECK_SPARK_LATE:
  brclr  SparkLSpeed,SparkBits,jINJ_FIRE_CTL ; Skip if not low speed sparking
  brclr  sparktrigg,sparkbits,jINJ_FIRE_CTL	 ; Skip if spark already done
;  brclr  crank,engine,timebased	         ; if not cranking don't do irq_spark (NEW 021v)
...
ChkHold:
  ...
  lda    wheelcount		; (HoldSpark) Check if spark is held (after stall and restart)
  beq    DoSparkLSpeed
  dec    wheelcount		; (HoldSpark) One spark has been held, x to go
  jmp    INJ_FIRE_CTL
...
; as we schedule the spark to maintain a consistent dwell
  ...
  brset  dwellcont,feature7,b_INJFC2	; don't schedule chg time here (low speed)   ;* dwellcont = 1, go to line 9606
...
;  brclr  sparkon,revlimbits,INJ_FIRE_CTL	; if output not active then skip
;  brclr  sparktrigg,sparkbits,INJ_FIRE_CTL	; if sparktrigg??? not active then skip
  brclr  SparkLSpeed,Sparkbits,tfi_fast	   ; if not slow then do high speed calc
...
inject_water:
  ...
  bset   water2,porta		        ; Turn water injector on with fuel inj 2
...
boostOn:
  ...
  bit    #BoostDirb
  bne    bcClrout
;  brset  BoostDir,feature6,bcClrout	; Change dir for high pulsewidth reduce boost
  bra    bcSetout
...
boostOff:
  ...
  bit    #BoostDirb
  bne    bcSetout
;  brset  BoostDir,feature6,bcSetout	; Change dir for high pulsewidth reduce boost
  bra    bcClrout
...
;  brclr  KpaDotSet,feature4,tps_dot_mode	; If not in KPA dot mode jump past KPa settings
  lda    kpa
  bra    Kpa_Dot_Mode
...
set_spkon:
  brclr SparkTrigg,Sparkbits,INT_SPARK_OFFa	; Check for spark trigg, used end of pulse
...
; see that section within 0.1ms
  ...
  cmp    #$FF
  bne    split_timed
  jmp    force_trail_off     ; ensure trailing coil off maybe need some hysteresis with this to avoid jittery behaviour
...
force_trail_off:      ; ensure trailing coil is really off
  bset   wled,portc
...
to_exit:
  bclr   SparkTrigg,Sparkbits	; No more sparks for this IRQ kill the dwell timers for trailing in the mainloop
  pulh
  rti
...
set_saw_on:	  ; now set timer for SAW on period using sawh/l calculated in main loop
...
; rather than after all the other missing tooth calcs by the time we reach done_decode
  ...
  cpx    #0
  bne    no_rollchk
  brclr  roll2,EnhancedBits4,no_rollchk  ; we were't about to rollover a few ms ago or byte already
...
n_notlog:
  bclr   rise,sparkbits		     ; reset flag so we can detect next rising IRQ edge
...
tooth_sync:				        ; ignore first few pulses
  ...
  brclr  6,wheelcount,tooth_decode2	; if bit 6 clr then we've done holdoff
  dec    wheelcount
  bne    tooth_rti
  bclr   6,wheelcount
...
tooth_decode2:
  ...
  brset  7,wheelcount,tooth_decode3	; bit 7 is !sync. if not synced then look for the long trigger
  lda    wheelcount		            ; ignore the three short pulses after primary trigger
  beq    tooth_decode3		; =0
  dec    wheelcount
  bne    tooth_rti		; >0
...
tooth_decode3:             ; divide this cycle time 2
  lsr    cTimeH
...
coil_detecta:
  ...
  bset   coilerr,revlimbits	        ; out of sync once, so ignore and follow instinct
  bra    set_b_detect
...
; as per Neon, use cTimeH/L where poss as it is zp use lowres timer for calcs
...
; ignore first few pulses
  ...
  dec    wheelcount
  lda    wheelcount
  and    #$3F           ; ignore top bits during holdoff downcount  keeps wheelcount compatible with Neon mode
  bne    w_rti
...
is_miss:
  clr    wheelcount		; declare we are synced and reset counter
...
;                360-719 deg = 60-119
  ...
  lda     numteeth_f   ; from 028c now holds 2 revs number (i.e. 60-2 -> 120)   ;* numteeth_f = 36T
  lsra
  sta     wheelcount
...
not_2ndmiss:
  bset   wsync,EnhancedBits6
  brclr  WHEEL2,personality,not_miss  ;* WHEEL2 = 0, go to line 11290
  inc    wheelcount
...
;check if using old decoder
  ...
  lda    feature6_f
  bit    #wheel_oldb   ;* wheel_oldb = 0
  beq    tooth_avg     ;* go to line 11309
...
; see if our tooth matches the user input trigger point
  ...
  lda    wheelcount
  brclr  nextcyl,EnhancedBits4,wc_op2   ;* nextcyl = 0, go to line 11428
...
w_trig2:
  brclr  REUSE_LED19,outputpins,w_trig1	; if spark B not defined then just one o/p  ;* REUSE_LED19
  clr    coilsel
  bset   coilbbit,coilsel
  jmp    w_store2   ;* go to line 11567
...
w_trigret2:
  brclr  REUSE_LED19,outputpins,w_trigret1	; if spark B not defined then just one o/p
  clr    coilsel
  bset   coilbbit,coilsel
  bra    ret_w2
...
done_decode:
  brclr  REUSE_LED18,outputpins,dcd_no_led    ;* REUSE_LED18 = 0, go to line 11588
...
; tacho output
  ...
  bit    #$7f               ;* %01111111
  beq    tach_done
  bit    #$80     ; see if in divide by 2 mode  ;* %10000000
  beq    tach_full                ;* go to line 11602
  lda    EnhancedBits5
  eor    #ctodivb
  sta    EnhancedBits5
  bit    #ctodivb
...
; tachon:
  ...
  cbeqa   #2T,tachon_x3
  cbeqa   #3T,tachon_x4
  cbeqa   #4T,tachon_x5    ; go to line 11632
...
tachon_x5:
  bset    output2,porta
  bra     tach_done    ;* go to line 11644
...
; check for dual dizzy feature
  ...
  beq    nondualdizzy          ;* go to line 11707
...
miss_chk:
  brset  invspk,EnhancedBits4,mc_inv   ;* invspk = 1, go to line 11735
...
mc_fire_done:
  bclr   TOIE,T2SC1		             ; Disable timer interrupt (never got there)
  brclr  dwellcont,feature7,mc_fd    ;* dwellcont = 1
  
; if next_cyl and cranking then skip

  brclr  nextcyl,EnhancedBits4,mc_fd2  ;* nextcyl = 0, go to line 11748
  brset  crank,engine,mc_cd          ; can cause a conflict
  
mc_fd2:
  brset  SparkLSpeed,SparkBits,jmcd	; low speed & dwell so don't schedule now
  
mc_fd:
  brclr  min_dwell,feature2,mc_cd	; don't schedule here  ;* min_dwell = 0, go to line 11757
  
jmcd:
  jmp    miss_chk_done			; if minimal dwell wanted
...
delay_done:
  brclr	 REStaging,EnhancedBits,cont_inc_counters   ;* REStaging = 0, go to line 11785
...
cont_inc_counters:
  lda	igncount1
  bne   EGOBUMP		       ; Only increment counters if cylinder count is zero
  lda   feature10_f5
...
TPS_COUNTER:
  brclr   taeIgnCount,feature1,EGOBUMP	; Are we in Cycle counter mode for TPS Accel? ;* taeIgnCount = 1 (cycles)
...
; acceleration enrichment section or KPa in KPa last if in MAP dot
  ...
  lda    feature4_f
  bit    #KpaDotSetb    ;* KpaDotSetb = 0 (TPSdot)
  beq    tps_dot_on     ;* go to line 11807
;  brclr  KpaDotSet,feature4,tps_dot_on	; If not in KPA dot mode jump past KPa settings
  lda    kpa
  bra    Kpa_Dot_on
...
No_Ego_Cnt:
  brset  running,engine,CYCLE_CALC	; should always be running if we get here
  jmp    SKIP_CYCLE_CALC
...
; Stick with Magnus' 0.1ms method for now as it works.
  ...
  brclr  EDIS,personality,non_edis  ;* EDIS = 0, go to line 11844
  jmp    edis_speed
...
; are we doing oddfire wheel ?
  ...
  lda    SparkConfig1_f
  bit    #M_SC1oddfire   ;* M_SC1oddfire = 0
  beq    CCnot_odd       ;* go to line 11901
...
LOW_SPEED:
  ...
  brclr  TFI,personality,LOW_cont   ;* TFI = 0, go to line 11968
...
LOW_cont:
  bra    DELAY_CALC   ;* go to line 11994

ASIS_SPEED:

; need to check for low speed+TFI or we'll miss the output

  brclr  TFI,personality,DELAY_CALC        ;* TFI = 0, go to line 11994
...
DELAY_CALC:
  lda    config11_f1		    ; Get engine config ;* config11_f1 = %00110001
  nsa                           ; nibble swap accumulator ;* Accu A = %00010011
  and    #$0f			        ; Mask out cylinders  (was $07) ;* %1111 Accu A now % 00000011, 3T
  inca				            ; Prepare loop counter  ;* Accu A = 4T
...
; 2x ctime - ctimep
  ...
  mov    iTimeL,cTimeL		; Prepare to calculate with highres time
        mov     iTimeH,cTimeH
...
;  clr    SparkCarry
  lsl    cTimeL
  rol    cTimeH
;  rol    SparkCarry		; redundant. If it overflows we'll only subtract it in a sec
  lda    cTimeL
  sub    iTimepL
  sta    cTimeL
  lda    cTimeH
...
; take a copy - used later by next-cyl calcs
  ...
  lda    config11_f1    ;* config11_1 = %00110001
  bit    #M_TwoStroke   ;* M_TwoStroke = 0
  beq    lt4_4s			; eq 0 so branch 4 stroke 2 stroke for 1,2,4  ;* go to line 12081
  txa
  cbeqa   #1T,jsmd4
  cbeqa   #2T,jsmd2
  cbeqa   #4T,jsmt
...
jsmd2:
  jmp    spk_mult_div2  ;* go to line 12263 
...
; Time for 90 deg * Angle (256=90 deg)/256
  ...
  lda    DelayAngle
  add    offsetang        ; for oddfire, zero otherwise  ;* offsetang = 0
  ldx    SparkTempH
  mul
  stx    SparkDelayH
  sta    SparkCarry
...
; check for oddfire offset
  ...
  lda    SparkConfig1_f
  bit    #M_SC1oddfire  ;* M_SC1oddfire = 0
  beq    ck_xlong       ; not oddfire, use normal method  ;* go to line 12315
...
; Check for long trigger (more than 90 deg)
  ...
  beq    ck_nextcyl     ;* go to line 12353
...
; check for next cyl mode - only get here if NOT in long-trigger
  ...
  brclr  nextcyl,EnhancedBits4,SDelayDone  ;* nextcyl = 0, go to line 12371
...
; if next_cyl and cranking then skip
  ...
  brclr  nextcyl,EnhancedBits4,sdd2             ;* nextcyl = 0, go to line 12380
...
lsd_done2:
  ...
  brset  coilcbit,coilsel,lsd_c   ;* won't get here
  brset  coildbit,coilsel,lsd_d   ;* won't get here
  brset  coilebit,coilsel,lsd_e   ;* won't get here
  brset  coilfbit,coilsel,lsd_f   ;* won't get here
...
lsd_a:
  sthx    SparkOnLeftah		; Store time to keep output the same
  jmp     SKIP_CYCLE_CALC   ;* go to line 12643 
...
lsd_c:
  sthx    SparkOnLeftch		; Store time to keep output the same   ;* won't get here
  jmp     SKIP_CYCLE_CALC
  
lsd_d:
  sthx    SparkOnLeftdh		; Store time to keep output the same   ;* won't get here
  jmp     SKIP_CYCLE_CALC
  
lsd_e:
  sthx    SparkOnLefteh		; Store time to keep output the same   ;* won't get here
  jmp     SKIP_CYCLE_CALC
  
lsd_f:
  sthx    SparkOnLeftfh		; Store time to keep output the same   ;* won't get here
  jmp     SKIP_CYCLE_CALC
...
; if next_cyl and cranking then skip
  ...
  brclr  nextcyl,EnhancedBits4,sst2    ;* nextcyl = 0, go to line 12447
...
; now calc dwell start point into SparkDelay
  ...
  sbc    dwellush
  sta    SparkDelayH		; H
  bset   indwell,EnhancedBits4	; flag that we are doing dwell delay not spark delay
...
zd_e:
  sthx  SparkOnLefteh		; Store time to keep output the same
  bra   do_set_spk
  
zd_f:
  sthx  SparkOnLeftfh		; Store time to keep output the same

do_set_spk:
  lda    SparkDelayL
  sub    latency_f    ;* latency = 0
  sta    SparkDelayL
  lda    SparkDelayH
  sbc    #0
  sta    SparkDelayH
...
setit2:
  ...
  lda    SparkDelayH
  adc    T2CurrH
  sta    T2CH1H			; Write high byte timer output compare first
...
; rotary low revs ...
  ...
  brclr	 rotary2,EnhancedBits5,skip_rotary_jmp   ;* rotary2 = 0, go to line 12591
...
;; brclr  crank,engine,bsc1	    ; we don't if in crank mode then min_dwell does same thing
  jmp    squirtCheck1   ;* go to line 12730 
...
squirtCheck1:
  brset   crank,engine,schedule1a	; Squirt on every pulse if cranking
  inc    IgnCount1		            ; Check to see if we are to squirt or skip
  lda    IgnCount1
...
sc2single:
  brset  crank,engine,schedule2sa	; Squirt on every pulse if cranking
  inc    IgnCount2		            ; Check to see if we are to squirt or skip
  lda    IgnCount2
  cmp    divider_f1      ;* divider_f1 = 4T
  beq    schedule2s
  lda    IgnCount2
...
sc2dual:
  brset  crank,engine,schedule2da	; Squirt on every pulse if cranking
  inc    IgnCount2		            ; Check to see if we are to squirt or skip
  lda    IgnCount2
...
IRQ_EXIT:
  ...
  brset  WHEEL,personality,IRQ_EXIT2    ;* Wheel = 1, go to line 12833
  lda    feature6_f
  bit    #falsetrigb           ; can disable false trigger protection for testing
  bne    IRQ_EXIT2
...
KPa_ADC_Check:
  ...
  lda    feature10_f5
  bit    #MAPHoldb                   ; Are we holding the MAP at a fixed value during ASE?  ;* MAPHoldb = 0
  beq    NormMAP_Count                                                                      ;* go to line 12881 
...
FUEL_JUMP:
  lda    adr
  sta    o2_fpadc			; Fuel Pressure, wheel sensor or second O2 sensor
  lda    adsel
  inca
  bra    ADCPTR
...
IN_SCI_RCV:
  ...
  cbeqa  #$05,TXMODE_5   ;* go to line 12971
  cbeqa  #$06,TXMODE_6   ;* go to line 12976
  cbeqa  #$07,TXMODE_7   ;* go to line 12987
  cbeqa  #$08,TXMODE_8   ;* go to line 12992
  cbeqa  #$09,TXMODE_9   ;* go to line 12997
  cbeqa  #$0C,TXMODE_C1  ;* go to line 12968
  jmp    CHECK_TXCMD     ;* go to line 113129
  
TXMODE_C1:
  jmp    TXMODE_C        ;* go to line 13556
...
TXMODE_6:
  brset  mv_mode,EnhancedBits2,TX6_MV  ;* mv_mode = 1, go to line 12984 
  clrh
  lda    SCDR
  ldx    rxoffset
...
TXMODE_8:				 ; Getting count for X command
  mov    SCDR,txgoal	 ; borrow txgoal as we aren't going to using it
  inc    txmode			 ; continue to next mode
  jmp    DONE_RCV        ;* go to line 13267
...
MODE_B_OK:
  bclr   SCRIE,SCC2		; turn off receive interrupt so don't re-enter
;  cli				    ; re-enable interrupts to reduce stumble during Burn. Too bad
  mov    #$CC,flocker
  jsr    burnConst		; routine disables interrupts during critical sections
;  cli				    ; returns with ints off
  clr    flocker
  clr    txmode
  lda    page			; check if page0, if so reload quick vars
  beq    ck_page0
...
; is PTC4 (pin11) an input (trig2) or output (shiftlight)
  ...
  brclr  wd_2trig,feature1,ckp0_norm_ddrc   ;* wd_2trig = 0, go to line 13063
  lda    #%00001111              ; make PTC4 an input for second trigger
  bra    ckp0_ddrc
...
; check this here so a changed setting or MSQ load will correctly init the variables
  ...
  brset  WHEEL,personality,pz_wspk    ;* Wheel = 1, go to line 13079
...
; EDIS and NEON are never next-cylinder
  ...
  lda    TriggAngle_f                ;* TriggAngle_f - 227T
  cmp    #57T			           ; check for next cyl mode
  bhi    not_nc		               ; trigger angle > 20, continue  ;* go to line 13110
  bset   nextcyl,EnhancedBits4
  bra     DONE_B
...
CHECK_TXCMD:
  ...
  cbeqa  #'A',MODE_A		; realtime vars      ;* go to line 13169 
  cbeqa  #'B',jMODE_B		; All I hear is BURN ;* go to line 13145
  cbeqa  #'C',MODE_C		; Comm test          ;* go to line 13187
  cbeqa  #'V',MODE_V		; (VE) read page     ;* go to line 13193
  cbeqa  #'W',jMODE_W		; Write byte         ;* go to line 13148
  cbeqa	 #'Q',jMODE_Q		; Query version      ;* go to line 13151
  cbeqa  #'P',jMODE_P		; Page select        ;* go to line 13154
  cbeqa  #'!',jMODE_BOOT	; bootloader         ;* go to line 13157
  cbeqa  #'S',jMODE_SIGN	; signature          ;* go to line 13160
  cbeqa  #'R',MODE_R		; Added for enhanced stuff was "a" now "R" for Megatunix compatabilty ;* go to line 13173
  cbeqa  #'X',jMODE_X		; Chunk write        ;* go to line 13163
  cbeqa  #'T',jMODE_T		; Text version       ;* go to line 13166
  jmp    DONE_RCV                                ;* go to line 13267

jMODE_B:
  jmp    MODE_B   ;* go to line 13012
  
jMODE_W:
  jmp    MODE_W   ;* go to line 13230
  
jMODE_Q:
  jmp    MODE_Q   ;* go to line 13239
  
jMODE_P:
  jmp    MODE_P   ;* go to line 13251
  
jMODE_BOOT:
  jmp    MODE_BOOT ;* go to line 13271
  
jMODE_SIGN:
  jmp    MODE_SIGN ;* go to line 13256
  
jMODE_X:
  jmp    MODE_X    ;* go to line 13234
  
jMODE_T:
  jmp    MODE_T    ;* go to line 13245

MODE_A:         ; Big A
  mov    #$16,txgoal		     ; B&G mode ($17) For Megaview use
  bra    MODE_AA_cont            ;* go to line 13178

MODE_R:        ; Big R
  bclr   mv_mode,EnhancedBits2	; clear MegaView mode to allow enhanced comms
  mov    #39T,txgoal		    ; was 32T in 021, was 36T in 021u, 38T from 021x1 onwards, 023b2:39T
...
;  mov    iTimeH,cTimeCommH	      ; otherwise it might get out of sync during communication
  clr    txcnt			         ; Send back all real-time variables
  lda    #$01
  bra    EN_XMIT   ;* go to line 13262 
...
MODE_V:
  clr    txcnt
  brset  mv_mode,EnhancedBits2,MODE_V_MV  ;* mv_mode - 1, go to line 13223 
...
jBootLoad:
  jmp    BootLoad   ;* go to line 13496

CONF_ERR:
  ldhx   tmp5			    ; tmp5,6 contain absolute address of data
  lda    ,x
  bne    conf_err2		    ; zero is string terminator
  clr    txmode
  jmp    FIN_TX   ;* go to line 13483 
  
conf_err2:
  sta    SCDR			    ; Send char
  aix    #1
  sthx   tmp5
  jmp    DONE_BYTE   ;* go to line 13492 
...
jIN_SIGN_MODE:
   jmp    IN_SIGN_MODE   ;* go to line 13464
   
jIN_T_MODE:
   jmp    IN_T_MODE       ;* go to line 13468
   
jIN_V_MODE:
   jmp    IN_V_MODE        ;* go to line 13439
...
IN_SCI_TX:
  ...
  lda    txmode
  beq    tx_done
  cbeqa	 #$05,IN_Q_MODEJMP                     ;* go to line 13400
  cbeqa  #$04,jIN_SIGN_MODE      ; see above   ;* go to line 13303
  cbeqa  #$0D,CONF_ERR           ; see above   ;* go to line 13281
  cbeqa  #$0E,jIN_T_MODE         ; see above   ;* go to line 13306
  cmp    #$01
  bne    jIN_V_MODE                            ;* go to line 13309
...
; check for iTime sending. Now send three bytes but don't waste extra byte, only store two
  ...
  jmp    CONT_TX			; sync during communication   ;* go to line 13475 
  
ac_chk38:
  cpx    #38T
  bne    ac_chk37
  lda    bcDC
  jmp    CONT_TX   ;* go to line 13475
...
; Added for MV compatability with 300 & 400KPa MAP sensors
  ...
  brclr  mv_mode,EnhancedBits2,Send_Data_Normal	; Yes so are we in MV mode?
  lda    config11_f1
  and    #$03
...
; so send 86% of the raw map value to MV so it converts it correctly
  ...
  lda    kpa
  cmp    #217T
  bhi    Load_Max_Map		  ; If raw map > 217 then we are above 255KPa, thats the limit in MV
  tax
...
KPa400_Reading:
  lda    kpa
  cmp    #159T
  bhi    Load_Max_Map		; If raw map > 159 then we are above 255KPa, the limit in MV
  tax
  lda    #160T
  mul
  txa
...
IN_V_MODE
  ...
  lda    ve_r,x			   ; get data from RAM (must have loaded a page first)
  bra	CONT_TX
...
; style view of our data
  ...
  cpx    #116T
  blo    V_MV2
  txa				             ; need to return config11,12,13 to get correct map reading
  add    #88T		             ; B&G byte 116 is at 204 in this code
  tax
  lda    config11_f1,x    ;* config11_f1 = %00110001
  jmp    CONT_TX          ;* go to line 13475 
...
TXMODE_C:
  ...
  beq    DONE_LOAD
  cbeqa  #$F0,toothl_F0          ;* go to line 13593
  cbeqa  #$F1,toothl_F1          ;* go to line 13597
  cbeqa  #$F2,okpage             ;* go to line 13589
  cbeqa  #$F3,okpage             ;* go to line 13589
  ...
  bhi    DONE_LOAD               ;* go to line 13615
  clrx
  sta    page
  add    #$E0			         ; hardcoded high byte of page area $Ex00
  psha
  pulh
  bclr   SCRIE,SCC2		         ; turn off receive interrupt so don't re-enter
  cli				             ; re-enable interrupts to reduce stumble when MT changes page
  
load_table:
  lda    0,x			         ; h:x
  pshh
  clrh
  sta    VE_r,x			         ; dump into RAM. Bit of a kludge, want h=0
  pulh
  incx
  cpx    #PAGESIZE+1		; copy 256 bytes reduced to 200  ;* PAGESIZE+1 = 200???
...
*
  ...
  SUB    tmp10			; subtract divisor lsb from remainder lsb
  STA    REMAINDER+1	; store new remainder lsb
  LDA    REMAINDER		; get remainder msb
  SBC    tmp9			; subtract divisor msb from remainder msb
...
SETLSB
  BSET   0,DIVIDEND+3		; set lsb of quotient to indicate ; successive subtraction
  
DECRMT  
  DBNZ   tmp11,SHFTLP		; decrement loop counter and do next shift
...
EnableN2O:
  brclr  LaunchControl,feature2,enablen2o_cont  ; Is Launch selected?         ;* LaunchControl = 0, go to line14066
...
enablen2o_cont:
  ...
  blo    Clr_Nos_Ang		                    ; No, no Nos
  lda    coolant
  cmp    NosClt_f		                        ; Is the engine warmer than the minimum point?
  blo    Clr_Nos_Ang		                    ; No, no Nos
  brset  NosDcOk,EnhancedBits,Clr_Nos_Out	    ; Have we hit 90% DC?
  brclr  Traction,EnhancedBits2,Not_AntiRev_NOS	; Have we activated Anti-Rev?
  brclr  TractionNos,feature7,Not_AntiRev_NOS	; Have we selected to cut NOS during Anti-Rev?
  bra    Clr_Nos_Ang		                    ; Yes so cut NOS during Anti-Rev
  
Not_AntiRev_NOS:
  lda    feature5_f
  bit    #NosLagSystemb
  beq    Nos_IP_Checked                          ; Have we selected  Anti-lag?
  brclr  NosSysReady,EnhancedBits,Which_io_First ; If NOS not ready yet is the input on?
  brset  NosAntiLag,EnhancedBits,Anti_Lag	     ; If antilag bit set then do Anti-lag checks
  bra    Nos_IP_Checked	                         ; Were in normal NOS mode

; Check if input on before the output, then were in anti-lag mode

Which_io_First:
  brset  NosIn,portd,Nos_IP_Checked	; If input not on then were in normal mode
  bset   NosAntiLag,EnhancedBits	; We are in Anti-lag mode so set the bit
  
Anti_Lag:
  lda    kpa
  cmp    NosLowKpa_f		        ; In anti-lag mode so are we above min KPa?
  blo    Clr_Nos_Out
...
Nos_IP_Checked:
  ...
  brset  NosIn,portd,Clr_Nos_Angle	; Is the Input for NOS SET? (low=on)
  bset   NosSysOn,EnhancedBits	    ; NOS System Is Running
  lda    feature5_f
  bit    #SparkTable2b
  bne    Nos_Lin         	        ; Are we using ST2?
  lda    #255T			
  sub    NosAngle_f		            ; (255-NOS Angle) turns it into a retard angle
  cmp    #$aa			            ; Limit the retard to 30 degrees (plenty)
  bhi    StoreNos
  lda    #$aa
  bra    StoreNos
  
Clr_Nos_Out:
  bclr   water2,porta		        ; Turn off the output as we've hit 90% DC or Anti-lag over setpoints
  lda    #$00
...
***************************************************************************
  ...
  clr    tmp7			    ; This is the negative slope detection bit
  lda    NosFuelLo_f		; Store the minimum PW incase we are lower than 3000rpm
  sta    NosPW
  lda    rpm
...
Are_We_Higher:
  ...
  lda    NosFuelHi_f		; We are above max rpm, so store max rpm PW in NosPW
  sta    NosPW
...
INTERP_PW:
  ...
  beq    End_Interpole		; Settings wrong so end with min rpm PW in NosPW
  psha
  lda    NosFuelHi_f
...
Interpole_Plus:
  psha
  lda    rpm
  sub    NosRpm_f		    ; Find out difference in rpm and  min setpoint
  beq    ZERO_Interpole
  pulx
  mul
  pshx
  ...
  psha
  lda    tmp7
  bne    NEG_Interpole		; Are we doing a negative sloped interpolation?
  pula
  add    NosFuelLo_f
  sta    NosPW
  bra    End_Interpole
...
multi_it:
   lda    rpmpl
   mul				        ; Accumulator now contains time between squirts
   cpx      #00T   
   
;; this was in error         bne      Stop_Nos

   bne    Nos_Done_Now		; if period > 25.5ms then must be ok as that is our max pulse
   ldx    #237T			    ; 90% (230/256=0.9) 237 allows for opening time
   mul
   lda    feature4_f
   bit    #DtNosb
   bne    Check_PW2     	; If were using Bank 2 then check PW2
   txa
...
Stop_Nos:
  bset   NosDcOk,EnhancedBits	; We are over 90 DC so cut NOS System out
...
Check_VE3_Table:
  ...
  brset  NosIn,portd,Check_VE3_Done	; If input not low dont use VE 3
...
AFR1_Targets:
  brset  useVE3,EnhancedBits,No_AFR_ForVE1	  ; If were running VE3 then no need to go any further
  lda    EGOcount		                      ; Are we about to check the ego?
  cmp    EGOcountcmp_f
  beq    Do_Targets		                      ; If yes then get the target from the table
  
No_AFR_ForVE1:
  rts				                          ; If No then return, this saves wasting time.

Do_Targets:				                      ; VE 1 Targets from AFR Table 1
  brclr  TPSTargetAFR,feature7,NO_TPS_SetAFR1 ; Have we selected to go to targets above tps setpoint?
  lda    tps
  cmp    AFRTarTPS_f
  bhi    NO_TPS_SetAFR1		; If tps higher than setpoint then do tables
  lda    O2targetV_f		; If not load in target from enrichments page
  sta    afrTarget
  rts
...
NO_TPS_SetAFR1:
  clrh
  clrx
  brset  AlphaTarAFR,feature7,AFR1_AN	; Have we selected to use tps for target afrs instead of kpa?
...
AFR2_Targets:
  brclr  useVE3,EnhancedBits,No_AFR_ForVE3	; If were not running VE3 then no need to go any further
  lda    EGOcount		                    ; Are we about to check the ego?
  cmp    EGOcountcmp_f
  beq    Do_Targets2		                ; If yes then get the target from the table
  
No_AFR_ForVE3:
  rts				                        ; If No then return, this saves wasting time.

Do_Targets2:
  brclr  TPSTargetAFR,feature7,NO_TPS_SetAFR2	; Have we selected to go to targets above tps setpoint?
  lda    tps
  cmp    AFRTarTPS_f
  bhi    NO_TPS_SetAFR2		; If tps higher than setpoint then do tables
  lda    O2targetV_f		; If not load in target from enrichments page
  sta    afrTarget
  rts
...
NO_TPS_SetAFR2:
  clrh
  clrx
  brset  TPSTargetAFR,feature7,AFR2_AN	; Have we selected to use tps for target afrs instead of kpa?
  lda    kpa
  bra    AFR2_STEP_1
...
flash_table0:  ; config variables                                             ;* Tuner Studio Page 4

personality_f   db      %00000001	; Only 1 allowed to be set unless in      ;* 0 Tuner Studio ignmode
					                ; EDIS or WHEEL mode, if all are set
					                ; to 0 then thats fuel only (std MS)!
;MSNS           equ      1    Megasquirtnspark                                ;* Tuner Studio msnsCyl bit0
;MSNEON         equ      2    MS neon decoder                                 ;* Tuner Studio neonCrank bit1
;WHEEL          equ      4    generalised decoder 36-1, 60-2 etc              ;* Tuner Studio wheelon bit2

;WHEEL2         equ      8    If in WHEEL mode then WHEEL2 is 0 = -1  1 = -2  ;* Tuner Studio onetwo bit3

;EDIS           equ      $10  edis                                            ;* Tuner Studio edis bit4
;DUALEDIS       equ      $20  if in EDIS mode then this allows two edis       ;* Tuner Studio edis bit5
;                             modules (for edis4 on V8, edis6 on V12)
;TFI            equ      $40  Ford TFI system                                 ;* Tuner Studio tfi bit6
;HEI7           equ      $80  GM 7 pin HEI                                    ;* Tuner Studio hei7 bit7

outputpins_f    db      %00000110                                             ;* 1 Tuner Studio ????????
;               bits=    76543210
;       equ 1       FIDLE for Idle Air Valve || spark output (as per MSnS)    ;* Tuner Studio fidleUse bit0
;       equ 2       LED17 for squirt led     || coila output                  ;* Tuner Studio led17Use bit 1
;                                                                             ;* Tuner Studio led18Use bit 2
;                   bit 2    bit 3                                            ;* Tuner Studio led18_2Use bit 3
...
;REUSE_LED19       equ $10   LED19 for accel led || coilb output              ;* Tuner Studio led19Use bit4
;X2_FAN            equ $20   X2 = water Inj pulsed out || fan control         ;* Tuner Studio x2use bit5
;       ** Please note: Water inj uses X2 to pulse output, X3 will still
;                       come on with water inj or NOS depending on which is on
;
;LED18_FAN         equ $40   LED18 output4 || fan control                     ;* Tuner Studio out4FAN bit6
;       ** only allowed if bit 2 = 1 and bit 3 = 0
;Mulitplex Ign     equ $80   NORMAL || toyota DLI ignition                    ;* Tuner Studio toy_dli bit7
; multiplex

SRevLimRPM	    db      240T	; Standard RPM limit for spark retard (rpm*100) ;* 2 Tuner Studio SoftRevLim
SRevLimAngle	db      56T	    ; Spark retard for above +10 degrees            ;* 3 Tuner Studio SoftLimRetard             
				                ; 10 = 10 + 10 i.e. -10 start point.
				                ; So 56T = 10BTDC )
SRevLimHTime	db      50T	    ; Time in 1/10Sec in Soft limit till            ;* 4 Tuner Studio SoftLimMax
				                ; hard limit cuts in *0.1
SRevLimCTime	db      10T	    ; NOT USED                                      ;* 5 Not Used
RevLimit_f	    db      250T	; Hard Rev limiter (rpm*100)                    ;* 6 Tuner Studio HardRevLim
Out1Lim		    db       0T	    ; Output1 On point in RAW except for            ;* 7 Tuner Studio out1lim
				                ; TEMPS then its in F -40, so 200F switch
				                ; on point = 240F

Out1Source	    db      0T	    ; Output 1 source, index from secl,             ;* 8 Tuner Studio out1src
				                ; Standard Out1source
				                ; This is secl + val up to 30 (ego2 correction)
				                ; 31 = Traction control  >31 is not valid as                                                                    
				                ; this is from RAM

Out2Lim		    db      0T	    ; Output 2 limit As out1Lim                     ;* 9 Tuner Studio out2lim

Out2Source	    db      0T	    ; Same as Out1Source                            ;* 10 Tuner Studio out2src

feature1_f	    db      %01000010                                               ;* 11 Tuner Studio ?????? 
; wd_2trig      equ 1        wheel decoder 2nd trigger i/p - new in 023c9       ;* Tuner Studio wd_2trig bit0
;               spare                                                           ;* NOT USED bit1
; whlsim        equ 4        Wheel simulator               off      on          ;* Tuner Studio whlsim bit2
; taeIgnCount	equ 8        Acceleration Timer      Seconds^ || Engine Cycles  ;* Tuner Studio taeIgnCount bit3
; NOT USE       equ $10      NOT USED NOW 023                                   ;* NOT USED bit4
; hybridAlphaN	equ $20      Hybrid Alpha N              OFF^ || OFF            ;* Tuner Studio hybridAN bit5
; CrankingPW2	equ $40      Fire PW2 during Cranking?   YES^ || NO             ;* Tuner Studio inj2cr bit6
; Nitrous	    equ $80      Nitrous system               OFF || ON
                                                       ;NOT allowed with W Inj

feature2_f	    db      %00000000  ; more features                              ;* 12 Tuner Studio ??????                                               
; BoostControl	equ 1        Boost Controller           OFF || ON               ;* Tuner Studio boostUse bit0
; ShiftLight	equ 2        Shift Lights               OFF || ON               ;* Tuner Studio shiftUse bit1
; LaunchControl	equ 4        launch Control             OFF || ON               ;* Tuner Studio launchUse bit2
; wasPWMidle	equ 8                                                           ;* Tuner Studio  bit3
;     only if outputpins_f bit 1 = 0
; out3sparkd	equ $10      Output 3             Output 3  || Spark D          ;* Tuner Studio out3sparkd bit4
; min_dwell	    equ $20                                                         ;* Tuner Studio dwellduty50 bit5
; dwellduty50	equ $40                                                         ;* Tuner Studio dwellduty50 bit6
; config_error	equ $80    this is only set if non-sense combination            ;* Tuner Studio                                                      bit7
; of options - don't run.

whlsimcnt       db      04T     ; How many outputs when simulating wheel         ;* 13 Tuner Studio whlsimcnt
bcFreqDiv_f	    db      3T	    ; Solenoid PW rate BITS 0 1 and 2 used :         ;* 14 Tuner Studio freqDiv                                     
				                ; "INVALID","39.0 Hz","19.5 Hz","13.0 Hz",
				                ; "9.8 Hz","7.8 Hz","6.5 Hz","5.6 Hz"
bcUpdate_f	    db      10T	    ; Boost Controller Update Rate in mS             ;* 15 Tuner Studio bcUpdateRate
                		        ; (10min   255max)
bcPgain_f	    db      64T	    ; B Controller P Gain % (0-100% = 0-255 in       ;* 16 Tuner Studio pGain 
				                ; MS so MT value displayed = MS*0.3922)
bcDgain_f	    db      5T	    ; Boost Controller D Gain % (0-100% = 0-255      ;* 17 Tuner Studio dGain
				                ; in MS so MT value displayed = MS*0.3922)
ShiftLo_f	    db      58T	    ; Shift light LED start point (rpm*100)          ;* 18 Tuner Studio ShiftLower 
ShiftHi_f	    db      60T	    ; Shift Light Final point (RPM * 100)            ;* 19 Tuner Studio ShiftUpper
LaunchLimit_f	db      40T	    ; Hard limit for Launch control (rpm*100)        ;* 20 Tuner Studio LaunchLim                                     
edisms_f	    db      11T	    ; max rpm for EDIS multi-spark (rpm*100)         ;* 21 Tuner Studio edismultirpm
NosClt_f	    db      160T	; Nitrous System Min Coolant Temp Minimum        ;* 22 Tuner Studio NosClt
				                ; point of coolant for NOS to enable in (F-40)
NosRpm_f	    db      30T	    ; Nitrous System Min RPM * 100, 3000rpm is       ;* 23 Tuner Studio NosRpm
				                ; minimum allowed  (23)
NosRpmMax_f	    db      60T	    ; Nitrous Max RPM *100 (used for                 ;* 24 Tuner Studio NosRpmMax 
				                ; interpolating and cutting nos)

Trig1_f		    db       0T	    ; wheel decoding                                 ;* 25 Tuner Studio trig1 (TargetB1)?
Trig2_f		    db       0T	    ;  "                                             ;* 26 Tuner Studio trig2 (TargetB2)?
Trig3_f		    db       0T	    ;  "                                             ;* 27 Tuner Studio trig3 (TargetB3)?
Trig4_f		    db       0T	    ;  "                                             ;* 28 Tuner Studio trig4 (TargetB4)?
Trig1ret_f	    db       0T	    ;  "                                             ;* 29 Tuner Studio trig1ret (KpaTarg10)?
Trig2ret_f	    db       0T	    ;  "                                             ;* 30 Tuner Studio trig2ret (KpaTarg40)?
Trig3ret_f	    db       0T	    ;  "                                             ;* 31 Tuner Studio trig3ret (KpaTarg60)?
Trig4ret_f	    db       0T	    ;  "                                             ;* 32 Tuner Studio trig4ret (KpaTarg100)?

DTmode_f	    db       %01100001  ; DualTable control                          ;* 33 Tuner Studio ???????
...
alt_i2t2        equ      %00010000  ; inj2: 0 = t1, 1 = t2                              ;* Tuner Studio inj2t bit4
alt_i1ge        equ      %00100000                                                      ;* Tuner Studio inj1g bit5
alt_i2ge        equ      %01000000                                                      ;* Tuner Studio inj2g bit6
trig2fallb      equ      1          ; 0 = rising edge trigger, 1 = falling edge         ;* Tuner Studio trig2fall bit0
trig2risefallb  equ      2          ; 0 = rising or falling, 1 = rise and falling edge  ;* Tuner Studio trig2fall bit1

latency_f       db       0T         ; "known" latency in spark input to output    ;* 34 Tuner Studio latency
spare1_2_f      db       0T                                                       ;* 35 Not Used
spare1_3_f      db       0T                                                       ;* 36 Not Used
spare1_4_f      db       0T                                                       ;* 37 Not Used
spare1_5_f      db       0T                                                       ;* 38 Not Used

EgoLimitKPa_f	db      255T	    ; MAP KPa Point to change Ego +- limit (39)             ;* 39 Tuner Studio EgoLimKPa                                                                                                                     
EgoLim2_f	    db      05T	        ; New Ego limit when MAP KPa above EgoLimitKPa_f        ;* 40 Tuner Studio EgoLim2
LC_Throttle_f	db      30T	        ; Throttle position in RAW data for launch control mode ;* 41 Tuner Studio LachTps
LC_LimAngle_f	db      42T	        ; Launch control soft limiter angle           ;* 42 Tuner Studio LachDeg
				                    ; *0.352   -28.4     -10 to 45 allowed        
LC_Soft_Rpm_f	db      35T	        ; Launch Soft Limit RPM (43)                  ;* 43 Tuner Studio LachRpm
Over_B_P_f	    db      0T	        ; Over boost Protection KPa setpoint          ;* 44 Tuner Studio OvrBKpa
				                    ; <100 = no boost protection
SparkCutNum_f	db      3T	        ; Rev Limiter Hard cut spark cut number to    ;* 45 Tuner Studio HCutSpark                                                               
				                    ; remove sparks from SparkCutBase_f

feature3_f	    db      %00110000   ; (46)                                        ;* 46 Tuner Studio ????????
KPaTpsOpenb      equ 1                                                            ;* Tuner Studio OLoopEgo bit0
VarLaunchb       equ 2                                                            ;* Tuner Studio VLaunch bit1
CltIatIgnitionb  equ 4                                                            ;* Tuner Studio CltAdv bit2
WaterInjb        equ 8                                                            ;* Tuner Studio WaterInj bit3
Fuel_SparkHardb  equ $10            ; Fuel or Spark cut for Rev limiter           ;* Tuner Studio HCutType bit4
FuelSparkCutb    equ $20            ; Fuel or spark cut for Rev limiter           ;* Tuner Studio HCutType bit5
KnockDetb        equ $40                                                          ;* Tuner Studio KnockOn bit6
TargetAFRb       equ $80                                                          ;* Tuner Studio AfrTar bit7

cltAdvance_f	db      180T        ; Advance ignition whilst temp below this value F -40  ;* 47 Tuner Studio IgAdvTemp
cltDeg_f	    db      27T         ; Add 1 degree of advance for this value(F)            ;* 48 Tuner Studio IgAdvDeg
				                    ; below cltAdvance_f, so if
				                    ; cltAdvance_f=120(80F) and cltDeg_f=20 then
                                    ; at 10F advance will be 80-10/20= 3.5
maxAdvAng_f	    db      15T         ; Limit in degrees of advance for coolant              ;* 49 Tuner Studio MaxIatAdv  
				                    ; related advance so it doesnt add loads of
				                    ; advance when very cold *0.352
iatDeg_f	    db      18T         ; Iat Temp for 1 degree of retard related              ;* 50 Tuner Studio IgRetDeg                                           
				                    ; to IAT, exactly the same as cltDeg_f but
				                    ; retard rather than advance and
                                    ; IAT rather that CLT. F
kpaRetard_f	    db      75T         ; Apply the IAT related retard when above              ;* 51 Tuner Studio IatAdvKpa
				                    ; this KPa, to stop retard at tickover
iatDanger_f	    db      200T        ; Iat Temp to start Retard F -40                       ;* 52 Tuner Studio IgRetTemp
KnockRpmL_f	    db      55T         ; Knock sensor max rpm RPM*100 (53)                    ;* 53 Tuner Studio KnockRPM  
KnockRpmLL_f	db      15T         ; Knock sensor min rpm RPM*100                         ;* 54 Tuner Studio KnockRpmL
KnockKpaL_f	    db      255T        ; knock sensor max KPa                                 ;* 55 Tuner Studio KnockKpa
KnockRet1_f	    db      06T         ; First Retard amount for knock system *0.352          ;* 56 Tuner Studio KnockRet
KnockRet2_f	    db      03T         ; Subsequent Retard amount *0.352                      ;* 57 Tuner Studio KnockSb
KnockAdv_f	    db      03T         ; Advance amount for knock system *0.352               ;* 58 Tuner Studio KnockAdv
KnockMax_f	    db      15T         ; Max Allowable retard *0.352                          ;* 59 Tuner Studio KnockMax
KnockTim_f	    db      01T         ; Timer for steps of knock advance / retard            ;* 60 Tuner Studio KnockTim
				                    ; to be applied in Seconds 1
iatpoint_f	    db      100T        ; Water Inj IAT setpoint point F -40 (61)              ;* 61 Tuner Studio WaterIIat
wateripoint_f	db      120T        ; Water Injection KPa setpoint                         ;* 62 Tuner Studio StartWIKpa
wateriRpm_f     db      35T         ; Water Injection RPM setpoint RPM*100                 ;* 63 Tuner Studio WaterIRpm
kpaO2_f		    db      80T         ; KPa Open loop setpoint for no O2 correction          ;* 64 Tuner Studio KpaOn
tpsO2_f		    db      192T        ; TPS Open Loop setpoint for no O2 correction Raw ADC  ;* 65 Tuner Studio TpsOn

feature4_f	    db      %00000000   ; Another feature bit for enhanced (66)                ;* 66 Tuner Studio ???????
miss2ndb        equ     1           ; Missing tooth AND 2nd trigger                        ;* Tuner Studio  bit0
InvertOutOneb   equ     2                                                                  ;* Tuner Studio InvOut1 bit1
InvertOutTwob   equ     4                                                                  ;* Tuner Studio InvOut2 bit2
multisparkb     equ     8           ; EDIS multi-spark                                     ;* Tuner Studio edismulti bit3
KPaDotBoostb    equ     $10                                                                ;* Tuner Studio KPaDotBoost bit4
DtNosb          equ     $20         ; If DT which Bank do we add NOS PW to (Bank1=0 Bank2=1) ;* Tuner Studio DtNos bit5
OverRunOnb      equ     $40                                                                ;* Tuner Studio OvrRunC bit6
KpaDotSetb      equ     $80                                                                ;* Tuner Studio mapProportion4 (AccDecEnrich) bit7

NosTps_f	    db      200T        ; Nitrous System Min TPS RAW ADC                           ;* 67 Tuner Studio NosTps
NosAngle_f	    db      50T         ; Nitrous System Angle to remove from ignition *0.352 (68) ;* 68 Tuner Studio NosAngle                                       
NosFuelLo_f	    db      12T         ; Nitrous Pulse Width to add to fuel at                    ;* 69 Tuner Studio NosPWLo
				                    ; 3000 rpm *0.1 in mSec this is for                                                         
				                    ; additional fuel for NOS
NosFuelHi_f	    db      03T         ; Nitrous Pulse Width to add to fuel at                    ;* 70 Tuner Studio NosPWHi
				                    ; NosRpmMax_f *0.1 in mSec
ORunRpm_f	    db      17T         ; Max RPM for Over run fuel cut *100                       ;* 71 Tuner Studio OvrRunRpm
ORunKpa_f	    db      20T         ; Over run fuel cut when below kpa                         ;* 72 Tuner Studio OvrRunKpa
ORunTPS_f	    db      05T         ; Over run when throttle position lower than this RAW ADC (73) ;* 73 Tuner Studio OvrRnTps
EfanOnTemp_f	db      234T        ; X2 or LED 17 electric fan output on temp F-40            ;* 74 Tuner Studio efanontemp
EfanOffTemp_f	db      185T        ; X2 or LED 17 electric fan output off temp F-40           ;* 75 Tuner Studio efanofftemp

feature5_f	    db      %00110011   ; Yet another feature bit (76)                             ;* 76 Tuner Studio ?????
Fuel_SparkHLCb  equ     1           ; Fuel or Spark cut for Launch                                         ;* Tuner Studio HCutLCType bit0
FuelSparkLCb    equ     2           ; Fuel or Spark cut for Launch                                         ;* Tuner Studio HCutLCType bit1
stagedb:        equ     4           ; Roger Enns Staged Mode   xxxx00xx = Staged Off  xxxx01xx = RPM Based ;* Tuner Studio IStagedMode bit2
stagedModeb:    equ     8           ; Roger Enns Staged Mode   xxxx10xx = MAP Based   xxxx11xx = TPS Based ;* Tuner Studio IStagedMode bit3
stagedeither:   equ     $0c         ; either staging                                                       
BoostCutb:      equ     $10         ; Over boost Cut type, option2 or spark cut                            ;* Tuner Studio OvrBCutType bit4                            
BoostCut2b:     equ     $20         ; Option2 for Over boost Cut type, fuel only or both fuel and spark    ;* Tuner Studio OvrBCutType bit5    
NosLagSystemb:  equ     $40         ; Nos Anti-lag System used                                             ;* Tuner Studio NosLagSystem bit6                                             
SparkTable2b:   equ     $80         ; Second Spark Table                                                   ;* Tuner Studio STTable2On bit7                                                   


SparkCutNLC_f	db      03T	        ; Launch control spark cut, this is the                      ;* 77 Tuner Studio HCutLCSpark
				                    ; amount of sparks to remove from
				                    ; SparkCutBase_f when in Launch hard cut
SparkCutBase_f	db      06T	        ; Base number to cut sparks from MS                          ;* 78 Tuner Studio SparkCutBase
				                    ; = MT value - 1
SCALEFAC_f	    db      255T	    ; Scaling factor for STAGED INJECTION MODE                   ;* 79 Tuner Studio ScaleFac
				                    ; (prim flow/total flow*100) 255=100% 123=50%
STGTRANS_f	    db      25T	        ; Staged transition point, rpm*100, kpa,                     ;* 80 Tuner Studio StgTrans
				                    ; or tps raw adc depending on staging
				                    ; method selected (See feature5_f bits 3-4)
STGDELTA_f	    db      03T	        ; Staged operation off at (STGTRANS-STGDELTA)                ;* 81 Tuner Studio StgDelta                                                                          
				                    ; so this is raw data as STGTRANS
BarroHi_f	    db      110T	    ; Barometric Correction Max Limit in KPa (82                 ;* 82 Tuner Studio BarroHi
BarroLow_f	    db      60T	        ; Barometric Correction Lower Limit in KPa                   ;* 83 Tuner Studio BarroLow
SparkCutBNum_f	db      03T	        ; Number of sparks to remove from BASE value when Over Boost ;* 84 Tuner Studio SparkCutBNum
NosLowKpa_f	    db      80T	        ; Minimum KPa to fire Nos Anti-lag                           ;* 85 Tuner Studio NosLowKpa
NosHiKpa_f	    db      120T	    ; Maximum KPa to fire Nos Anti-lag,                          ;* 86 Tuner Studio NosHiKpa
				                    ; Anti-lag will switch off when this is reached
Spark2Delay_f	db      00T	        ; Delay for Spark Table 2 to come in when                    ;* 87 Tuner Studio ST2Delay
				                    ; input received. *0.1   1/10Sec
Out1UpLim_f	    db      00T	        ; Output 1 top limit for window, so output1                  ;* 88 Tuner Studio Out1UpLim
				                    ; will go off above this value unless its 0
				                    ; then its ignored
Out2UpLim_f	    db      00T	        ; Output 2 top limit for window (89)                         ;* 89 Tuner Studio Out2UpLim
NumTeeth_f	    db      12T	        ; Number of teeth for wheel decoder                          ;* 90 Tuner Studio numteeth
MAPThresh_f	    db      30T	        ; MAP dot threshold for Accel Decel                          ;* 91 Tuner Studio mapThresh4                
				                    ; Enrichments *10 (KPa/Sec)

feature6_f	    db      %00000000   ; More feature bits (92)                                     ;* 92 Tuner Studio ?????
VETable3b       equ     1           ; Use VE table 3                                             ;*  Tuner Studio UseVE3 bit0
TargetAFR3b     equ     2           ; Use Target AFR for VE3                                     ;*  Tuner Studio bit1
falsetrigb      equ     4           ; 0=Enable false trigger protection, 1=disable   ; testing   ;*  Tuner Studio falsetrig_dis bit2
wheel_oldb      equ     8           ; 0= new(025) wheel decoder or 1=old(024s9) style ; testing  ;*  Tuner Studio wheel_old bit3
dualdizzyb      equ     $10         ;                                                            ;*  Tuner Studio dualdizzy bit4
TractionCb      equ     $20         ; Traction control system on                                 ;*  Tuner Studio TractionOn bit5
BoostDirb       equ     $40         ; Direction for boost control output                         ;*  Tuner Studio bit6
NoDecelBoostb   equ     $80         ; No decelleration when in boost                             ;*  Tuner Studio DecelBoost bit7 (TPSRPMTarg)?

VE3Delay_f	    db      00T	        ; Delay for VE Table 3 to come in when                       ;* 93 Tuner Studio VE3Delay
				                    ; input received. *0.1   1/10Sec
RPMrate_f:
	            db      00T		    ; Fuel enrichment in mSec for 3000RPM/Sec                    ;* 94 Tuner Studio RPM1S
				                    ; increase or 5% slip if in VSS mode *0.1
       	        db      100T		; Fuel enrichment in mSec for 4000RPM/Sec                    ;* 95 Tuner Studio RPM2S
				                    ; increase or 15% slip if in VSS mode *0.1
             	db      150T		; Fuel enrichment in mSec for 6000RPM/Sec                    ;* 96 Tuner Studio RPM3S
				                    ; increase or 30% slip if in VSS mode *0.1 (96)
       	        db      200T		; Fuel enrichment in mSec for 10000RPM/Sec                   ;* 97 Tuner Studio RPM4S
				                    ; increase or 70% slip if in VSS mode *0.1

RPMthresh_f	    db      2T	        ; Threshhold for RPM change for traction                     ;* 98 Tuner Studio RPMThresh
				                    ; (rpm * 1000) because it checks every 1/10
				                    ; sec and rpm = rpm*100. So 2 = 2000RPM/Sec
				                    ; threshold
TractDeg_f:
	            db      00T		    ; Ignition retard in Degrees for 3000RPM/Sec                 ;* 99 Tuner Studio TractRet1
				                    ; increase or 5% slip if in VSS mode *0.352
	            db      56T		    ; Ignition retard in Degrees for 4000RPM/Sec                 ;* 100 Tuner Studio TractRet2
				                    ; increase or 15% slip if in VSS mode *0.352
	            db      56T		    ; Ignition retard in Degrees for 6000RPM/Sec                 ;* 101 Tuner Studio TractRet3
				                    ; increase or 30% slip if in VSS mode *0.352
	            db      85T	    	; Ignition retard in Degrees for 10000RPM/Sec                ;* 102 Tuner Studio TractRet4
				                    ; increase or 70% slip if in VSS mode *0.352(102)
TractSpark_f:
	            db      00T		    ; Spark Cut from Base number for 3000RPM/Sec                 ;* 103 Tuner Studio TractSC1
				                    ; increase or 5% slip if in VSS mode MAX ALLOWED 5
	            db      00T		    ; Spark Cut from Base number for 4000RPM/Sec                 ;* 104 Tuner Studio TractSC2
				                    ; increase or 15% slip if in VSS mode
	            db      01T		    ; Spark Cut from Base number for 6000RPM/Sec                 ;* 105 Tuner Studio TractSC3
				                    ; increase or 30% slip if in VSS mode
	            db      02T		    ; Spark Cut from Base number for 10000RPM/Sec                ;* 106 Tuner Studio TractSC4
				                    ; increase or 70% slip if in VSS mode (106)

BoostKnock_f	db      00T	        ; Boost to remove from controller target when                ;* 107 Tuner Studio KnockBoost
				                    ; Knock detected (PSI) so value of MS 1 = 7KPa
BoostKnMax_f	db      30T	        ; Max Boost to remove when knocking in PSI so                ;* 108 Tuner Studio BoostMaxKn
				                    ; send 1 to MS this is 7KPa inside the code

feature7_f	    db      %00000010   ; More feature bits     (109)                                ;* 109 Tuner Studio ????
; 029g changed to enable dwell by default
; TractionNos	equ 1   Turn Nos off in Traction Control if traction lost,                       ;*  Tuner Studio TractNOS bit0
				      ; only if Traction oN TractionCb:feature6 bit 6
; dwellcont	    equ 2   Real (crude) dwell control                                               ;*  Tuner Studio dwellcont bit1
; TCcycleSec	equ 4   Hold traction settings for cycles || untill rpm                          ;*  Tuner Studio TractCySec bit2
				      ; stable for 0.1S only if Traction ON
				      ; see TractionCb:feature6 bit 6
; WheelSensor	equ 8   Traction control    RPM Based || VSS                                     ;*  Tuner Studio TractMode bit3
				      ; only if Traction ON
				      ; see TractionCb:feature6 bit 6
; AlphaTarAFR	equ $10  speed density for target afr tables || Alpha-N                          ;*  Tuner Studio KpaTPSTar bit4
				      ; only if Target AFR tables ON
				      ; see TargetAFRb:feature3 bit 8
; TPSTargetAFR	equ $20    0=  Use Target AFR all the while || 1=Only when                       ;*  Tuner Studio TPSTar bit5
				      ; TPS above AFRTarTPS_f  if Target AFR
				      ; tables ON
				      ; see TargetAFRb:feature3 bit 8
;spare	        equ $40                                                                          ;*  Tuner Studio StagedMap2nd bit6
;spare	        equ $80                                                                          ;*  Tuner Studio StagedAnd bit7

dwellcrank_f	db       60T	; cranking dwell in 0.1ms                                         ;* 110 Tuner Studio dwellcrank
dwellrun_f	    db       40T	; running  dwell in 0.1ms  (111)                                  ;* 111 Tuner Studio dwellrun

TractCycle_f:
	            db      03T		; Engine cycles to hold enrichment /                              ;* 112 Tuner Studio TractCyc1
				                ; spark cut / retard for 3000RPM/Sec increase
				                ; or 5% slip if in VSS mode
	            db      05T		; Engine cycles to hold enrichment /                              ;* 113 Tuner Studio TractCyc2
				                ; spark cut / retard for 4000RPM/Sec increase
				                ; or 15% slip if in VSS mode
	            db      08T		; Engine cycles to hold enrichment /                              ;* 114 Tuner Studio TractCyc3
				                ; spark cut / retard for 6000RPM/Sec increase
				                ; or 30% slip if in VSS mode
	            db      12T		; Engine cycles to hold enrichment /                              ;* 115 Tuner Studio TractCyc4                               
				                ; spark cut / retard for 15000RPM/Sec increase
				                ; or 70% slip if in VSS mode

feature8_f	    db      %00000000  ; More feature bits (116)                                      ;* 116 Tuner Studio ????
; spare 1                                                                                         ;* Tuner Studio bit0
; spare 2                                                                                         ;* Tuner Studio bit1
BoostTable3b:   equ     4          ; Use boost table 3 when switch table input on                 ;* Tuner Studio BooTbl2Use bit2
spkeopb         equ     8          ; Enable spark E output (instead of shiftlight)                ;* Tuner Studio spkeop bit3
spkfopb         equ     $10        ; Enable spark F output (instead of knock in)                  ;* Tuner Studio spkfop bit4
DecelMAPb:      equ     $20        ; Use MAP for Decel                                            ;* Tuner Studio bit5
InterpAcelb:    equ     $40        ; Interpole the accel enrichments down to a setpoint           ;* Tuner Studio bit6
Out1_Out3b:     equ     $80        ; Output3 only if output1 is on.                               ;* Tuner Studio bit7

UDSpeedLo_f	    db       00T	   ; Min speed from the Undriven wheel for                        ;* 117 Tuner Studio TractVSSMin
				                   ;traction to work at. Volts *0.0196  5V=255
UDSpeedLim_f	db      255T	   ; Max speed from the Undriven wheel for                        ;* 118 Tuner Studio TractVSSMax
				                   ; traction to work at. Volts *0.0196  5V=255
TCScaleFac_f	db      125T	   ; Difference factor for speed inputs from                      ;* 119 Tuner Studioi TractScale
				                   ; driven and undriven inputs
				                   ; (255=100%) *0.39216
TCSlipFac_f	    db      25T	       ; Slip allowed between wheel sensors at low                    ;* 120 Tuner Studio TractSlip
				                   ; speed (255=100%) *0.39216
AFRTarTPS_f	    db      255T	   ; TPS setpoint to go over to switch to target                  ;* 121 Tuner Studio TPSAFRPoint
				                   ; afr tables in RAW ADC (121)
spare1          db      00T                                                                       ;* 122 Not Used
TCSlipFacH_f	db       5T	       ; Slip allowed between wheel sensors at                        ;* 123 Tuner Studio TractSlipH                                                                                     
				                   ; high speed (255=100%) *0.39216
LC_flatsel_f	db      255T	   ; rpm above which arms flat shift mode                         ;* 124 Tuner Studio LC_flatsel
bc_max_diff	    db      255T	   ; Boost Controller max Difference in KPa                       ;* 125 Tuner Studio bc_max_diff 
Out1Hys_f	    db      00T	       ; Hysterisis for Output1 in Raw ADC (126)                      ;* 126 Tuner Studio out1Hysis
Out2Hys_f	    db      00T	       ; Hysterisis for Output2 in Raw ADC                            ;* 127 Tuner Studio out2Hysis
LC_flatlim      db      55T        ; flat shift revlimit                                          ;* 128 NOT USED
DecelKpa_f	    db      255T	   ; No Decel enrichment above this value in                      ;* 129 Tuner Studio DecelKpa                                                       
				                   ; KPa (129)
OverRunT_f	    db      00T	       ; Over Run Timer before enabling over run                      ;* 130 Tuner Studio OvrRunTimr
				                   ; in Seconds *1
BarCorr300_f	db      255T	   ; Correction factor for KPA Factor for                         ;* 131 NOT USED
				                   ; 300KPa sensor and 400KPa sensors only.
				                   ; 255=100% *0.39216
				                   ; (24%=GM300 28%=6300A Series and
				                   ; 78%= 6400A series)

Out3Source_f	db      00T	       ; Same as Out1Source                                           ;* 132 Tuner Studio out3src

Out3Lim_f	    db      00T	       ; Output 3 On/Off Limit as per standard MSnS                   ;* 133 Tuner Studio out3lim
TimerOut3_f	    db      00T	       ; Output 3 OFF delay timer in Seconds (134) *1                 ;* 134 Tuner Studio out3Timer 
iatBoostSt_f	db      00T	       ; Start point for boost reduction related to                   ;* 135 Tuner Studio IATBoostSt
				                   ; IAT when using Boost controller F -40
iatBoost_f	    db      00T	       ; Amount of IAT to remove 1 PSI from boost                     ;* 136 Tuner Studio IATBoost
				                   ; controller F, same theory as cltDeg_f
tpsBooIAT_f	    db      00T	       ; TPS point to start removing boost from                       ;* 137 Tuner Studio TPSBooIat
				                   ; boost controller in RAW ADC
Out4Source_f	db      00T	       ; Same as Out1Source                                           ;* 138 Tuner Studio out4src bits0 to 4 
Out4Lim_f	    db      00T	       ; Output 4 On/Off Limit as per MSnS                            ;* 139 Tuner Studio out4lim
LC_f_slim_f	    db      00T	       ; Retard timing above this rpm in flat shift mode              ;* 140 Tuner Studio LC_f_slim
LC_f_limangle_f	db      00T	       ; Retard timing to this in flat shift mode                     ;* 141 Tuner Studio LC_f_limangle
spare3_f	    db      00T	       ;   ** SPARE **                                                ;* 142 NOT USED
mindischg_f  	db      05T	       ; minimum discharge period for dwell                           ;* 143 Tuner Studio mindischg
				                   ; control in mSec *0.1

;pwm idle was here

tachconf_f      db      0T         ; tach output config (159)                                     ;* 144 Tuner Studio tachconf bits 0 to 2, tachconfdiv bit7
Trig5_f		    db      0T	       ; wheel decoding (160)                                         ;* 145 Tuner Studio trig5                                         
Trig6_f		    db      0T	       ;  "                                                           ;* 146 Tuner Studio trig6
Trig5ret_f	    db      0T	       ;  "                                                           ;* 147 Tuner Studio trig5ret
Trig6ret_f	    db      0T	       ;  "                                                           ;* 148 Tuner Studio trig6ret


RPMbasedrate_f:
	            db	    05T		   ; These next 4 are for adding AE based on engine rpm           ;* 149 Tuner Studio RPMAE4
                db	    20T		   ; This is the actual engine rpm settings                       ;* 150 Tuner Studio RPMAE3
	            db	    35T		   ;                                                              ;* 151 Tuner Studio RPMAE2
	            db	    55T		   ; (167)                                                        ;* 152 Tuner Studio RPMAE1
RPMAQ_f2        db      01T        ; Amount of fuel to add for 1st area of rpm based AE           ;* 153 Tuner Studio RPMS1
                db      10T        ; Fuel for 2nd rpm area                                        ;* 154 Tuner Studio RPMS2
                db      15T        ; Fuel for 3rd                                                 ;* 155 Tuner Studio RPMS3
RPMAQL_f2:      db      20T        ; Fuel for 4th (171)                                           ;* 156 Tuner Studio RPMS4

n2odel_launch_f: db     00T        ; delay from launch to nitrous activation                      ;* 157 Tuner Studio N2Odel_launch
n2odel_flat_f:   db     00T        ; delay from flat shift to nitrous activation                  ;* 158 Tuner Studio N2Odel_flat
n2oholdon_f:     db     00T        ; how long do extra nitrous fuel and retard hold on            ;* 159 Tuner Studio N2Oholdon 

xxKPaCorr300_f  db      00T        ; KPa correction factor for 400/300KPa sensor (175)            ;* 160 NOT USED


tpsdotrate:
	            db	    05T		    ; These next 4 are delta points for TPSdot                    ;* 161 Tuner Studio taeRates4
				                    ; V/Sec
				                    ; these were hard coded points, now users can
				                    ; select what values
				                    ; they want where. *0.1960784 MAX=25.5
	            db	    20T		    ; So 40 = 0.8V/0.1Sec or 8V/Sec as we check it                ;* 162 NOT USED
				                    ; every 0.1Sec
	            db	    40T		    ;                                                             ;* 163 NOT USED
	            db	    77T		    ; (179)                                                       ;* 164 NOT USED
mapdotrate_f:
	            db	    05T		    ; These next 4 are delta points for MAPdot                    ;* 165 Tuner Studio maeRates4
				                    ; KPa/Sec *10 so 255=2550KPa/Sec as we check it
				                    ; every 0.1Sec
                db	    10T		    ;                                                             ;* 166 NOT USED
	            db	    15T		    ;                                                             ;* 167 NOT USED
	            db	    25T		    ; (183)                                                       ;* 168 NOT USED
MAPAQ_f:
	            db	    20T		    ; Enrichment to add in mSec for first Delta                   ;* 169 Tuner Studio maeBins4
				                    ; mapdotrate_f when in MAPdot mode *0.1
				                    ; these are all interpoled values)
	            db	    50T		    ; Enrichment to add in mSec for second Delta                  ;* 170 NOT USED
				                    ; mapdotrate_f when in MAPdot mode *0.1
	            db	    105T		; Enrichment to add in mSec for third Delta                   ;* 171 NOT USED
				                    ; mapdotrate_f when in MAPdot mode *0.1
	            db	    150T		; Enrichment to add in mSec for fourth Delta                  ;* 172 NOT USED
				                    ; mapdotrate_f when in MAPdot mode *0.1 (187)
TPSAQ_f1:
	            db	    20T		    ; Enrichment to add in mSec for first Delta                   ;* 173 Tuner Studio taeBins4 
				                    ; tpsdotrate_f when in TPSdot mode *0.1
	            db	    50T		    ; Enrichment to add in mSec for second Delta                  ;* 174 NOT USED
				                    ; tpsdotrate_f when in TPSdot mode *0.1
	            db	    105T		; Enrichment to add in mSec for third Delta ;                 ;* 175 NOT USED
				                    ; tpsdotrate_f when in TPSdot mode *0.1
	            db	    150T		; Enrichment to add in mSec for fourth Delta ;                ;* 176 NOT USED
				                    ; tpsdotrate_f when in TPSdot mode *0.1

TPSACOLD_f1	    db      90T	        ; TPSACOLD (ms to add in when cold) *0.1                      ;* 177 Tuner Studio taeColdA4
TPSTHRESH_f1	db      03T	        ; TPSTHRESH for Accel enrichment when in                      ;* 178 Tuner Studio tpsThresh4
				                    ; TPS mode *0.1953125
TPSASYNC_f1	    db      02T	        ; TPSASYNC (accel enrich time in 1/10                         ;* 179 Tuner Studio taeTime4
				                    ; second increments) or in Enfgine Cycles.
TPSDQ_f1	    db      100T	    ; TPSDQ   (195)                                               ;* 180 Tuner Studio tdePct4
ACMULT_f1	    db      100T	    ; Cold ACCELMULT                                              ;* 181 Tuner Studio taeColdM4
OverRunClt_f1	db      100T	    ; No Over run fuel cut when below this                        ;* 182 Tuner Studio OvrRunClt
				                    ; coolant temp F-40 (197)
AccelDecay_f	db       00T	    ; This is the value in mS that the Accel                      ;* 183 Tuner Studio DecayAcPoint
				                    ; enrichment will end up at when the timer
				                    ; has run. *0.1

feature9_f	    db      %00000000	;  (199)                                                      ;* 184 Tuner Studio ????
CrankPWTableb:  equ     1           ; Use cranking PW Table                                       ;* Tuner Studio bit0
ASETableb:      equ     2           ; After start enrichment use table                            ;* Tuner Studio bit1
NoAccelASEb:    equ     4           ; No Accel Enrich during After start enrichment               ;* Tuner Studio AccelMapASE bit2
BaroCorConstb:  equ     8           ; If Alpha-n Mode then do we use MAP for Baro cor constantly. ;* Tuner Studio AlphBaroCor bit3
RpmAEBased:     equ     $10         ; RPM Based Accel Enrichment                                  ;* Tuner Studio RPMBAsedAE bit4
MassAirFlwb:    equ     $20         ; Using Mass AirFlow meter instead of a MAP sensor.           ;* Tuner Studio AFMUse bit5
NoAirFactorb:   equ     $40         ; If using MAF do we use Air Density in fueling cals?         ;* Tuner Studio AirCorAFM bit6
ConsBarCorb:    equ     $80         ; Constant Bar Cor using MAP on X7                            ;* Tuner Studio ConsBaroCor bit7
...
flash_table0_end:                ; marker for easy lookup in lst file
        ...
        org     $E100
flash_table1:           ; FUEL 1   12x12 Total Bytes = 144                 ;* Tuner Studio Page 1
VE_f1:                                                                     ;* 0 Tuner Studio veBins1
	...
	db      75T,77T,79T,73T,82T,82T,82T,85T,87T,89T,99T,100T; VE (11,0-11)  ;* 143

EGOTEMP_f	    db      200T	; EGOTEMP                                   ;* 144 Tuner Studio egoTemp1
EGOCOUNTCMP_f	db      16T	    ; EGOCOUNTCMP                               ;* 145 Tuner Studio egoCount1
EGODELTA_f	    db      1T	    ; EGODELTA                                  ;* 146 Tuner Studio egoDelta1
EGOLIMIT_f	    db      15T	    ; EGOLIMIT                                  ;* 147 Tuner Studio egoLimit1
REQ_FUEL_f1	    db      155T	; REQFUEL (148)                             ;* 148 Tuner Studio reqfuel1
DIVIDER_f1	    db      4T	    ; DIVIDER                                   ;* 149 Tuner Studio divider1
Alternate_f1	db      0	    ; alternate or simult (single table ONLY)   ;* 150 Tuner Studio alternate1
INJOPEN_f1	    db      10T	    ; INJOPEN                                   ;* 151 Tuner Studio injOpen1
INJOCFUEL_f1	db      0T	    ; INJOCFUEL  NOT USED NOW !!!!!             ;* 152  NOT USED
				                ; Kept to fill hole                         
INJPWM_f1	    db      75T	    ; INJPWM                                    ;* 153 Tuner Studio injPwmP1
INJPWMT_f1   	db      255T	; INJPWMT                                   ;* 154 Tuner Studio injPwmT1 
BATTFAC_f1	    db      12T	    ; BATTFAC                                   ;* 155 Tuner Studio battFac1
rpmk_f1		    db      $05	    ; RPMK[0]                                   ;* 156 Tuner Studio rpmk1 (16 bit)
		        db      $DC	    ; RPMK[1]                                   ;* 157 Tuner Studio rpmk1
RPMRANGEVE_f1:
	            db      5T		; RPMRANGEVE[0]                             ;* 158 Tuner Studio rpmBins1
	            db      10T		; RPMRANGEVE[1]                             ;* 159
	            db      15T		; RPMRANGEVE[2]                             ;* 160
	            db      20T		; RPMRANGEVE[3]                             ;* 161
	            db      28T		; RPMRANGEVE[4]                             ;* 162
	            db      36T		; RPMRANGEVE[5]                             ;* 163
	            db      44T		; RPMRANGEVE[6]                             ;* 164
	            db      52T		; RPMRANGEVE[7]                             ;* 165
	            db      55T		; RPMRANGEVE[8]                             ;* 166
	            db      60T		; RPMRANGEVE[9]                             ;* 167
	            db      62T		; RPMRANGEVE[10]                            ;* 168
	            db      65T		; RPMRANGEVE[11]                            ;* 169
KPARANGEVE_f1:
	            db      20T		; KPARANGEVE[0]                             ;* 170 Tuner Studio mapBins1
	            db      30T		; KPARANGEVE[1]                             ;* 171
	            db      40T		; KPARANGEVE[2]                             ;* 172
       	        db      50T		; KPARANGEVE[3]                             ;* 173
	            db      60T		; KPARANGEVE[4]                             ;* 174
	            db      75T		; KPARANGEVE[5]                             ;* 175
	            db      90T		; KPARANGEVE[6]                             ;* 176
	            db      100T	; KPARANGEVE[7]                             ;* 177
	            db      110T	; KPARANGEVE[8]                             ;* 178
	            db      120T	; KPARANGEVE[9]                             ;* 179
	            db      130T	; KPARANGEVE[10]                            ;* 180
	            db      150T	; KPARANGEVE[11]                            ;* 181

config11_f1	    db      113T	; Config11 (originally 113T for 8 cyl) (182);* 182 Tuner Studio Config1
config12_f1	    db      112T	; Config12 (originally 112T for 8 injectors);* 183 Tuner Studio Config3(Config2?)
config13_f1	    db      00T	    ; Config13                                  ;* 184 Tuner Studio Config3

EGOrpm_f	    db      13T	    ; RPMOXLIMIT                                ;* 185 Tuner Studio egoRPM1
...
flash_table1_end:
        ...
        org     $E200
flash_table2:           ; FUEL 2 - For PW2 when in Dual Table mode, if not       ;* Tuner Studio Page 2
			            ; in DT mode then this whole page is ignored
VE_f2:                  ; 12x12 Total Bytes = 144                                ;* 0 Tuner Studio veBins2
	db      39T,40T,41T,44T,44T,44T,45T,45T,45T,46T,47T,50T ; VE (0,0-11)
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (1,0-11)
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (2,0-11)
    db      52T,55T,55T,57T,60T,61T,61T,65T,67T,70T,72T,75T ; VE (3,0-11)
...
EGOtemp_f2	        db      200T    ; For second O2 sensor (feature12_f2)       ;* 144 Tuner Studio egoTemp2
EGOCOUNTCMP_f2	    db      16T	    ; NOT USED                                  ;* 145 NOT USED
EGOdelta_f2	        db      1T	    ; For second O2 sensor (feature12_f2) (146) ;* 146 Tuner Studio egoDelta2
EGOlimit_f2	        db      10T     ; For second O2 sensor (feature12_f2)       ;* 147 Tuner Studio egoLimit2
REQ_FUEL_f2	        db      155T    ; (148)                                     ;* 148 Tuner Studio reqFuel2
Divider_f2	        db      4T                                                  ;* 149 Tuner Studio divider2
Alternate_f2	    db      0T	    ; NOT USED                                  ;* 150 Tuner Studio alternate2
InjOpen_f2	        db      10T                                                 ;* 151 Tuner Studio injOpen2
InjOCFuel_f2	    db      0T	    ; NOT USED NOW !!!!! Kept to fill hole      ;* 152 NOT USED
INJPWM_f2	        db      100T                                                ;* 153 Tuner Studio injPwm2
INJPWMT_f2	        db      255T                                                ;* 154 Tuner Studio injPwmT2
BATTFAC_f2	        db      12T     ; (155)                                     ;* 155 Tuner Studio battFac2
rpmk_f2:	                        ; type=byte  entries=2  total bytes=2       
	db      $05,$DC		            ; (156,157)                                 ;* 156,157 Tuner Studio rpmk2 (16 bit)
RPMRANGEVE_f2:	                    ; type=byte  entries=8  total bytes=8
	db      5T,10T,15T,20T,28T,36T,44T,52T,55T,60T,65T,70T                      ;* 158 Tuner Studio rpmBins2
KPARANGEVE_f2:                      ; type=byte  entries=8  total bytes=8
	db      20T,30T,40T,50T,60T,75T,90T,100T,110T,120T,130T,140T                ;* 170 Tuner Studio mapBins2
config11_f2:
config21_f	       db      113T	    ; NOT USED by code, but MT??(182)           ;* 182 Tuner Studio Config1
config12_f2:
config22_f	       db      112T     ; NOT USED                                  ;* 183 Tuner Studio Config3 (Config2?)
config13_f2:
config23_f	       db      %00000000       ; (184)                              ;* 184 Tuner Studio Config3
;                  equ $02   Narrow Band   |  Wide Band   Note: DT bank 2 only (second O2 sensor type)
                                                              ;(feature12_f2)
;                  equ $04   Speed Density |  Alpha N     Note: DT bank 2 only

EGOrpm_f2	        db      13T	     ; RPMOXLIMIT for second O2 sensor if used (feature12_f2)   ;* 185 Tuner Studio primePulse2 and egoRPM2 ????
       		        db      0	     ; not used                                                 ;* 186 Tuner Studio fastIdleT2
O2targetV_f2	    db      26T	     ; VOLTOXTARGET for second O2 sensor if used (feature12_f2) ;* 187 Tuner Studio egoSwitch2

feature12_f2        db      %00000000  ; (188)                                                  ;* 188 Tuner Studio TwoLambda
SecondO2b           equ     1
				 ; Mode (DTmode_f bit 4)???
...
flash_table2_end:
        ...
        org     $E300
flash_table3:           	; SPARK Table 1                                   ;* Tuner Studio Page 3
ST_f1: 				; *0.352 -28.4 Min -10  Max 80                            ;* 0 Tuner Studio advTable1
...
RPMRANGEST_f1:
	db      05T,07T,13T,20T,30T,40T,50T,60T,61T,62T,63T,64T                        ;* 144 Tuner Studio rpmBins3

				; RPMRANGEST[0-11]

KPARANGEST_f1:
	db      30T,40T,50T,60T,70T,80T,90T,100T,110T,120T,130T,140T  ;(last byte 167) ;* 156 Tuner Studio mapBins3
				; KPARANGEST[0-b]

;; org $d3a8  ; stick them up out of the way at known values
TriggAngle_f	db      171T	; TriggAngle (60 deg)     (168)  *0.352            ;* 168 Tuner Studio TrigAng (= 227T)
FixedAngle_f	db      0T	    ; FixedAngle   *0.352  -28.4   min -10 Max 80      ;* 169 Tuner Studio FixAng
				                ; THIS MUST BE -10 (0) for non fixed angle
TrimAngle_f	    db      0T	    ; TrimAngle (NOT cleared on startup)  *0.352       ;* 170 Tuner Studio Trim
CrankAngle_f	db      56T	    ; Cranking advance (10deg)  *0.352 -28.4           ;* 171 Tuner Studio CrankAng
				                ;  min -10 max 80
SparkHoldCyc_f	db      1T	    ; SparkHoldCyc (hold spark x cycles on             ;* 172 Tuner Studio IgHold
				                ; stall and restart)
SparkConfig1_f	db   %00001100	; SparkConfig1 (Normal trigger, trigger            ;* 173 Tuner Studio Trig_plus
...
IdleRPMThresh_f	db	    0T	    ; below this RPM value idle advance (0 disables)   ;* 176 Tuner Studio IdleAdvRPM
IdleCLTThresh_f db	    0T	    ; below this CLT value don't use idle advance      ;* 177 Tuner Studio IdleAdvClt
IdleDelayTime_f db	    1T	    ; wait this long before using the idle advance     ;* 178 Tuner Studio IdleDelayTime

StgCycles_f	    db	    25T	    ; gradually bring on secondary injectors over      ;* 179 Tuner Studio StgCycles
				                ; this many ignition events

Stg2ndParmKPA_f db      0T      ; Staged 2nd parameter kPa value                   ;* 180 Tuner Studio Stg2ndParmKPA

Stg2ndParmDlt_f db      0T      ; staged 2nd parameter delta                       ;* 181 Tuner Studio Stg2ndParmDlt

spare3_182      db      0T      ; spare byte as demo                               ;* 182 Tuner Studio spare3_182
spare3_183      db      0T      ; spare byte as demo                               ;* 183 Tuner Studio spare3_183
spare3_184      db      0T      ; spare byte as demo                               ;* 184 Tuner Studio spare3_184

flash_table3_end:

        org     $E400
flash_table4:                   ; Spark Table 2, used when input switched    ;* Tuner Studio page 5
				; low if selected
ST_f2:				; *0.352  -28.4   Min -10 Max 80                         ;* 0 Tuner Studio advTable2
	db    53T,53T,58T,70T,90T,119T,131T,131T,131T,131T,131T,131T ;(0,0-11)
	db    53T,53T,58T,70T,87T,113T,119T,119T,119T,119T,119T,119T ;(1,0-11)
	db    58T,58T,64T,70T,84T,107T,113T,113T,113T,113T,113T,113T ;(2,0-11)
	db    58T,58T,64T,70T,81T,104T,107T,107T,107T,107T,107T,107T ;(3,0-11)
...
RPMRANGEST_f2:
        db      05T,07T,13T,20T,30T,40T,50T,60T,61T,62T,63T,64T             ;* 144 Tuner Studio rpmBins4
				; RPMRANGEST[0-b]
...
flash_table4_end:
        ...
        org     $E500
flash_table5:    ; FUEL Table 3 (VE3) used when input switched low if selected  ;* Tuner Studio Page 6
VE_f3:
	db      39T,40T,41T,44T,44T,44T,45T,45T,45T,46T,47T,50T ; VE (0,0-11)       ;* 0  Tuner Studio veBins3
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (1,0-11)
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (2,0-11)
	db      52T,55T,55T,57T,60T,61T,61T,65T,67T,70T,72T,75T ; VE (3,0-11)
	db      52T,55T,55T,57T,60T,61T,61T,65T,67T,70T,72T,75T ; VE (4,0-11)
	...
	db      75T,77T,79T,73T,82T,82T,82T,85T,87T,89T,99T,100T; VE (11,0-11)
RPMRANGEVE_f3:
	db      5T	    ; RPMRANGEVE[0] (144)                                      ;* 144 Tuner Studio rpmBins5
	db      10T	    ; RPMRANGEVE[1]
	db      15T  	; RPMRANGEVE[2]
	db      20T	    ; RPMRANGEVE[3]
	db      28T	    ; RPMRANGEVE[4]
...
KPARANGEVE_f3:
    db      20T	    ; KPARANGEVE[0] (156)                                    ;* 156 Tuner Studio mapBins5
	db      30T	    ; KPARANGEVE[1]
	db      40T	    ; KPARANGEVE[2]
	db      50T	    ; KPARANGEVE[3]
	db      60T	    ; KPARANGEVE[4]
...
ASEVTbl_f:
	db      30T	    ; -40F This is the ASE table, only used if               ;* 168 Tuner Studio ASETable1
			        ; $02 set in feature9_f     (168)                        
	db      20T	    ; -20F  this is in percentage *1 so 30 = 30%             ;* 169 Tuner Studio ASETable2
	db      15T	    ; 0F                                                     ;* 170 Tuner Studio ASETable3
	db      12T	    ; 20F                                                    ;* 171 Tuner Studio ASETable4
	db      10T	    ; 40F                                                    ;* 172 Tuner Studio ASETable5
	db       9T	    ; 60F                                                    ;* 173 Tuner Studio ASETable6
	db       8T	    ; 80F                                                    ;* 174 Tuner Studio ASETable7
	db       7T	    ; 100F                                                   ;* 175 Tuner Studio ASETable8
	db       6T	    ; 130F                                                   ;* 176 Tuner Studio ASETable9
	db       5T	    ; 160F   (177)                                           ;* 177 Tuner Studio ASETable10

AWC_f1  	     db      250T	    ; After Start Warmup Time                                                ;* 178 Tuner Studio ASETime
feature10_f5     db      %00000000  ; (179)                                                                  ;* 179 Tuner Studio 
aseIgnCountb     equ     1          ;  AFTER START Enrichment Seconds || Engine Cycles^                      ;* Tuner Studio aseIgnCount bit0
ASEHoldb:        equ     2          ; Hold ASE from decaying for a period of time determined by TimFixASE_f  ;* Tuner Studio ASEFixDe bit1
MAPHoldb:        equ     4          ; Fix MAP value during Fixed ASE timer                                   ;* Tuner Studio VEFixASE bit2


TimFixASE_f     db      5T     ; Amount of time or cycles to hold ase to fixed value (180)                 ;* 180 Tuner Studio ASEFixTim
                               ; rather than decay to 0 % over the timer
CltFixASE_f     db      85T    ; Coolant temp setpoint to use Fixed value ASE                              ;* 181 Tuner Studio ASEFixTemp
MAPFixASE_f     db      60T    ; If in fixed MAP mode then hold MAP at this value during fixed ASE time    ;* 182 Tuner Studio VEFixValue
...
flash_table5_end:
        ...
        org     $E600
flash_table6:		; AFR Table 1 - 8x8 AFR targets for VE table 1    ;* Tuner Studio Page 7
AFR_f1:			; This is in RAW ADC so 255 = 5V from O2 sensor
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (0,0-7)      ;* 0 Tuner Studio afrBins1
...
RPMRANGEAFR_f1:
	db      5T	    ; RPMRANGEAFR1[0]                            ;* 64 Tuner Studio rpmBins6
...
KPARANGEAFR_f1:
	db      15T	    ; KPARANGEAFR1[0]                            ;* 72 Tuner Studio mapBins6
...
AFR_f2:			; This is in RAW ADC so 255 = 5V from O2 sensor
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (0,0-7) ;* 80 Tuner Studio afrBins2
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (1,0-7)
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (2,0-7)
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (3,0-7)
	db      130T,130T,130T,130T,130T,130T,130T,130T; AFR (4,0-7)
...
RPMRANGEAFR_f2:
	db      5T	    ; RPMRANGEAF2[0]                             ;* 144 Tuner Studio mapBins7
...
KPARANGEAFR_f2:
	db      15T	    ; KPARANGEAF2[0]                             ;* 152 Tuner Studio mapBins7 
...
Nos2Rpm_f     db     255T  ; rpm starts at            ;* 160 Tuner Studio Nos2Rpm
Nos2RpmMax_f  db     255T  ; rpm ends st              ;* 161 Tuner Studio Nos2RpmMax
Nos2delay_f   db     0T    ; delay after stage 1      ;* 162 Tuner Studio Nos2Delay
Nos2Angle_f   db     0T    ; retard                   ;* 163 Tuner Studio Nos2Angle
Nos2PWLo_f    db     0T    ; +pw at low rpm           ;* 164 Tuner Studio Nos2PWLo
Nos2PWHi_f    db     0T    ; +pw at max rpm           ;* 165 Tuner Studio Nos2PWHi

; oddfire wheel decoder bits - very experimental
outaoffs_f   db     0T    ; offset in steps           ;* 166 Tuner Studio outaoffs
;bit0 = 0, 22.5             ignore for now
;bit1 = 0, 45               use 0 or 45 or 90 only
;bit2 = 0, 90
outaoffv_f   db     0T    ; 0-45deg variable offset   ;* 167 Tuner Studio outaoffv
outboffs_f   db     0T                                ;* 168 Tuner Studio outboffs
outboffv_f   db     0T                                ;* 169 Tuner Studio outboffv
outcoffs_f   db     0T                                ;* 170 Tuner Studio outcoffs
outcoffv_f   db     0T                                ;* 171 Tuner Studio outcoffv
outdoffs_f   db     0T                                ;* 172 Tuner Studio outdoffs
outdoffv_f   db     0T                                ;* 173 Tuner Studio outdoffv
outeoffs_f   db     0T                                ;* 174 Tuner Studio outeoffs
outeoffv_f   db     0T                                ;* 175 Tuner Studio outeoffv
outfoffs_f   db     0T                                ;* 176 Tuner Studio outfoffs
outfoffv_f   db     0T                                ;* 177 Tuner Studio outfoffv
...
bc_kpa_f:
	db      100T,100T,100T,100T,100T,100T                        ;* 0 Tuner Studio bcBins1
...
RPMRANGEbc_f:
	db      10T,20T,30T,40T,50T,70T   ; RPMRANGEbc[0-5]           ;* 36 Tuner Studio bcrpmBins1
TPSRANGEbc_f:
	db      51T,77T,102T,127T,179T,230T   ; TPSRANGEbc[0-5]       ;* 42 Tuner Studio bctpsBins1

;boost controller, duty cycle target rpm vs tps

bc_dc_f:
	db      50T,50T,50T,50T,50T,50T                               ;* 48 Tuner Studio bcBins2
...
RPMRANGEbc_f2:
	db      10T,20T,30T,40T,50T,70T   ; RPMRANGEbc[0-5]           ;* 84 Tuner Studio bcrpmBins2
TPSRANGEbc_f2:
	db      51T,77T,102T,127T,179T,230T   ; TPSRANGEbc[0-5]       ;* 90 Tuner Studio bctpsBins2


; Second boost target table, switched over on input if selected.
bc3_kpa_f:
	db      100T,100T,100T,100T,100T,100T                         ;* 96 Tuner Studio bcBins3
	db      100T,120T,120T,120T,120T,120T
	db      100T,120T,120T,120T,120T,120T
	db      100T,120T,120T,120T,120T,120T
	db      100T,120T,120T,120T,120T,120T
...
RPMRANGE3bc_f:
	db      10T,20T,30T,40T,50T,70T   ; RPMRANGE2bc[0-5]         ;* 132 Tuner Studio bcrpmBins3
TPSRANGE3bc_f:
        db      51T,77T,102T,127T,179T,230T   ; TPSRANGE2bc[0-5] ;* 138 Tuner Studio bctpsBins3
...
flash_table7_end:
                ...
                org     $E800
flash_table8:                                                                            ;* Tuner Studio Page 9
idle_dc_lo	      db      50T	; Idle duty cycle at lower temp for 2-wire               ;* 0 Tuner Studio idle_dc_lo
				                ; Fielding Idle control *1  MIn = 0 Max = 100
idlePeriod_f	  db      25T	; idle period in ignition events                         ;* 1 Tuner Studio idleperiod
;idlekickrpm_f	  db      07T	; low rpm to trigger kick up of pwm idle duty
;idlekickdc_f	  db      55T	; default duty cycle
idlecrankdc_f	  db      50T	; cranking idle dc kg                                    ;* 2 Tuner Studio idlecrankdc
idledelayclock_f  db	  01T	; idle dashpot settling delay kg                         ;* 3 Tuner Studio idledelayclock

idledashdc_f	  db      45T	; dashpot default duty                                   ;* 4 Tuner Studio idledashdc
idlemindc_f       db      27T	; minimum duty cycle                                     ;* 5 Tuner Studio idlemindc

idle_dc_hi        db      00T   ; rmd Idle duty cycle at upper temp for 2-wire (warmup)  ;* 6 Tuner Studio idle_dc_hi
;idlekpa1_f        db      100T  ; rmd
;idlekpa2_f        db      100T  ; rmd
;ikpamin1_f        db      30T   ; rmd
;ikpamin2_f        db      30T   ; rmd
ictlrpm1_f        db      3T	; idle deviation rpmx10 kg                               ;* 7 Tuner Studio ict1rpm1 
ictlrpm2_f        db      10T	; idle deviation rpmx10 kg                               ;* 8 Tuner Studio ict1rpm2
Ideadbnd_f        db      03T	; idle deadband range kg                                 ;* 9 Tuner Studio Ideadbnd
Idashdelay_f      db      0T	; AIC closure delay ign events kg                        ;* 10 Tuner Studio Idashdelay
idlefreq_f        db      100T  ; rmd                                                    ;* 11 Tuner Studio idlefreq
;delay2rpm_f       db      20T   ; rmd
idlestartclk_f    db      10T	; startup decay timer ign events kg                      ;* 12 Tuner Studio idlestartclk
idlePeriod2_f     db      10T   ; rmd                                                    ;* 13 Tuner Studio idlePeriod2
irestorerpm_f     db      15T   ; rmd                                                    ;* 14 Tuner Studio irestorerpm
idleclosedc_f     db      0T    ; rmd (015)                                              ;* 15 Tuner Studio idleclosedc

feature13_f       db      2T                                                             ;* 16 Tuner Studio ?????
PWMidleb          equ     $01   ; pwm idle on vs B&G on/off                              ;* Tuner Studio pwmIdle bit0
idle_warmupb      equ     $02   ; pwm idle warmup open loop                              ;* Tuner Studio pwmidlewhen bit1
idle_clb          equ     $04   ; pwm idle closed loop                                   ;* Tuner Studio pwmidlewhen bit2
cltMAPb:          equ     $08   ; Use Correction table in the Air Density factor         ;* Tuner Studio IATcor bit3
CltMATCheckb:     equ     $10   ; Correction table MAT or IAT based                      ;* Tuner Studio IATCorTy bit4

FASTIDLEtemp_f	  db      105T	; Feilding 2-Wire Idle control Fast Idle                 ;* 17 Tuner Studio fastIdleTemp 
				                ; lower temperature F -40
slowIdleTemp_f	  db      234T	; Feilding 2-Wire Idle control Slow Idle                 ;* 18 Tuner Studio slowIdleTemp
				                ; upper temperature F -40
fastIdle_f	      db      110T	; Fast Idle RPM (RPM*10 100-2550 rpm range)              ;* 19 Tuner Studio fastIdleRPM
slowIdle_f	      db       65T	; Slow Idle RPM (RPM*10 100-2550 rpm range)              ;* 20 Tuner Studio slowIdleRPM
idleThresh_f	  db       30T	; TPS Raw value for Idle mode to kick in.                ;* 21 Tuner Studio idleThresh                                              

WWU_f1
	db      180T	; WWU (-40 F) (22)                                                   ;* 22 Tuner Studio wueBins9
	db      180T	; WWU (-20 F)                                                        ;* 23
	db      160T	; WWU (0 F)                                                          ;* 24
	db      150T	; WWU (20 F)                                                         ;* 25
	db      135T	; WWU (40 F)                                                         ;* 26
	db      125T	; WWU (60 F)                                                         ;* 27
	db      113T	; WWU (80 F)                                                         ;* 28
	db      108T	; WWU (100 F)                                                        ;* 29
	db      102T	; WWU (130 F)                                                        ;* 30
	db      100T	; WWU (160 F)                                                        ;* 31
...
CrankPWs_f:
	...
	db      55T	; 40F                                                                    ;* 36 Tuner Studio CrankPWT5
	db      50T	; 60F                                                                    ;* 37 Tuner Studio CrankPWT6
	db      45T	; 80F                                                                    ;* 38 Tuner Studio CrankPWT7
	db      40T	; 100F                                                                   ;* 39 Tuner Studio CrankPWT8
	db      35T	; 130F                                                                   ;* 40 Tuner Studio CrankPWT9
	db      30T	; 160F                                                                   ;* 41 Tuner Studio CrankPWT10

feature11_f4    db  %00010000       ; (42)                                                  ;* 42 Tuner Studio ?????
AlwaysPrimeb:   equ     1    ; Only fire pump if Prime pulse ON | Prime pump every time     ;* Tuner Studio AlwaysPrime bit0
PrimeLateb      equ     2    ; Fire prime pulse after 2 seconds                             ;* Tuner Studio PrimeLate bit1
PrimeTwiceb     equ     4    ; Fire the Prime Pulses Twice                                  ;* Tuner Studio PrimeLate bit2
NoPrimePb:      equ     8    ; Use Priming Table or Use Prime Pulse                         ;* Tuner Studio PrimeTBox bit3
cltcrankb:      equ     $10  ; use coolant temp for crank pulsewidth                        ;* Tuner Studio crankpwip bit4
matcrankb:      equ     $20  ; use inlet air temp for crank pulsewidth. Both means average  ;* Tuner Studio crankpwip bit5
ExCrFuelb:      equ     $40  ; Look at TPS to see if we trigger extra fuel during cranking? ;* Tuner Studio ExFuelCrank bit6

CrankRPM_f	    db      $03	 ; Maximum RPM for cranking (rpm*100)                        ;* 43 Tuner Studio CrankRPM
tpsflood_f	    db      $B2	 ; Throttle position for floodclear mode in RAW ADC          ;* 44 Tuner Studio tpsflood
...
cltMATcorr_f:                   
                db      100T ; 7 positions for CLT related correction to          (47)   ;* 47 Tuner Studio IATCorS
                db      98T  ; IAT Air Density Correction 110 = correction * 110%        ;* 48 NOT USED
                db      96T                                                              ;* 49 NOT USED
                db      94T                                                              ;* 50 NOT USED
                db      92T                                                              ;* 51 NOT USED
                db      90T                                                              ;* 52 NOT USED
                db      88T  ; (53)                                                      ;* 53 NOT USED

RPMReduLo_f     db      10T  ; lowest rpm to reduce correction by (54)                   ;* 54 Tuner Studio RPMRedLo
RPMReduHi_f     db      30T  ; Highest rpm when correction is back to normal             ;* 55 Tuner Studio RPMRedHi

CltMATRange:
    db      200T    ; 160F   User defined Temp settings for Clt Related Air Dens (56)    ;* 56 Tuner Studio IATCTemp
    db      207T    ; etc.                                                               ;* 57 NOT USED 
    db      216T    ;                                                                    ;* 58 NOT USED
    db      225T    ;                                                                    ;* 59 NOT USED
    db      234T    ;                                                                    ;* 60 NOT USED
    db      243T    ;                                                                    ;* 61 NOT USED
    db      252T    ; 212F (62)                                                          ;* 62 NOT USED

;rotary leading trailing split timing 6x6 table
split_f:   ; (63)
	db      29T,88T,88T,88T,88T,88T                                                      ;* 63 Tuner Studio splitTable
...
RPMRANGEsplit_f: ;(99)
	db      6T,8T,30T,40T,50T,70T       ; RPMRANGE2bc[0-5]                               ;* 99 Tuner Studio splitrpmBins
KPARANGEsplit_f: ;(105)
    db      40T,50T,60T,80T,105T,106T   ; TPSRANGE2bc[0-5]                               ;* 105 Tuner Studio splitkpaBins
p8feat1_f:        db      0T            ; a page8 config byte (111)                      ;* 111 Tuner Studio rotary2
rotary2b          equ     1             ; enable/disable twin rotor mode for BIT

fixedsplit_f:     db      0T            ; fixed split for testing like Fixed in spark (112)  ;* 112 Tuner Studio fixedsplit
