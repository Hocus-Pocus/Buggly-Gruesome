...
;is PTC4 an input?    - see also 'B' code section
  ...
  beq   norm_op_ddrc                                                ;* GO TO LINE 941 
  lda   #%00001111      ; make PTC4 an input for second trigger
  bra   op_ddrc
	
norm_op_ddrc:                                                       ;* LINE 941                                                        
...
;  mov   #T1SCX_NO_PWM,T1SC1	 ; make this normal port output (PWM MODE is #$5E)
  ...
  beq   setpwmsingle                                        ;* GO TO LINE 988
  lda   INJPWM_f2
  bra   store_pwm
	
setpwmsingle:                                               ;* LINE 988
  lda   INJPWM_f1
...
; real angle. Shouldn't happen if angles set correctly.
  ...
  bhi   init_crang		        ; trigger angle > 20, continue                       ;* GO TO LINE 1185
  bset  nextcyl,EnhancedBits4
	
init_crang:                                                                          ;* LINE 1185
  lda   CrankAngle_f                ;* CrankAngle_f = 10/.352=28
  sta   SparkAngle
  brset EDIS,personality,init_edis                                                   ;* EDIS = 0
...
init_cont:                                                                           ;* LINE 1206
  ...
  brset WHEEL,personality,init_wheel   ;* WHEEL = 1,                                 ;* GO TO LINE 1212
  bra   init_no_hold
	
init_wheel:                                                                          ;* LINE 1212
...
; if 2nd trig rising and falling then store existing value of pin to monitor state
  ...
  beq   init_no_hold                                           ;* GO TO LINE 1232
...
; decide if we are doing multiple wasted spark outputs
  ...
  brset WHEEL,personality,wsp_init                              ;* WHEEL = 1, GO TO LINE 1268
...
;If HEI set bypass to 0v
  ...
  brclr HEI7,personality,not_hei7_init                          ;* HEI7 = 0, GO TO LINE 1284
  bset  aled,portc
	
not_hei7_init:                                                  ;* LINE 1284

  brclr DUALEDIS,personality,chk_out                            ;* DUALEDIS = 0, GO TO LINE 1289
  bset  EDIS,personality	            ; DUALEDIS implies EDIS

chk_out:                                                        ;* LINE 1289
**** add in some sanity checks for outputs vs. code base ****
  lda   personality                                             ;* personality = 1
  bne   check_out_config                                        ;* GO TO LINE 1305
...
check_out_config:                                               ;* LINE 1305
  brclr REUSE_LED17,outputpins,block_neon                       ;* REUSE_LED17 = 1, GO TO LINE 1310
  brclr REUSE_LED19,outputpins,block_neon
  bra   check_msns
	
block_neon:                                                     ;* LINE 1310
  brclr MSNEON,personality,check_msns                           ;* MSNEON = 0, GO TO LINE 1316
  mov   #2,tmp4
  bset  config_error,feature2	       ; if MSNEON but haven't reused led17&19 then error
  jmp   done_checks

check_msns:                                                      ;* LINE 1316
  brclr MSNS,personality,check3                                  ;* MNS = 0, GO TO LINE 1324
...
check3:                               ; check for idle conflict  ;* LINE 1324

;  brclr PWMidle,feature2,check4
  lda   feature13_f
  bit   #pwmidleb                                                         ;* pwmidleb = 1
  beq   check4                                                            
  brclr REUSE_FIDLE,outputpins,check4                                     ;* REUSE_FIDLE = 0, GO TO LINE 1335
  mov   #4,tmp4
  bset  config_error,feature2	     ; trying to use PWM idle and spark on FIDLE
  jmp   done_checks

check4:                              ; check we don't have Water and Fan control as both use X2 ;* LINE 1335
  lda   feature3_f
  bit   #WaterInjb                                                      ;* WaterInjb = 0
  beq   check5                                                          ;* GO TO LINE 1347
...
check5:                                                                ;* LINE 1347
  brclr Nitrous,feature1,check6                                        ;* Nitrous = 0, GO TO LINE 1358
...
check6:                         ; 7pin HEI must have spark output B (LED19) defined. For bypass output ;* LINE 1358
  brclr HEI7,personality,check7                                           ;* HEI7 = 0, GO TO LINE 1366
...
check7:                         ; Do some checks on wasted spark outputs  ;* LINE 1366

  brset rotary2,EnhancedBits5,check8a                                     ;* rotary2 = 0
	
; coilc is the pain - set if LED18=1 and LED18_2=1

  brclr wspk,EnhancedBits4,check8	    ; don't bother if we aren't doing multiple outputs ;* wspk = 1
  brclr WHEEL,personality,check8                                                           ;* wheel = 1
  brset out3sparkd,feature2,ck74	    ; 4th output                                       ;* out3sparkd = 0
  brclr REUSE_LED18,outputpins,ck72	    ; not 3rd output                                   ;* REUSE_LED18 is clear, GO TO LINE 1390
...
check7b:	                    ; can't use FIDLE for spark if doing wasted spark          ;* LINE 1406
...
; first check for dual dizzy feature
  ...
  beq   ck7c3                                                                      ;* trig4_f = 0  GO TO LINE 1424
  brclr out3sparkd,feature2,ck7err	; 4th output
	
ck7c3:                                                                             ;* LINE 1424
  lda   trig3_f				        ; if trig3 pt set must have spark o/p c        ;* trig3_f = 0
  beq   check8                                                                     ;* GO TO LINE 1430                                                                      
  brclr REUSE_LED18,outputpins,ck7err
  brclr REUSE_LED18_2,outputpins,ck7err
...
check9:                                                              ;* LINE 1468
  brclr REUSE_FIDLE,outputpins,check10                               ;* REUSE_FIDLE = 0, GO TO LINE 1475
...
; count how many ignition types and if more than one give an error
  ...
  clra
  brclr MSNS,personality,check10a                                   ;* MSNS = 0, GO TO LINE 1483
  
  inca
	
check10a:                                                           ;* LINE 1483
  brclr MSNEON,personality,check10b                                 ;* MSNEON = 0, GO TO LINE 1487 
  inca
...
check10c:
  brclr EDIS,personality,check10d                                   ;* Edis = 0, GO TO LINE 1495 
  inca
	
check10d:
  brclr TFI,personality,check10e                                    ;* tfi = 0, GO TO LINE 1499
   inca
	
check10e:
  brclr HEI7,personality,check10f                                   ;*  HE17 = 0, GO TO LINE 1503
  inca
...
Prime_Not_Done:
  ...
  beq  PrimeTable_P            ; Prime table                                  ;* GO TO LINE 1596
...
PrimeTable_P:                   ; Interpolate from CLT, same curve as cranking PW.   ;* LINE 1596
  jsr   crankingModePrime
  lda   tmp6
  bra   prime                                                                        ;* GO TO LINE 1613

NotPrimed:					   ; If were here we must be priming late                ;* LINE 1601
  lda   secl
  cmp   #02T				   ; Have we been powered up for 2 secs?
  bhs   PrimeNow			   ; Yes so fire prime pulse now                         ;* GO TO LINE 1571
  jmp   Prime_Checked	       ; No so go back to main loop                          ;* GO TO LINE 5987
...
prime:                                                                               ;* LINE 1613
  ...
  bset  sched1,squirt
  bset  inj1,squirt
  brclr CrankingPW2,feature1,CalcRunningParameters ; can skip prime on second channel  ;* CrankingPW2 = 0, GO TO LINE 1720 
  sta   pw2
  clr   pwrun2
  bset  sched2,squirt
  bset  inj2,squirt
...
CalcRunningParameters:                                                                                   ;* LINE 1720
  clrh
  ...
  beq    OneShot_Bar            ; No so one shot baro only!                                              ;* GO TO LINE 1754
...
OneShot_Bar:                                                                                            ;* LINE 1754
  bset  OneShotBArro,EnhancedBits2
  bra   DoBaroCorr                                                                                      ;* GO TO LINE 1762

ConsBar:                                ; MAP connected to X7, so using constant BARO COR
  lda   o2_fpadc
  sta   baro

DoBaroCorr:                                                                                    ;* LINE 1762
  ldx   baro
; check if within sensible range

  cpx   BarroHi_f                 ;* BarroHi_f = 116T
  blo   Baro_Lo_Check
  ldx   BarroHi_f
  bra   Do_Baro
  
Baro_Lo_Check:
  cpx   BarroLow_f                ;* BarroLow_f = 67T
  bhi   Do_Baro
  ldx   BarroLow_f
  
Do_Baro:
  stx   baro                    ; re-store whatever it ended up as
  lda   config11_f1                                                ;* config11_f = %00110001
  and   #$03                    ; What MAP sensor?                 ;*          and %00000011
  beq   do_baro4115                                                ;*     Accu A = %00000001  
...
; do_baro_4250:
  ...
  sta   Pambient
  lda   BAROFAC4250,x
  bra   DoneBaroCorr                                               ;* go to line 1805
...
; now convert map ADC count into internal kpa
  ...
  lda   config11_f1                                    ;* config11_f = %00110001
  and   #$03                                           ;*          and %00000011
  cbeqa #1T,do_kpa4250                                 ;*     Accu A = %00000001, GO TO LINE 1820
  cbeqa #2T,do_kpa6300
  cbeqa #3T,do_kpa6400
  bra   do_kpa4115

do_kpa4250:                                            ;* LINE 1820
  lda   KPAFACTOR4250,x
  bra   Donekpa                                        ;* GO TO LINE 1838
...
Donekpa:                                                               ;* LINE 1838
  ...
  beq   Do_AirDens                                                     ;* GO TO LINE 1855
...
Do_AirDens:             ; Not using a Air correction within a MAF     ;* LINE 1855

;******** CHECK IF CORRECTING AIR DENSITY *****************

  lda   feature13_f                                                       ;* feature13_f = %00000000
  bit   #cltMAPb             ; Are we correcting the air density factor?  ;* cltMAPb = 0
  beq   NormAirDen           ; If no then do normal air density           ;* GO TO LINE 1945
...
; 50% re-written in 026i with aim of better odd-fire averaging
  ...
  brset  running,engine,dorpmCalc                                                      ;* GO TO LINE 1977
  ldhx   rpmph
  bne    dorpmCalc		 ; If zero then jump over calculation - prevent divide by zero
  jmp    rpmCalcZero     ; previous branches out of range                              ;* GO TO LINE 2080
...
; If odd-fire is set (bit zero of Config13), then average RPM values
  ...
  lda     config13_f1                                ;* config13_f1 = %00001010
  and     #$01                                       ;*           and %00000001                           
  beq     NO_ODD_FIRE                                ;* Accu A =      %00000000, GO TO LINE 2011
...
; need to divide period (tmp12,13,14) by 100 to obtain period time in 0.1ms
  ...
  lda     rpmk_f1+1
  sta     intacc1+3
  jsr     udvd32		       ; 32 / 16 divide                                    ;* GO TO LINE 13841  
  lda     intacc1+3		       ; get 8-bit RPM result
  bra     rpmCalcDone                                                              ;* GO TO LINE 2083
...
; Multiply rpmk x 100 then do 32/16 divide using 1us time
  ...
  mul                   ;* X:A<- (X)x(A)
  add    intacc1+2      ;* A<-(A)+(M)			; Add in carry from LSB.
...
; calc 'PW1' from table 1
  ...
  brclr  UseVE3,EnhancedBits,Do_VE1_4_Now	; Jump if aren't using VE table 3  ;* UseVE3 = 0, GO TO LINE 2120
  jmp    VE3_Table
  
Do_VE1_4_Now:                                                                  ;* LINE 2120
...
VE1_LOOKUP:				  ; ALWAYS page 1
  clrh
  clrx
  lda    feature9_f                                         ;* feature9_f = %00000000
  bit    #MassAirFlwb                                       ;* MassAirFlwb = 0
  beq    VE1_LOOKUP_PW1   ; Are we using a MAF on pin X7?   ;* GO TO LINE 2170
  lda    o2_fpadc         ; Using MAF thats on pin X7
  bra    VE1_STEP_1

VE1_LOOKUP_PW1:
  lda    config13_f1                                        ;* config13_f1 = %00001010
  bit    #c13_cs                                            ;* c13_cs (bit2) = 0
  bne    VE1_AN			 ; Using Alpha_n?                   
  lda    kpa			 ; SD, so use kpa for load
  bra    VE1_STEP_1                                         ;* GO TO LINE 2180

VE1_AN:                                                     
  lda    tps             ; Alpha_n

VE1_STEP_1:                                                 ;* LINE 2180
  sta    kpa_n
  ldhx   #KPARANGEVE_f1
  sthx   tmp1
  lda    #$0b			 ; 12x12
  ...
  lda    kpa_n
  sta    tmp4
  jsr    tableLookup                                               ;* GO TO LINE 13666
  lda    tmp1
  lda    tmp2
  mov    tmp5,tmp8		 ; Index
  mov    tmp1,tmp9		 ; X1
...
VE1_STEP_3:
  ...
  VE1X
  sta    tmp18
  jsr    VE_STEP_4                                                         ;* GO TO LINE 6435
  mov    tmp6,vecurr
  jmp    No_VE3

VE3_Table:
  lda    VE3Timer                                                          ;* VE3Timer = 0
  beq    VE3_LOOKUP                                                        ;* GO TO LINE 2243
  jmp    Do_VE1_4_Now
...
VE3_LOOKUP:				; ALWAYS page 3                                    ;* LINE 2243
  clrh
  clrx
  lda    feature9_f                                                        ;* feature9_f = %00000000
  bit    #MassAirFlwb                                                      ;* MassAirFlwb = 0
  beq    VE3_LOOKUP_PW1 ; Are we using a MAF on pin X7?                    ;* GO TO LINE 2252
  lda    o2_fpadc       ; Using MAF thats on pin X7
  bra    VE3_STEP_1

VE3_LOOKUP_PW1:                                                            ;* LINE 2252
  lda    config13_f1                                                       ;* config13_f1 = %00001010
  bit    #c13_cs                                                           ;* c13_cs (bit2) = 0
  bne    VE3_AN			; if alpha-n                                       ;* GO TO LINE 2259
  lda    kpa			; SD, so use kpa for load
  bra    VE3_STEP_1
...
WUE_CALC:
  brclr  crank,engine,WUE1          ; already out of crank mode    ;* GO TO LINE 2362 
  bclr   crank,engine
  clr    TCCycles
  clr    TCAccel
  bset   startw,engine
...
WUE1:                                                              ;* LINE 2362
  brset  warmup,engine,WUE1a        ; only run code if in warmup   ;* GO TO LINE 2367
...
;Warm_NotDone:
  ...
  lda    coolant
  sta    liX
  jsr    LinInterp                                                      ;* GO TO LINE 13708
  lda    tmp6
  sta    tmp31         ; save result
  bra    WUE2          ; only end warmup when reached temp              ;* GOL TO LINE 2410
;  cmp    #100T
;  bhi    WUE2
...
Warm_Done_Now:
  ...
  bclr   startw,engine
  bclr   warmup,engine
  brset  REUSE_LED18,outputpins,jWUE_DONE                             ;* REUSE_LED18 = 0 
  brset  REUSE_LED18_2,outputpins,jWUE_DONE	; Using led as output 4   ;* REUSE_LED18_2 = 0
  bclr   wled,portc		                    ; not when crank sim or if LED re-used as IRQ indicator
  
jWUE_DONE:
  jmp    WUE_DONE                                                        ;* GO TO LINE 2490
  
WUE2:                                                                    ;* LINE 2410
  brset  REUSE_LED18,outputpins,WUE2_ledskip                             ;* REUSE_LED18 = 0 
  brset  REUSE_LED18_2,outputpins,WUE2_ledskip ; Using led as output 4   ;* REUSE_LED18_2 = 0
  bset   wled,portc
...
; goes to the normal ASE decay type of ASE
  ...
  bit    #ASEHoldb                               ; Are we holding the ASE at a fixed percentage?  ;* ASEHoldb = 0
  beq    NormASE_Count                                                                            ;* GO TO LINE 2438
...
NormASE_Count:                                                                           ;* LINE 2438
  lda    ASEcount
  cmp    awc_f1			           ; Check if ASE period has expired.
  bhs    WUE3                                                                            ;* GO TO LINE 2487
;  bra    Table_ASEStuff
...
Table_ASEStuff:
  ...
  jsr    tableLookup	; This finds the bins when the temperatures are set                 ;* GO TO LINE 13666
  clrh
  ldx    tmp5
  lda    ASEVTbl_f,x
  sta    liY2
  ...
  sta    liY1
  mov    coolant,liX
  jsr    LinInterp		; tmp6 contains amount of ase enrichment in percent for this temperature       ;* GO TO LINE 13708
  clr    liX1
  lda    AWC_f1
  sta    liX2
  ...
  clr    liX
  lda    feature10_f5                                                    ;* feature10_f5 = %00000000
  bit    #ASEHoldb                                                       ;* ASEHoldb = 0
  beq    NormASE_Interp                                                  ;* GO TO LINE 2473
...
TAE_CALC:
  lda    feature4_f                                                                    ;* feature4_f = %00000000 
  bit    #KpaDotSetb                                                                   ;* KpaDotSetb = 0
  beq    tps_dotty                                                                     ;* GO TO LINE 2570
...
tps_dotty:                                                                ;* LINE 2570
  ...
  lda    tmp1
  cmp    tmp2
  bhi    AE_CHK                                                          ;* GO TO LINE 2589
  beq    Dec_Accel                                                       ;* GO TO LINE 2582
  jmp    TDE                                                             ;* GO TO LINE 2910

Dec_Accel:                            ; Throttle steady but lets check if we have just triggered decel  ;* LINE 2582
  brclr  TPSDEN,ENGINE,AE_CHK         ; If we are not decel then check accel threshold
  jmp    TDE			              ; We are deceling so check decel timers, etc  ;* GO TO LINE 2910
...
AE_CHK:
  ...
  sub    tmp2
  sta    tmp31
  lda    feature4_f		              ; Are we in TPS or KPA mode?                 ;* feature4_f = %00000000
  bit    #KpaDotSetb                                                               ;* KpaDotSetb = 0
  beq    tps_ThreshCheck                                                           ;* GO TO LINE 2603
  lda    tmp31
...
tps_ThreshCheck:                                                                  ;* LINE 2603
  lda    tmp31
  cmp    TPSthresh_f1		          ; Are we above the Accel threshold for TPS? ;* TPSthresh_f1 = 3T
  bhs    AE_SET
  brclr  TPSAEN,ENGINE,acc_done_led   ; If we are not in AE mode then jump to end
  jmp    TAE_CHK_TIME
...
; Add in accel enrichment
  ...
  lda    feature9_f                                                                    ;* feature9_f = %00000000
  bit    #RpmAEBased             ; This is a basic AE system that uses                 ;* RpmAEBased = 0
  beq    NormalBased_AE          ; RPM rather than dot                                 ;* GO TO LINE 2637
  ...
  sta    tmp4
  sta    tmp10
  jsr    tableLookup             ; Find the rpm bins we are going to use               ;* GO TO LINE 13666
  clrh
  ldx    tmp5
  lda    RPMAQ_f2,x
  sta    liY2
...
NormalBased_AE:                                                                       ;* LINE 2637
  lda    feature4_f                                                                   ;* feature4_f = %00000000
  bit    #KpaDotSetb                                                                  ;* KpaDotSetb = 0
  beq    tps_FirstElem                                                                ;* GO TO LINE 2644
  lda    MAPAQ_f	    ; start out using first element - will determine actual next time around
  bra    Store_TEA1

tps_FirstElem:                                                                        ;* LINE 2644
...
RPMBackAE:
  ...
  bset   TPSAEN,ENGINE
  bclr   TPSDEN,ENGINE
  brset  REUSE_LED19,outputpins,acc_done_led	; LED already used in NEON as coilb  ;* REUSE_LED19 = 1, GO TO LINE 2658
  bset   aled,portc
  
acc_done_led:                                                                        ;* LINE 2658
  jmp    TAE_DONE                                                                    ;* GO TO LINE 2974
		
TAE_CHK_JMP:
  jmp    TAE_CHK_TIME                                                                ;* GO TO LINE 2800
...
Decay_AE_Aw:
  ...
  jsr    lininterp		    ; Go and work out the value                                  ;* GO TO LINE 13708 
  lda    tmp6
...
no_ego_w_chk:                                                                         ;* LINE 3017
  ...
  lda    EGOdelta_f		                   ; No delta means open loop.                ;* EGOdelta_f = 0 for open loop, > 0  for closed loop 
  beq    SkipO2JMP                                                                    ; GO TO LINE 3041
  lda    feature3_f                                                                        ;* feature3_f = %10000001
  bit    #KPaTpsOpenb                                                                      ;* KPaTpsOpenb = 1 (KPA) 
  beq    throttle_check                                                                    
;  brclr  KPaTpsOpen,feature3,throttle_check ; 0 = throttle do throttle check  1 = KPa
  lda    kpaO2_f			                 ; In KPa mode so is it higher than setpoint?  ;* kpaO2_f = 80T 
  beq    SETAFR_UP			                 ; If its zero dont check it as no open loop
  cmp    kpa
  blo    SKIP_ALL_O2		                 ; If it is dont check O2   ;* If (A)<(M) go to line 3044 
  
No_KPA_Check:
  bra    SETAFR_UP                                                                   ;* GO TO LINE 3050
...
SkipO2JMP:
  bra    SKIPO2A                                                                      ;* GO TO LINE 3135

SKIP_ALL_O2:			; Skip both O2 checks                                         ;* LINE 3042
  lda    #100T
  sta    EGOCorr
  sta    EgoCorr2
  jmp    EGOALL_DONE                                                                  ;* GO TO LINE 3189

SETAFR_UP:                                                  ;* LINE 3050
  lda   feature3_f                                          ;* feature3_f = %10000001
  bit   #TargetAFRb                                         ;* TargetAFRb = 1
  bne   CheckVE1                                            ;* GO TO LINE 3063
...
;  brset   TargetAFR3,feature6,CheckVE3
  jmp   SETAFRNORMAL                                        ;* GO TO LINE 3070

CheckVE1:                                                   ;* LINE 6063
  brset useVE3,EnhancedBits,Re_CheckTarg  ; Are we are using VE3 at present if so check if targets needed?  ;* useVE3 = 0 
  bra   SETAFRTABLE                                                                                         ;* GO TO LINE3074
...
SETAFRTABLE:				             ; AFR Table value is already in afrTarget                        ;* LINE 3074
  lda    ego
...
; Check if we are over the O2 operating temp
  ...
  lda    coolant
  cmp    EGOtemp_f                                                          ;* EGOtemp_f = 200T (160 gegrees F)
  blo    SkipO2A
  bra    Do_The_Ego
...
Do_The_Ego:
  ...
  cmp    EGOrpm_f                                                           ;* EGOrpm_f = 50T for no closed loop, 13T for closed loop
  blo    SkipO2A                                                            ;* id RPM < set point go to line 3136, otherwise fall through

; Check if rich/lean

  clr    EGOcount
  lda    kpa			; See if we need to load in a new Ego Limit
  cmp    EgoLimitKPa_f                                                      ;* EgoLimitKPa_f = 255T
  bhi    New_EgoLim                                                         ;* if (A) > (M) branch so no branch
  lda    EGOlimit_f		; Original Ego Limit                                ;* EGOlimit_f = 15T (default)
  sta    tmp31
  bra    EgoLim_Done                                                        ;* GO TO LINE 3124
...
EgoLim_Done:
  lda    config13_f1	; Check if Narrow-band (bit=0) or DIY-WB (bit=1)        ;* config13_f1 = %00001010
  bit    #c13_o2		; Use BIT instead of brset because outside of zero-page ;* c13_o2 = 1 
  bne    WBO2TYPE		; Branch if the bit is set                              ;* GO TO LINE 3138
...
O2_IS_RICH:
  lda    #100T
  sub    tmp31		   ;* A<-(A)-(M) ; Generate the lower limit rail point
  sta    tmp2
  lda    EGOcorr
  sub    EGOdelta_f	   ;* A<-(A)-(M) ; remove the amount required per step.              ;* EGOdelta_f = 1T (default)
  sta    tmp1
  cmp    tmp2
...
O2_IS_LEAN:
  lda    #100T
  add    tmp31			;* A<-(A)+(M) ; Generate the upper limit rail point
  sta    tmp2
  lda    EGOcorr
  add    EGOdelta_f     ;* A<-(A)+(M)                                                        ;* EGOdelta_f = 1T (default)
  sta    tmp1
  cmp    tmp2
...
EGO_DONE:
  lda    DTmode_f		; check if DT in use                                  ;* DTmode_f = %01100001 
  bit    #alt_i2t2                                                            ;* alt_i2t2 = 0
  beq    No_DT_SecondO2                                                       ;* GO TO LINE 3185
  lda    feature12_f2
  bit    #SecondO2b
  bne    DO_Second_Ego
;  brset  SecondO2,feature4,DO_Second_Ego	; Do we have a second O2 sensor?

No_DT_SecondO2:                                                              ;* LINE 3185
  lda    Egocorr
  sta    Egocorr2
  
EGOALL_DONE:
  jmp    EGO_2Done                                                           ;* GO TO LINE 3294
...
WARMACCEL_COMP:
  ...
  lda    DTmode_f		    ; Must check the INJ1 GammaE bit,              ;* DTmode_f = %01100001               
  bit    #alt_i1ge		    ; if 0 then set it to 100T to remove GammaE.   ;* alt_i1ge = 1
  bne    ld_ve_1                                                           ;* GO TO LINE 3355
  mov    #100T,GammaE
  bra    ld_ve_1Done

ld_ve_1:
  mov    tmp10,GammaE                                                      ;* LINE 3351

ld_ve_1Done:
  mov    EGOcorr,tmp10		                     ; closed-loop correction percent into tmp10
  clr    tmp11			                         ; remainder is zero
  brset  hybridAlphaN,feature1,skip_loadcontcomp ; if hybrid then skip AN bypass    ;* hybridAlphaN = 0
  lda    config13_f1                                                                ;* config13_f1 = %00001010
  bit    #c13_cs                                                                    ;* c13_cs = 0
  beq    MAFCheck                ; No Alpha but are we using MAF?                   ;* GO TO LINE 3367
  bra    LoadContribDone

MAFCheck:                                                                           ;* LINE 3367
  lda    feature9_f                                                                 ;* feature9_f = %00000000
  bit    #MassAirFlwb                                                               ;* MassAirFlwb = 0             
  beq    skip_loadcontcomp      ; Are we using a MAF on pin X7?                     ;* GO TO LINE 3373
  bra    LoadContribDone

skip_loadcontcomp:                                                                  ;* LINE 3373
...
MBFF:
  ...
  jsr    LinInterp		; injector open time in tmp6                         ;* GO TO LINE 13708
...
***************************************************************************
  ...
  and    #$03                                              ;* and           %00000011
  cmp    #$02			; Are we using Turbo Map sensor?   ;* Accu A =      %00000001
  blo    CALC_FINAL     ; skip if 0 or 1                   ;* GO TO LINE 3500 
...
CALC_FINAL:
  ...
  add    tmp6			;* A<-(A)+(M) ; from batt correction
  bcs    MaxPulse
  add    TPSACCEL       ;* A<-(A)+(M)
  bcs    MaxPulse
  bra    PW_Done
...
PW_Done:
  sta    tmp20
  lda    feature5_f                                                  ;* feature5_f = %00000000
  bit    #stagedeither                                               ;* stagedeither = 0
  beq    Calc_Final1Done                                             ;* GO TO LINE 3519 
  jsr    CALC_STAGED_PW   ; Do the Staged PW calculations if set
  
Calc_Final1Done:                                                     ;* LINE 3519
...
****************************************************************************
  ...
  bit    #alt_i2t2                                                           ;* alt_i2t2 = 0
  bne    do_dt
  jmp    both_table1		    ; if (inj2=t2) =0 then single table          ;* GO TO LINE 3814 
...
PW2_calc:
  clr    tmp2
  lda    DTmode_f                                                                  ;* DTmode_f = %01100001
  bit    #alt_i2t2		   ; if inj2 is not driven from table1 then skip           ;* alt_i2t2 = 0
  bne    pw2_table2
...
; Do all the rpm related stuff here.
  ...
  brclr  ShiftLight,feature2,ShiftLightDone                                           ;* ShiftLight = 0, go to line 3866   
...
shiftLightDone:                                                                        ;* LINE 3866

; Hard Cut Rev and Launch checks

  bclr    sparkCut,RevLimBits		        ; Reset spark cut bit
  brclr   LaunchControl,feature2,LaunchDone ; Is Launch selected?    ;* LaunchControl = 0, GO TO LINE 3923
...
LaunchDone:                                                                          ;* LINE 3923

; ***Over Boost Protection**********************************

  lda    Over_B_P_f			  ; load in Over boost KPa value                          ;* Over_B_P_f = 0
  cmp    #101T
  blo    BoostP_Done	      ; If set to less than 100KPa then no boost protection   ;* GO TO LINE 3946
...
checkHighLimit:
  ...
  cmp    rpm
  bhs    checkRevsOk	; We have not hit any                          ;* GO TO LINE 4010 
						; Rev limits
...
SparkFuel_LC:
  lda    feature5_f             ; Launch fuel cut?                                 ;* feature5_f = %00000000
  bit    #FuelSparkLCb
  bne    cutChannels
  bra    checkRevsOk
...
checkRevsOk:                                                                      ;* LINE 4010
  brclr  Traction,EnhancedBits2,No_Traction_On                                    ;* Traction = 0, GO TO LINE 4018
...
; Add in the NOS and Staged PW's here
  ...
  bne    Add_to_PWCALC                                                            ;* GO TO LINE 4042
...
Add_to_PWCALC:                                                                   ;* LINE 4042
  lda    DTmode_f			; check if DT in use                                 ;* DTmode_f = %01100001
  bit    #alt_i2t2                                                               ;* alt_i2t2 = 0
  beq    Do_Nos_PW1			; i2t2=1                                             ;* GO TO LINE 4051
  lda    feature4_f
  bit    #DtNosb
  bne    Dont_Nos_PW1
;  brset  DtNos,feature4,Dont_Nos_PW1

Do_Nos_PW1:                                                                      ;* LINE 4051
  lda    tmp1
  add    NosPW				; Add Nos PW to pw1                                  ;* NosPW = 0
  sta    tmp1
  
Dont_Nos_PW1:
  brclr  REStaging,EnhancedBits,No_Staging      ; Staging not running so dont add PW Staging  ;* REStaging = 0, GO TO LINE 4061
  lda    pw_staged
  sta    tmp1
...
No_Staging:					                    ; Staging not running
  ...
  bra    Staging_Done_PW                                                           ;* GO TO LINE 4076
...
Staging_Done_PW:                                                                   ; LINE 4076
  lda    DTmode_f			; check if DT in use                                   ;* DTmode_f = %01100001
  bit    #alt_i2t2                                                                 ;* alt_i2t2 = 0
  beq    Nos_PWCal2			; i2t2=1                                               ;* GO TO LINE 4048
  lda    feature4_f
  bit    #DtNosb
  beq    Calc_PWs_DONE      ; In DT mode so do we add NosPW to PW2?
  
Nos_PWCal2:                                                                        ;*LINE 4048
  lda    tmp2
  add    NosPW				; Add Nos PW to pwcalc2                                ;* NosPW = 00T
  sta    tmp2
...
spark_lookup:
  ...
  lda    page
  cmp    #3
  bne    fixed_fl                                              ;* branch if not on page 3
  lda    FixedAngle_r                                          ;* FixedAngle_r = 0 for table, 56T for 10 degrees)
  bra    fxr_c
...
fxr_c:
  cmp    #$03
  blo    NOT_FIXED	    ; Added this as earlier MT didnt send a perfect 00T for -10 (use map) ;* GO TO LINE 4130
;; sta    SparkAngle	 ; else use this fixed advance
  jmp    CALC_DELAY
  
NOT_FIXED:                                                            ;* LINE 4130
  brclr  LaunchOn,RevLimBits,Not_LC_in                                ;* LaunchOn = 0, go to line 4142
...
Not_LC_in:                                                           ;* LINE 4142
  ...
  cmp	 #$03
  blo	 use_spark_table                                             ;* go to line 4191
...
use_spark_table:
  brclr  RevLimSoft,RevLimBits,STTABLELOOKUP                    ;*RevLimSoft = 0, GO TO LINE 4228
...
; variable is MAP, not kpa
  ...
  lda    feature9_f                                                            ;* feature9_f = %00000000
  bit    #MassAirFlwb                                                          ;* MassAirFlwb = 0
  beq    SD_ALPHa_N      ; Are we using a MAF on pin X7?  ;* go to line 4241   ;* GO TO LINE 4242
  lda    o2_fpadc        ; Using MAF thats on pin X7
  sta    kpa_n
  bra    ST_STEP_1

SD_ALPHa_N:
  lda    config13_f1	; Check if in speed-density or Aplha-N mode                 ;* config13_f1 = %00001010             
  bit    #$04			; Use BIT instead of brset because outside of zero-page     ;* bit2 = 0
  beq    Kpa_n_Kpa		; Branch if the bit is clear                                ;* go to line 4250
  lda    tps            ; Alpha_N Mode
  sta    kpa_n			; Added so as KPa can be used elsewhere in code
  bra    ST_STEP_1

Kpa_n_Kpa:              ; Speed Den Mode                                            ;* LINE 4250
  lda    kpa
...
ST2_STEP_1:
  lda    feature5_f                         ; Are we using SparkTable2?   ;* feature5_f = %00000000
  bit    #SparkTable2b                                                    ;* SparkTable2b = 0 
  beq    LookUp_Done                                                      ;* GO TO LINE 4360
...
LookUp_Done:                                                       ;* LINE 4360
  jmp    LookUp_Finished                                           ;* GO TO LINE 4432
...
Not_ST1:
  lda    page
  cmp    #3           
  bne    trim_fl         ;* branch if page is not = 3
  lda    TrimAngle_r
  bra    trim_c
...
trim_c: 
  ...
  add    tmp6			;* A<-(A)+(M) ; add lookup angle
...
TRIM_DONE:
  ...
  bra    TRIM_DONE2                                                            ;* go to line 4470
...
Store_Spark_Ang:
  brclr  LaunchOn,RevLimBits,store_spark2                                  ;* LaunchOn = 0, GO TO LINE 4493
...
this_cyl:
  ...
  sub    SparkAngle               ;* A<-(A)-(M)
  add    #28T                     ;* A<-(A)+(M)
  sta    DelayAngle
  bra    CheckSoftLimit                                                         ;* go to line 4596
...
CheckSoftLimit:
  ...
  brclr  LaunchControl,feature2,Magnus_revlimiters  ; Is Launch selected?                      ;* LaunchControl = 0, GO TO LINE 4620
...
Magnus_revlimiters:
  ...
  beq    SRevLimOnDone		              ; skip if zero                      ;* go to line 4643
  cmp    rpm
...
CheckOutputs:
  ...
  cmp    #06T
  beq    CLT1Source
  bra    Not_Temps1                                                                ;* go to line 4699
...
Not_Temps1:                                                          ;* LINE 4699
  ldx    Out1Source	    ; Get source                                 ;* Out1Source = 0
  beq    Out1Done	    ; No source = no check                       ;* GO TO LINE 4752
  lda    secl,x		    ; Get data
  sta    tmp31
  cmp    Out1Lim		; Check limit
  bhi    Out1On		    ; Above limit, set output
...
Out1Done:
  ...
  cmp    #06T
  beq    CLT2Source
  bra    Not_Temps2                                                   ;* go to line 4794
...
Not_Temps2:
  ...
  beq    Out2Done	    ; No source = no check                         ;* go to line 4852
  lda    secl,x		    ; Get data
  sta    tmp31
  cmp    Out2Lim		; Check limit
  bhi    Out2On		    ; Above limit, set output
...
; for those using WATER INJECTION on X2
  ...
  brset  REUSE_LED18,outputpins,Out4Done	; being used as IRQ or COIL C       ;* REUSE_LED18 = 0, go to line 4999
...
***************************************************************************
  ...
  brset  X2_FAN,outputpins,DO_FAN_Check	    ; Are we using X2 as fan control?     ;* X2_FAN = 1, go to line 5016
...
No_FAN_Porta:
  brclr  LED18_FAN,outputpins,FAN_DONE	  ; Are we using LED18?               ;* LED18_FAN = 0, go to line 5042
  bclr   wled,portc
  bra    FAN_DONE
...
Over_Run:
  ...
  beq    Over_Run_Done                                                         ;* go to line 5089
...
***************************************************************************
  ...
  lda    feature3_f                                                         ;* feature3_f = %10000001
  bit    #WaterInjb                                                         ;* WaterInjb = 0
  beq    Water_Inj_Done                                                     ;* go to line 5130
...
*****************************************************************************
  ...
  beq    retard_endJmp                                                      ;* go to line 5204
...
retard_endJmp:
  jmp    retard_end		; If Coolant advance running dont  ;* GO TO LINE 5262
					    ; check IAT retard
...
;  brset  PWMidle,feature2,idlePWM
  lda    feature13_f                                                        ;* feature13_f = %00001010
  bit    #pwmidleb                                                          ;* pwmidleb = 1
  bne    idlePWM                                                            ;* go to line 5315
...
idlePWM:
  brset  istartbit,EnhancedBits6,Crank_PWM	; loop to stabilize on startup     ;* IF istartbit = 1, GO TO LINE 5360 
  brset  crank,engine,Crank_PWM		        ; open AIC for cranking            ;* IF crank = 1, GO TO LINE 5360
  brclr  running,engine,Idle_doneJMP1	    ; no PWM adjust when not running   ;* IF running = 0, GO TO LINE 5357
...
idle_openloop:
  ...
  blo    idle_loopcold                                                           ;* GO TO LINE 5341
  lda    feature13_f             ; If we are not using closed loop then clear DC ;* feature13_f = %00001010 
  bit    #idle_clb                                                               ;* idle_clb = 0
  beq    clrNskip                                                                ;* go to line 5336
  bra    idle_closedloop

clrNskip:                                                                           ;* LINE 5336
  lda    idle_dc_hi              ; Store hot DC in Idle DC                          ;* idle_dc_hi = 0T
  sta    idleDC                  ; Added for setting idle DC as if ignition turned
  bra    idle_DoneJMP1		     ; on when engine hot                            ;* GO TO LINE 5357

idle_loopcold:                                                             ;* LINE 5341
; determine duty cycle by linear interpolation
  ...
  sta    liX2
  lda    idle_dc_lo                                                        ;* idle_dc_lo = 50T
  sta    liY1
  lda    idle_dc_hi                                                        ;* idle_dc_hi = 0T
  sta    liy2		             ; rmd upper duty limit
  lda    coolant
  sta    liX
  jsr    lininterp                                                         ;* GO TO LINE 13708
  mov    liY,idleDC
  bra	 idle_closedloop                                                   ;* GO TO LINE 5396

Idle_doneJMP1:                                                             ;* LINE 5357
  jmp	 Idle_done                                                         ;* GO TO LINE 5553

Crank_PWM:                                                                 ; LINE 5360
  brset	 istartbit,EnhancedBits6,start_delay                               ;* IF istartbit = 1, GO TO LINE 5375
  lda    fastIdletemp_f		                  ; interpolate delay to 0 at  ;* fastIdletemp_f = 105T (65 deg F)
  sta    tmp1			                      ; slow idle temp             
  lda    slowIdleTemp_f                                                    ;* slowIdletemp_f = 234T (194 deg F)
  sta    tmp2
  lda	 idlestartclk_f                                                    ;* idlestartclk_f = 10t
  sta    tmp3
  clr    tmp4
  mov    coolant,tmp5
  jsr    lininterp                                                         ;* GO TO LINE 13708
...
start_delay:                                                               ;* LINE 5375
  lda    idlecrankdc_f                                                     ;* idlecrankdc_f = 50T
  sta	 idleDC
  sta	 idlelastdc
  ...
  lda	 idlestartclk_f		                ; load start delay clock again  ;* idlestartclk_f = 10t
  cmp	 idleDelayClock_f                                                   ;* idleDelayClock_f = 1T
  bhi	 longer_delay
  lda	 idleDelayClock_f                                                   ;* idleDelayClock_f = 1T
...
idle_closedloop:                                                             ;* LINE 5396
;  brclridle_cl,feature7,Idle_doneJMP1
  lda    feature13_f                                                         ;* feature13_f = %00001010 
  bit    #idle_clb                                                           ;* idle_clb = 0
  beq    idle_DoneJMP1                                                       ;* GO TO LINE 5357
  lda    tps
...
Knock_Detection:
  ...
  beq    End_KnockJmp                    ; knock not enabled                      ;* GO TO LINE 5656
  lda    feature8_f
  bit    #spkfopb
...
End_KnockJmp:                                                                            ;* LINE 5656
  jmp    End_Knock                                                                       ;* GO TO LINE  5709
...
********************************************************************************
  ...
  beq   Traction_DoneJMP                                                          ;* go to line 5818
...
; (store allowed slip in tmp31)
  ...
  sta    liX
  jsr    LinInterp		                    ; Go and find out what slip is allowed at current speed     ;* GO TO LINE 13708
  sta    tmp31
...
Traction_DoneJMP:                                                                          ;* LINE 5818
  jmp    Traction_Done                                                                     ;* GO TO LINE 5976
...
; SubSectionLoop:
  ...
  jmp    NotPrimed                          ;* IF NOT GO TO LINE 1601 

Prime_Checked:                             ;* LINE 5987
  brclr  BoostControl,feature2,no_boost    ;* BoostControl = 0, go to line 5991
  jsr    CalcBoostDC

no_boost:                                  ;* LINE 5991
  brclr  Nitrous,feature1,no_nitrous       ;* Nitrous = 0, GO TO LINE 5995
  jsr    EnableN2O

no_nitrous:                                ;* LINE 5995
  lda    feature6_f                        ;* feature6_f = %00000000 
  bit    #VETable3b                        ;* VETable3b = 0
  beq    No_VE_Table_3                     ;* GO TO LINE 6001
  jsr    Check_VE3_Table

No_VE_Table_3:                              ;* LINE 6001
  lda    feature3_f                         ;* feature3_f = %10000001
  bit    #TargetAFRb                        ;* Target AFRb = 1
  beq    No_AFRTar_VE1                      ; Are we using the target afrs for table 1?  
  jsr    AFR1_Targets                       ; Get Target AFR from table 1 for VE 1   ;* GO TO LINE 14310
  
NO_AfrTar_VE1:
  lda    feature6_f                         ;* feature6_f = %00000000
  bit    #TargetAFR3b                       ;* TargetAFR3b = 0
  beq    No_AfrTar_VE3                      ;* go to line 6013
  jsr    AFR2_Targets                       ; Are we using the target afrs for table 3?
            		                        ; Get Target AFR from table 2 for VE 3
No_AfrTar_VE3:
  brset  running,engine,nospkoff            ; skip next check if not running then make sure all spark outputs are OFF  ;* GO TO LINE 6018
                                            ; this is a bandaid, but better safe than sorry
  jsr    turnallsparkoff                    ; subroutine to stop them all                      ;* GO TO LINE 7074
  
nospkoff:
  lda    personality_f                      ;* personality_f = 1
  beq    No_misc_Spark                      
  jsr    misc_spark                         ; dwell and other bits   ;* GO TO LINE 7091
...
hei7_spd:  ;moved from Sparktime - set/clr HEI7 output
  brclr  HEI7,personality,dwellornot                                             ;* HE17 = 0, GO TO LINE 7124
...
; first lookup battery correction factor from above table
  ...
  lda    batt
  sta    liX
  jsr    LinInterp         ; result in tmp6                               ;* GO TO LINE 13708
  brset  crank,engine,crankdwell                                          ;* IF CRANKING GO TO LINE 7148
  lda    dwellrun_f                                                       
  bra    dwell_corr                                                       ;* IF NOT CRANKING GO TO LINE 7151
  
crankdwell:                                                               ;* LINE 7148
  lda    dwellcrank_f
  
dwell_corr:                                                               ;* LINE 7151
  ldx     mp6
  mul				;* X:A<-(X)x(A) ; result in x:a
...
; somehow I'd got the sense wrong.
  ...
  sta    dwelltmpLac           ; ddt
  lda    dwelltmpHp
  sbc    dwelltmpH             ;* A<-(A)-(M)-(C) 
  sta    dwelltmpHac
  lda    dwelltmpXp
  sbc    dwelltmpX             ;* A<-(A)-(M)-(C)
  sta    dwelltmpXac
...
; dwell control for 1,2,3,4, rotary2 outputs
  ...
  jmp    true_dwell_calc                                               ;* GO TO LINE 7600
...
; add off the accel factor (-ve)    dt = dt-1 + (-ddt) (predicted next period)
  ...
  lda    dwelltmpL                    ; dt-1
  add    dwelltmpLac   ;* A<-(A)+(M) ; ac
  sta    dwelltmpLp                   ; used if wspk
  sta    dwelltmpLop                  ; output value
  lda    dwelltmpH
  adc    dwelltmpHac  ;* A<-(A)+(M)+(C)
  sta    dwelltmpHp
  sta    dwelltmpHop
  lda    dwelltmpX
  adc    dwelltmpXac ;* A<-(A)+(M)+(C)
  sta    dwelltmpXp
  sta    dwelltmpXop

    SubDwell        ; subtract dwell  ;* MACRO AT LINE 6992                                                 
	
  brset  rotary2,EnhancedBits5,cd0                                                     ;* rotary2 = 0
  brset  wspk,EnhancedBits4,cd1_start                                                  ;* wspk = 1, GO TO LINE 7654
...
; we are either have one spark output or rotary. We dwell across a single period only.
    ...
    DwellDiv                    ; convert microseconds to 0.1ms units           ;* MACRO AT LINE 7031 
...
; check to see if value we _would_ store in dwelldelay1 is negative ie. top bit set
  ...
  bmi    cd_1rail   ; if pos ok, else set to zero  ?? is BMI correct?
    ...
    DwellDiv                                                                           ;* MACRO AT LINE 7031 
	
  bra    cd_1store                                                                     ;* GO TO LINE 7667
...
;,dd2 = dd1 +dt-1 + ac + ac
  ...
  sta    dwelltmpLop
  lda    dwelltmpHp
  adc    dwelltmpH    ;* A<-(A)+(M)+(C)
  sta    dwelltmpHp
  sta    dwelltmpHop
  lda    dwelltmpXp
  adc    dwelltmpX    ;* A<-(A)+(M)+(C) 
  sta    dwelltmpXp
  sta    dwelltmpXop
  lda    dwelltmpLop
  add    dwelltmpLac  ;* A<-(A)+(M)
  sta    dwelltmpLop
  lda    dwelltmpHop
  adc    dwelltmpHac  ;* A<-(A)+(M)+(C)
  sta    dwelltmpHop
  ...
  brclr  REUSE_LED18,outputpins,cd2_done    ; want 1 } spark c      ;* REUSE_LED18 = 0, go to line 7702  
...
;  sthx   dwelldelay3
;  sthx   dwelldelay4
 jmp    really_done_dwell                                            ;* go to line 7962
...
really_done_dwell:                                                                   ;* LINE 7962

; finally we've calculated everything we need to for dwell and saved it away - phew!

  brset  rotary2,EnhancedBits5,rotary_split  ; are we doing rotary split             ;* rotary2 = 0
...
nils:					          ; normal sparking non inverted           ;* LINE 8214

  brset  coilabit,coilsel,dslsa   ;* IF coilabit = 1 GO TO LINE 8228
  brset  coilbbit,coilsel,dslsb   ;* IF coilbbit = 1 GO TO LINE 8232
...
dslsa:                                       ;* LINE 8228 
  bset   coila,portc		; Set spark on   ;* COIL A
  bra    cn_end                              ;* GO TO LINE 8255

dslsb:                                       ;* LINE 8232
  bset   coilb,portc		; Set spark on   ;* COIL B
  bra    cn_end                              ;* GO TO LINE 8255
...
; note no Toyota, because never inverted - ??? is this right
  ...
  brset  coilabit,coilsel,ilsoa          ;* IF coilabit = 1 GO TO LINE 8299
  brset  coilbbit,coilsel,ilsob          ;* IF coilbbit = 1 GO TO LINE 8303
...
;;redundant     brset   coilabit,coilsel,wd2a360
  brset  coilbbit,coilsel,wd2b360                                      ;* IF coilbbit = 1 GO TO LINE 8510  
  
wd2a360:
  sthx   SparkOnLeftah                                                 ;* DO THIS IF coilabit = 1
  bra    wd2end360                                                     ;* GO TO LINE 8513
...
; consider oddfire, do not delay from previous spark
  ...
  lda    SparkConfig1_f                                                ;* SparkConfig1_f = %00001100
...
wd2ok:
  ldhx   dwelldelay1
  brset  coilabit,coilsel,wd2a180                                          ;* IF coilabit = 1 GO TO LINE 8534
  brset  coilbbit,coilsel,wd2b180                                          ;* IF coilbbit = 1 GO TO LINE 8538
  
wd2a180:                                                                   ;* LINE 8534
  sthx   SparkOnLeftbh
  bra    wd2end180                                                         ;* GO TO LINE 8541
...
edis2_fire:                                                                                ;* SHOULD NEVER GET HERE
...
;
	...
	brclr	indwell,EnhancedBits4,sin_a                                          ;* IF indwell = 0, GO TO LINE 9412 
	brset	rotary2,EnhancedBits5,clr_a_b                                        ;* rotary2 = 0

sin_a:                                                                           ;* LINE 9412
  ldhx   SparkOnLeftah
  beq    sin_b                                                                   ;* IF SparkOnLeftah = 0 go to line 9423 
  ...
  clr    coilsel
  bset   coilabit,coilsel
  bra    lowspdspk                                                               ;* GO TO LINE 9490 
  
sin_b:                                                                           ;* LINE 9423
  ldhx   SparkOnLeftbh
  beq    sin_c                                                                   ;* IF SparkOnLeftbh = 0 go to line 9439 
  aix    #-1			; is it time to start charging
  sthx   SparkOnLeftbh
  cphx   #0
  bne    sin_c
...
sin_c:                                                                           ;* LINE 9439
  ldhx   SparkOnLeftch                                                           ;* SparkOnLeftch = 0, GO TO LINE 9450
  beq    sin_d                                                                    
  aix    #-1			; is it time to start charging
  sthx   SparkOnLeftch
  cphx   #0
...
sin_d:                                                                           ;* LINE 9450
  ldhx   SparkOnLeftdh                                                           ;* SparkOnLeftdh = 0, GO TO LINE 9461
  beq    sin_e                                                                    
  aix    #-1			; is it time to start charging
  sthx   SparkOnLeftdh
  cphx   #0
...
sin_e:                                                                           ;* LINE 9461
  ldhx   SparkOnLefteh                                                           ;* SparkOnLefteh = 0, GO TO LINE 9472 
  beq    sin_f                                                                    
  aix    #-1			; is it time to start charging
  sthx   SparkOnLefteh
  cphx   #0
...
sin_f:                                                                           ;* LINE 9472
  ldhx   SparkOnLeftfh                                                           ;* SparkOnLeftfh = 0, GO TO LINE 9483
  beq    j_CSL
  aix    #-1			; is it time to start charging
  sthx   SparkOnLeftfh
  cphx   #0
...
j_CSL:
  cli
  jmp    CHECK_SPARK_LATE                                                      ;* GO TO LINE 9536
...
chkindwell:                                                                    ;* LINE 9506
  brset  indwell,EnhancedBits4,blssd	; if doing hi-res dwell then don't turn on coil now  ;* IF indwell = 1 GO TO LINE 9526
...
EnableN2O:                                                                            ;* LINE 14060, LAUNCH CONTROL IS NOT ENABLED SO SKIP ALL DOWN TO LINE 14282 
...
Nos_Done_Now:
  rts                                                                         ;*  LINE 14282, END OF LAUNCH CONTROL SECTION

******************************************************************************
**        Check if were ready to use VE Table 3
******************************************************************************

Check_VE3_Table:                                                              ;* lINE 14288, NITROUS VE3 IS NOT USED SO SKIP ALL DOWN TO LINE 14302
...
Check_VE3_Done:
  ...
  rts                                                                         ;* LINE 14302, END OF NITROUS VE3 SECTION 
...
AFR1_Targets:                                                                                       ;* LINE 14310
  brset  useVE3,EnhancedBits,No_AFR_ForVE1	  ; If were running VE3 then no need to go any further  ;* useVE3 = 0, 
  lda    EGOcount		                      ; Are we about to check the ego?
  cmp    EGOcountcmp_f                                                                              ;* EGOcountcmp_f = 16T
  beq    Do_Targets		                      ; If yes then get the target from the table           ;* GO TO LINE 14319
  
No_AFR_ForVE1:
  rts				                          ; If No then return, this saves wasting time.

Do_Targets:				                      ; VE 1 Targets from AFR Table 1                          ;* LINE 14319
  brclr  TPSTargetAFR,feature7,NO_TPS_SetAFR1 ; Have we selected to go to targets above tps setpoint?  ;* TPSTargetAFR = 0, GO TO LINE 14328
  lda    tps
  cmp    AFRTarTPS_f
...
NO_TPS_SetAFR1:                                                                                         ;* LINE 14328
  clrh
  clrx
  brset  AlphaTarAFR,feature7,AFR1_AN	; Have we selected to use tps for target afrs instead of kpa?  ;* AlphaTarAFR = 0
  lda    kpa                            ; Normal Speed density
  bra    AFR1_STEP_1                                                                                   ;* GO TO LINE 14338

AFR1_AN:
  lda    tps

AFR1_STEP_1:                                                                                           ;* LINE 14338
  sta    kpa_n
  ldhx   #KPARANGEAFR_f1
  sthx   tmp1
  lda    #$07			; 8x8
...
AFR1_STEP_3:
  ...
  jsr    VE_STEP_4                                                           ;* GO TO LINE 6435
  mov    tmp6,afrTarget
  rts                                                                        ;* LINE 14389 END OF AFR1 TARGETS

*****************************************************************************
**             VE 3 Targets from AFR Table 2
*****************************************************************************

AFR2_Targets:                                                               ;* LINE 1496, AFR2_TARGETS NOT USED, SKIP ALL DOWN TO LINE 14475
...
AFR2_STEP_3:
  ...
  jsr    VE_STEP_4                                                 ;* GO TO LINE 6435
  mov    tmp6,afrTarget
  rts                                                              ;* LINE 14475 END OF AFR2 TARGETS
...
SRevLimAngle	db      56T	    ; Spark retard for above +10 degrees            ;* 3 Tuner Studio SoftLimRetard (%00111000, $38, 56T)             
				                ; 10 = 10 + 10 i.e. -10 start point.
				                ; So 56T = 10BTDC )
SRevLimHTime	db      50T	    ; Time in 1/10Sec in Soft limit till            ;* 4 Tuner Studio SoftLimMax  (%00110010, $32, 50T)
				                ; hard limit cuts in *0.1
SRevLimCTime	db      10T	    ; NOT USED                                      ;* 5 Not Used
RevLimit_f	    db      250T	; Hard Rev limiter (rpm*100)                    ;* 6 Tuner Studio HardRevLim  (%11111010, $FA, 250T)
...
bcFreqDiv_f	    db      3T	    ; Solenoid PW rate BITS 0 1 and 2 used :         ;* 14 Tuner Studio freqDiv   (N/A)                                     
				                ; "INVALID","39.0 Hz","19.5 Hz","13.0 Hz",
				                ; "9.8 Hz","7.8 Hz","6.5 Hz","5.6 Hz"
bcUpdate_f	    db      10T	    ; Boost Controller Update Rate in mS             ;* 15 Tuner Studio bcUpdateRate (N/A)
                		        ; (10min   255max)
bcPgain_f	    db      64T	    ; B Controller P Gain % (0-100% = 0-255 in       ;* 16 Tuner Studio pGain         (N/A) 
				                ; MS so MT value displayed = MS*0.3922)
bcDgain_f	    db      5T	    ; Boost Controller D Gain % (0-100% = 0-255      ;* 17 Tuner Studio dGain         (N/A)
				                ; in MS so MT value displayed = MS*0.3922)
ShiftLo_f	    db      58T	    ; Shift light LED start point (rpm*100)          ;* 18 Tuner Studio ShiftLower    (N/A) 
ShiftHi_f	    db      60T	    ; Shift Light Final point (RPM * 100)            ;* 19 Tuner Studio ShiftUpper    (N/A)
LaunchLimit_f	db      40T	    ; Hard limit for Launch control (rpm*100)        ;* 20 Tuner Studio LaunchLim     (N/A)                                     
edisms_f	    db      11T	    ; max rpm for EDIS multi-spark (rpm*100)         ;* 21 Tuner Studio edismultirpm  (N/A)
NosClt_f	    db      160T	; Nitrous System Min Coolant Temp Minimum        ;* 22 Tuner Studio NosClt        (N/A)
				                ; point of coolant for NOS to enable in (F-40)
NosRpm_f	    db      30T	    ; Nitrous System Min RPM * 100, 3000rpm is       ;* 23 Tuner Studio NosRpm        (N/A)
				                ; minimum allowed  (23)
...
spare1_2_f      db       0T                                                       ;* 35 Not Used             (N/A)
spare1_3_f      db       0T                                                       ;* 36 Not Used             (N/A)
spare1_4_f      db       0T                                                       ;* 37 Not Used             (N/A)
spare1_5_f      db       0T                                                       ;* 38 Not Used             (N/A)

EgoLimitKPa_f	db      255T	    ; MAP KPa Point to change Ego +- limit (39)             ;* 39 Tuner Studio EgoLimKPa (= 255T default))                                                                                                                    
EgoLim2_f	    db      05T	        ; New Ego limit when MAP KPa above EgoLimitKPa_f        ;* 40 Tuner Studio EgoLim2   (N/A)
LC_Throttle_f	db      30T	        ; Throttle position in RAW data for launch control mode ;* 41 Tuner Studio LachTps   (N/A)
LC_LimAngle_f	db      42T	        ; Launch control soft limiter angle           ;* 42 Tuner Studio LachDeg             (N/A)
				                    ; *0.352   -28.4     -10 to 45 allowed        
LC_Soft_Rpm_f	db      35T	        ; Launch Soft Limit RPM (43)                  ;* 43 Tuner Studio LachRpm             (N/A)
Over_B_P_f	    db      0T	        ; Over boost Protection KPa setpoint          ;* 44 Tuner Studio OvrBKpa             (N/A)
...
feature3_f	    db      %00110000   ; (46)                                        ;* 46 Tuner Studio ????????   (= %10000001, $81, 129T)
KPaTpsOpenb      equ 1                                                            ;* Tuner Studio OLoopEgo bit0 (= 1 (KPA))
...
TargetAFRb       equ $80                                                          ;* Tuner Studio AfrTar bit7   (= 1 (On)
...
maxAdvAng_f	    db      15T         ; Limit in degrees of advance for coolant              ;* 49 Tuner Studio MaxIatAdv (N/A) 
				                    ; related advance so it doesnt add loads of
				                    ; advance when very cold *0.352
iatDeg_f	    db      18T         ; Iat Temp for 1 degree of retard related              ;* 50 Tuner Studio IgRetDeg  (N/A)                                          
				                    ; to IAT, exactly the same as cltDeg_f but
				                    ; retard rather than advance and
                                    ; IAT rather that CLT. F
kpaRetard_f	    db      75T         ; Apply the IAT related retard when above              ;* 51 Tuner Studio IatAdvKpa  (N/A)
				                    ; this KPa, to stop retard at tickover
iatDanger_f	    db      200T        ; Iat Temp to start Retard F -40                       ;* 52 Tuner Studio IgRetTemp  (N/A)
KnockRpmL_f	    db      55T         ; Knock sensor max rpm RPM*100 (53)                    ;* 53 Tuner Studio KnockRPM   (N/A) 
KnockRpmLL_f	db      15T         ; Knock sensor min rpm RPM*100                         ;* 54 Tuner Studio KnockRpmL  (N/A)
KnockKpaL_f	    db      255T        ; knock sensor max KPa                                 ;* 55 Tuner Studio KnockKpa   (N/A)
KnockRet1_f	    db      06T         ; First Retard amount for knock system *0.352          ;* 56 Tuner Studio KnockRet   (N/A)
KnockRet2_f	    db      03T         ; Subsequent Retard amount *0.352                      ;* 57 Tuner Studio KnockSb    (N/A)
KnockAdv_f	    db      03T         ; Advance amount for knock system *0.352               ;* 58 Tuner Studio KnockAdv   (N/A)
KnockMax_f	    db      15T         ; Max Allowable retard *0.352                          ;* 59 Tuner Studio KnockMax   (N/A)
KnockTim_f	    db      01T         ; Timer for steps of knock advance / retard            ;* 60 Tuner Studio KnockTim   (N/A)
				                    ; to be applied in Seconds 1
iatpoint_f	    db      100T        ; Water Inj IAT setpoint point F -40 (61)              ;* 61 Tuner Studio WaterIIat  (N/A)
wateripoint_f	db      120T        ; Water Injection KPa setpoint                         ;* 62 Tuner Studio StartWIKpa (N/A)
wateriRpm_f     db      35T         ; Water Injection RPM setpoint RPM*100                 ;* 63 Tuner Studio WaterIRpm  (N/A)
kpaO2_f		    db      80T         ; KPa Open loop setpoint for no O2 correction          ;* 64 Tuner Studio KpaOn      (Use default 80T in case we want closed loop)
tpsO2_f		    db      192T        ; TPS Open Loop setpoint for no O2 correction Raw ADC  ;* 65 Tuner Studio TpsOn      (Use default 192T in case we want closed loop)
...
NosFuelLo_f	    db      12T         ; Nitrous Pulse Width to add to fuel at                    ;* 69 Tuner Studio NosPWLo  (N/A)
				                    ; 3000 rpm *0.1 in mSec this is for                                                         
				                    ; additional fuel for NOS
NosFuelHi_f	    db      03T         ; Nitrous Pulse Width to add to fuel at                    ;* 70 Tuner Studio NosPWHi      (N/A)
				                    ; NosRpmMax_f *0.1 in mSec
ORunRpm_f	    db      17T         ; Max RPM for Over run fuel cut *100                       ;* 71 Tuner Studio OvrRunRpm    (N/A)
ORunKpa_f	    db      20T         ; Over run fuel cut when below kpa                         ;* 72 Tuner Studio OvrRunKpa    (N/A)
ORunTPS_f	    db      05T         ; Over run when throttle position lower than this RAW ADC (73) ;* 73 Tuner Studio OvrRnTps (N/A)
...
SparkCutBase_f	db      06T	        ; Base number to cut sparks from MS                          ;* 78 Tuner Studio SparkCutBase (N/A)
				                    ; = MT value - 1
SCALEFAC_f	    db      255T	    ; Scaling factor for STAGED INJECTION MODE                   ;* 79 Tuner Studio ScaleFac (N/A)
				                    ; (prim flow/total flow*100) 255=100% 123=50%
STGTRANS_f	    db      25T	        ; Staged transition point, rpm*100, kpa,                     ;* 80 Tuner Studio StgTrans (N/A)
				                    ; or tps raw adc depending on staging
				                    ; method selected (See feature5_f bits 3-4)
STGDELTA_f	    db      03T	        ; Staged operation off at (STGTRANS-STGDELTA)                ;* 81 Tuner Studio StgDelta (N/A)                                                                          
				                    ; so this is raw data as STGTRANS
BarroHi_f	    db      110T	    ; Barometric Correction Max Limit in KPa (82                 ;* 82 Tuner Studio BarroHi      (= %01110100, $74, 116T)
BarroLow_f	    db      60T	        ; Barometric Correction Lower Limit in KPa                   ;* 83 Tuner Studio BarroLow     (= %01000011, $43, 67T)
SparkCutBNum_f	db      03T	        ; Number of sparks to remove from BASE value when Over Boost ;* 84 Tuner Studio SparkCutBNum (N/A)
NosLowKpa_f	    db      80T	        ; Minimum KPa to fire Nos Anti-lag                           ;* 85 Tuner Studio NosLowKpa    (N/A)
NosHiKpa_f	    db      120T	    ; Maximum KPa to fire Nos Anti-lag,                          ;* 86 Tuner Studio NosHiKpa     (N/A)
				                    ; Anti-lag will switch off when this is reached
Spark2Delay_f	db      00T	        ; Delay for Spark Table 2 to come in when                    ;* 87 Tuner Studio ST2Delay     (N/A)
				                    ; input received. *0.1   1/10Sec
Out1UpLim_f	    db      00T	        ; Output 1 top limit for window, so output1                  ;* 88 Tuner Studio Out1UpLim    (N/A)
				                    ; will go off above this value unless its 0
				                    ; then its ignored
Out2UpLim_f	    db      00T	        ; Output 2 top limit for window (89)                         ;* 89 Tuner Studio Out2UpLim    (N/A)
NumTeeth_f	    db      12T	        ; Number of teeth for wheel decoder                          ;* 90 Tuner Studio numteeth     (= %00100100, $24, 36T)
MAPThresh_f	    db      30T	        ; MAP dot threshold for Accel Decel                          ;* 91 Tuner Studio mapThresh4   (N/A)                 
...
VE3Delay_f	    db      00T	        ; Delay for VE Table 3 to come in when                       ;* 93 Tuner Studio VE3Delay (N/A)
				                    ; input received. *0.1   1/10Sec
RPMrate_f:
	            db      00T		    ; Fuel enrichment in mSec for 3000RPM/Sec                    ;* 94 Tuner Studio RPM1S    (N/A)
				                    ; increase or 5% slip if in VSS mode *0.1
       	        db      100T		; Fuel enrichment in mSec for 4000RPM/Sec                    ;* 95 Tuner Studio RPM2S    (N/A)
				                    ; increase or 15% slip if in VSS mode *0.1
             	db      150T		; Fuel enrichment in mSec for 6000RPM/Sec                    ;* 96 Tuner Studio RPM3S    (N/A)
				                    ; increase or 30% slip if in VSS mode *0.1 (96)
       	        db      200T		; Fuel enrichment in mSec for 10000RPM/Sec                   ;* 97 Tuner Studio RPM4S    (N/A)
				                    ; increase or 70% slip if in VSS mode *0.1

RPMthresh_f	    db      2T	        ; Threshhold for RPM change for traction                     ;* 98 Tuner Studio RPMThresh (N/A)
				                    ; (rpm * 1000) because it checks every 1/10
				                    ; sec and rpm = rpm*100. So 2 = 2000RPM/Sec
				                    ; threshold
TractDeg_f:
	            db      00T		    ; Ignition retard in Degrees for 3000RPM/Sec                 ;* 99 Tuner Studio TractRet1  (N/A)
				                    ; increase or 5% slip if in VSS mode *0.352
	            db      56T		    ; Ignition retard in Degrees for 4000RPM/Sec                 ;* 100 Tuner Studio TractRet2  (N/A)
				                    ; increase or 15% slip if in VSS mode *0.352
	            db      56T		    ; Ignition retard in Degrees for 6000RPM/Sec                 ;* 101 Tuner Studio TractRet3  (N/A)
				                    ; increase or 30% slip if in VSS mode *0.352
	            db      85T	    	; Ignition retard in Degrees for 10000RPM/Sec                ;* 102 Tuner Studio TractRet4  (N/A)
				                    ; increase or 70% slip if in VSS mode *0.352(102)
TractSpark_f:
	            db      00T		    ; Spark Cut from Base number for 3000RPM/Sec                 ;* 103 Tuner Studio TractSC1  (N/A)
				                    ; increase or 5% slip if in VSS mode MAX ALLOWED 5
	            db      00T		    ; Spark Cut from Base number for 4000RPM/Sec                 ;* 104 Tuner Studio TractSC2  (N/A)
				                    ; increase or 15% slip if in VSS mode
	            db      01T		    ; Spark Cut from Base number for 6000RPM/Sec                 ;;* 105 Tuner Studio TractSC3  (N/A)
				                    ; increase or 30% slip if in VSS mode
	            db      02T		    ; Spark Cut from Base number for 10000RPM/Sec                ;* 106 Tuner Studio TractSC4  (N/A)
				                    ; increase or 70% slip if in VSS mode (106)

BoostKnock_f	db      00T	        ; Boost to remove from controller target when                ;* 107 Tuner Studio KnockBoost (N/A)
				                    ; Knock detected (PSI) so value of MS 1 = 7KPa
BoostKnMax_f	db      30T	        ; Max Boost to remove when knocking in PSI so                ;* 108 Tuner Studio BoostMaxKn (N/A)
...
TractCycle_f:
	            db      03T		; Engine cycles to hold enrichment /                              ;* 112 Tuner Studio TractCyc1  (N/A)
				                ; spark cut / retard for 3000RPM/Sec increase
				                ; or 5% slip if in VSS mode
	            db      05T		; Engine cycles to hold enrichment /                              ;* 113 Tuner Studio TractCyc2  (N/A)
				                ; spark cut / retard for 4000RPM/Sec increase
				                ; or 15% slip if in VSS mode
	            db      08T		; Engine cycles to hold enrichment /                              ;* 114 Tuner Studio TractCyc3  (N/A)
				                ; spark cut / retard for 6000RPM/Sec increase
				                ; or 30% slip if in VSS mode
	            db      12T		; Engine cycles to hold enrichment /                              ;* 115 Tuner Studio TractCyc4  (N/A)                               
				                ; spark cut / retard for 15000RPM/Sec increase
				                ; or 70% slip if in VSS mode

feature8_f	    db      %00000000  ; More feature bits (116)                                      ;* 116 Tuner Studio ????        (= %01000000, $0, 0T)
...
UDSpeedLim_f	db      255T	   ; Max speed from the Undriven wheel for                        ;* 118 Tuner Studio TractVSSMax  (N/A)
				                   ; traction to work at. Volts *0.0196  5V=255
TCScaleFac_f	db      125T	   ; Difference factor for speed inputs from                      ;* 119 Tuner Studioi TractScale  (N/A)
				                   ; driven and undriven inputs
				                   ; (255=100%) *0.39216
TCSlipFac_f	    db      25T	       ; Slip allowed between wheel sensors at low                    ;* 120 Tuner Studio TractSlip    (N/A)
...
OverRunT_f	    db      00T	       ; Over Run Timer before enabling over run                      ;* 130 Tuner Studio OvrRunTimr   (N/A)
				                   ; in Seconds *1
BarCorr300_f	db      255T	   ; Correction factor for KPA Factor for                         ;* 131 NOT USED                  (N/A)
...
iatBoostSt_f	db      00T	       ; Start point for boost reduction related to                   ;* 135 Tuner Studio IATBoostSt  (N/A)
				                   ; IAT when using Boost controller F -40
iatBoost_f	    db      00T	       ; Amount of IAT to remove 1 PSI from boost                     ;* 136 Tuner Studio IATBoost    (N/A)
...
spare3_f	    db      00T	       ;   ** SPARE **                                                ;* 142 NOT USED                   (N/A)
mindischg_f  	db      05T	       ; minimum discharge period for dwell                           ;* 143 Tuner Studio mindischg     (= %00000101, $5, 5T)
				                   ; control in mSec *0.1
...
Trig6ret_f	    db      0T	       ;  "                                                           ;* 148 Tuner Studio trig6ret (N/A)


RPMbasedrate_f:
	            db	    05T		   ; These next 4 are for adding AE based on engine rpm           ;* 149 Tuner Studio RPMAE4  (N/A)
                db	    20T		   ; This is the actual engine rpm settings                       ;* 150 Tuner Studio RPMAE3  (N/A)
	            db	    35T		   ;                                                              ;* 151 Tuner Studio RPMAE2  (N/A)
	            db	    55T		   ; (167)                                                        ;* 152 Tuner Studio RPMAE1  (N/A)
RPMAQ_f2        db      01T        ; Amount of fuel to add for 1st area of rpm based AE           ;* 153 Tuner Studio RPMS1   (N/A)
                db      10T        ; Fuel for 2nd rpm area                                        ;* 154 Tuner Studio RPMS2   (N/A)
                db      15T        ; Fuel for 3rd                                                 ;* 155 Tuner Studio RPMS3   (N/A)
RPMAQL_f2:      db      20T        ; Fuel for 4th (171)                                           ;* 156 Tuner Studio RPMS4   (N/A)

n2odel_launch_f: db     00T        ; delay from launch to nitrous activation                      ;* 157 Tuner Studio N2Odel_launch (N/A)
n2odel_flat_f:   db     00T        ; delay from flat shift to nitrous activation                  ;* 158 Tuner Studio N2Odel_flat   (N/A)
n2oholdon_f:     db     00T        ; how long do extra nitrous fuel and retard hold on            ;* 159 Tuner Studio N2Oholdon     (N/A)

xxKPaCorr300_f  db      00T        ; KPa correction factor for 400/300KPa sensor (175)            ;* 160 NOT USED                   (N/A)


tpsdotrate:
	            db	    05T		    ; These next 4 are delta points for TPSdot                    ;* 161 Tuner Studio taeRates4   (= %00001010, $A, 10T)
				                    ; V/Sec
...
TPSTHRESH_f1	db      03T	        ; TPSTHRESH for Accel enrichment when in                      ;* 178 Tuner Studio tpsThresh4  (= %00000011, $3, 3T)
				                    ; TPS mode *0.1953125
TPSASYNC_f1	    db      02T	        ; TPSASYNC (accel enrich time in 1/10                         ;* 179 Tuner Studio taeTime4    (= %00000010, $2, 2T)
				                    ; second increments) or in Enfgine Cycles.
TPSDQ_f1	    db      100T	    ; TPSDQ   (195)                                               ;* 180 Tuner Studio tdePct4     (= %01100100, $64, 100T)
ACMULT_f1	    db      100T	    ; Cold ACCELMULT                                              ;* 181 Tuner Studio taeColdM4   (= %01100100, $64, 100T)
OverRunClt_f1	db      100T	    ; No Over run fuel cut when below this                        ;* 182 Tuner Studio OvrRunClt   (= %01100100, $64, 100T)
				                    ; coolant temp F-40 (197)
AccelDecay_f	db       00T	    ; This is the value in mS that the Accel                      ;* 183 Tuner Studio DecayAcPoint (= %00000000, $0, 0T)
...
EGOTEMP_f	    db      200T	; EGOTEMP                                   ;* 144 Tuner Studio egoTemp1   (= 200T (160 degrees default))
EGOCOUNTCMP_f	db      16T	    ; EGOCOUNTCMP                               ;* 145 Tuner Studio egoCount1  (= 16T (default)
EGODELTA_f	    db      1T	    ; EGODELTA                                  ;* 146 Tuner Studio egoDelta1  (= 0 (open loop only, 1 = default)
EGOLIMIT_f	    db      15T	    ; EGOLIMIT                                  ;* 147 Tuner Studio egoLimit1  (= 15T default))
REQ_FUEL_f1	    db      155T	; REQFUEL (148)                             ;* 148 Tuner Studio reqfuel1   (= %01101000, $68, 104T)
DIVIDER_f1	    db      4T	    ; DIVIDER                                   ;* 149 Tuner Studio divider1   (= %00000100, $4, 4T)
Alternate_f1	db      0	    ; alternate or simult (single table ONLY)   ;* 150 Tuner Studio alternate1 (= %00000000, $0, 0T)
INJOPEN_f1	    db      10T	    ; INJOPEN                                   ;* 151 Tuner Studio injOpen1   (= %00001001, $9, 9T)
INJOCFUEL_f1	db      0T	    ; INJOCFUEL  NOT USED NOW !!!!!             ;* 152  NOT USED               (N/A)
				                ; Kept to fill hole                         
INJPWM_f1	    db      75T	    ; INJPWM                                    ;* 153 Tuner Studio injPwmP1
INJPWMT_f1   	db      255T	; INJPWMT                                   ;* 154 Tuner Studio injPwmT1 
BATTFAC_f1	    db      12T	    ; BATTFAC                                   ;* 155 Tuner Studio battFac1   (= %00010000, $14, 20T)
rpmk_f1		    db      $05	    ; RPMK[0]                                   ;* 156 Tuner Studio rpmk1 (16 bit) (= %1011, $B, 11T)
		        db      $DC	    ; RPMK[1]                                   ;* 157 Tuner Studio rpmk1          (= %10111000, $B8, 184T)
RPMRANGEVE_f1:
...
config11_f1	    db      113T	; Config11 (originally 113T for 8 cyl) (182);* 182 Tuner Studio Config1 (= %00110001, $31, 49T)
;  Bit 0-1 = MAP Type
;            00 = MPX4115AP
;            01 = MPX4250AP
;            10 = MPXH6300A
...
VE_f2:                  ; 12x12 Total Bytes = 144                                ;* 0 Tuner Studio veBins2 (N/A)
	db      39T,40T,41T,44T,44T,44T,45T,45T,45T,46T,47T,50T ; VE (0,0-11)
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (1,0-11)
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (2,0-11)
    db      52T,55T,55T,57T,60T,61T,61T,65T,67T,70T,72T,75T ; VE (3,0-11)
...
EGOtemp_f2	        db      200T    ; For second O2 sensor (feature12_f2)       ;* 144 Tuner Studio egoTemp2   (N/A)
EGOCOUNTCMP_f2	    db      16T	    ; NOT USED                                  ;* 145 NOT USED                (N/A)
EGOdelta_f2	        db      1T	    ; For second O2 sensor (feature12_f2) (146) ;* 146 Tuner Studio egoDelta2  (N/A)
EGOlimit_f2	        db      10T     ; For second O2 sensor (feature12_f2)       ;* 147 Tuner Studio egoLimit2  (N/A)
REQ_FUEL_f2	        db      155T    ; (148)                                     ;* 148 Tuner Studio reqFuel2   (N/A)
Divider_f2	        db      4T                                                  ;* 149 Tuner Studio divider2   (N/A)
Alternate_f2	    db      0T	    ; NOT USED                                  ;* 150 Tuner Studio alternate2 (N/A)
InjOpen_f2	        db      10T                                                 ;* 151 Tuner Studio injOpen2   (N/A)
InjOCFuel_f2	    db      0T	    ; NOT USED NOW !!!!! Kept to fill hole      ;* 152 NOT USED                (N/A)
INJPWM_f2	        db      100T                                                ;* 153 Tuner Studio injPwm2    (N/A)
INJPWMT_f2	        db      255T                                                ;* 154 Tuner Studio injPwmT2   (N/A)
BATTFAC_f2	        db      12T     ; (155)                                     ;* 155 Tuner Studio battFac2   (N/A)
rpmk_f2:	                        ; type=byte  entries=2  total bytes=2       
	db      $05,$DC		            ; (156,157)                                 ;* 156,157 Tuner Studio rpmk2 (16 bit)  (N/A)
RPMRANGEVE_f2:	                    ; type=byte  entries=8  total bytes=8
	db      5T,10T,15T,20T,28T,36T,44T,52T,55T,60T,65T,70T                      ;* 158 Tuner Studio rpmBins2  (N/A)
KPARANGEVE_f2:                      ; type=byte  entries=8  total bytes=8
	db      20T,30T,40T,50T,60T,75T,90T,100T,110T,120T,130T,140T                ;* 170 Tuner Studio mapBins2  (N/A)
config11_f2:
config21_f	       db      113T	    ; NOT USED by code, but MT??(182)           ;* 182 Tuner Studio Config1   (N/A)
config12_f2:
...
TriggAngle_f	db      171T	; TriggAngle (60 deg)     (168)  *0.352            ;* 168 Tuner Studio TrigAng   (= 227T) (80 deg)
FixedAngle_f	db      0T	    ; FixedAngle   *0.352  -28.4   min -10 Max 80      ;* 169 Tuner Studio FixAng    (= -10 for table, 56T for 10 degrees)
				                ; THIS MUST BE -10 (0) for non fixed angle
TrimAngle_f	    db      0T	    ; TrimAngle (NOT cleared on startup)  *0.352       ;* 170 Tuner Studio Trim      (0 deg)
CrankAngle_f	db      56T	    ; Cranking advance (10deg)  *0.352 -28.4           ;* 171 Tuner Studio CrankAng  (10 deg)
				                ;  min -10 max 80
...
flash_table4:                   ; Spark Table 2, used when input switched    ;* Tuner Studio page 5
				; low if selected
ST_f2:				; *0.352  -28.4   Min -10 Max 80                         ;* 0 Tuner Studio advTable2  (N/A)
	db    53T,53T,58T,70T,90T,119T,131T,131T,131T,131T,131T,131T ;(0,0-11)
	db    53T,53T,58T,70T,87T,113T,119T,119T,119T,119T,119T,119T ;(1,0-11)
	db    58T,58T,64T,70T,84T,107T,113T,113T,113T,113T,113T,113T ;(2,0-11)
	db    58T,58T,64T,70T,81T,104T,107T,107T,107T,107T,107T,107T ;(3,0-11)
...
RPMRANGEST_f2:
        db      05T,07T,13T,20T,30T,40T,50T,60T,61T,62T,63T,64T             ;* 144 Tuner Studio rpmBins4  (N/A)
				; RPMRANGEST[0-b]
...
VE_f3:
	db      39T,40T,41T,44T,44T,44T,45T,45T,45T,46T,47T,50T ; VE (0,0-11)       ;* 0  Tuner Studio veBins3  (N/A)
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (1,0-11)
	db      47T,47T,51T,51T,50T,50T,50T,50T,51T,55T,56T,60T ; VE (2,0-11)
	db      52T,55T,55T,57T,60T,61T,61T,65T,67T,70T,72T,75T ; VE (3,0-11)
	db      52T,55T,55T,57T,60T,61T,61T,65T,67T,70T,72T,75T ; VE (4,0-11)
	...
	db      75T,77T,79T,73T,82T,82T,82T,85T,87T,89T,99T,100T; VE (11,0-11)
RPMRANGEVE_f3:
	db      5T	    ; RPMRANGEVE[0] (144)                                      ;* 144 Tuner Studio rpmBins5 (N/A)
	db      10T	    ; RPMRANGEVE[1]
	db      15T  	; RPMRANGEVE[2]
	db      20T	    ; RPMRANGEVE[3]
	db      28T	    ; RPMRANGEVE[4]
...
KPARANGEVE_f3:
    db      20T	    ; KPARANGEVE[0] (156)                                     ;* 156 Tuner Studio mapBins5 (N/A)
	db      30T	    ; KPARANGEVE[1]
	db      40T	    ; KPARANGEVE[2]
	db      50T	    ; KPARANGEVE[3]
	db      60T	    ; KPARANGEVE[4]
...
ASEVTbl_f:
	...
	db      12T	    ; 20F                                                    ;* 171 Tuner Studio ASETable4  (N/A)
	db      10T	    ; 40F                                                    ;* 172 Tuner Studio ASETable5  (N/A)
	db       9T	    ; 60F                                                    ;* 173 Tuner Studio ASETable6  (N/A)
	db       8T	    ; 80F                                                    ;* 174 Tuner Studio ASETable7  (N/A)
	db       7T	    ; 100F                                                   ;* 175 Tuner Studio ASETable8  (N/A)
...
CltFixASE_f     db      85T    ; Coolant temp setpoint to use Fixed value ASE                              ;* 181 Tuner Studio ASEFixTemp     (N/A)
MAPFixASE_f     db      60T    ; If in fixed MAP mode then hold MAP at this value during fixed ASE time    ;* 182 Tuner Studio VEFixValue     (N/A)
...
AFR_f2:			; This is in RAW ADC so 255 = 5V from O2 sensor
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (0,0-7) ;* 80 Tuner Studio afrBins2  (N/A)
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (1,0-7)
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (2,0-7)
	db      147T,147T,147T,147T,147T,147T,147T,147T; AFR (3,0-7)
	db      130T,130T,130T,130T,130T,130T,130T,130T; AFR (4,0-7)
...
RPMRANGEAFR_f2:
	db      5T	    ; RPMRANGEAF2[0]                             ;* 144 Tuner Studio mapBins7 (N/A)
...
KPARANGEAFR_f2:
	db      15T	    ; KPARANGEAF2[0]                             ;* 152 Tuner Studio mapBins7 (N/A) 
...
Nos2Rpm_f     db     255T  ; rpm starts at            ;* 160 Tuner Studio Nos2Rpm    (N/A)
Nos2RpmMax_f  db     255T  ; rpm ends st              ;* 161 Tuner Studio Nos2RpmMax (N/A)
Nos2delay_f   db     0T    ; delay after stage 1      ;* 162 Tuner Studio Nos2Delay  (N/A)
Nos2Angle_f   db     0T    ; retard                   ;* 163 Tuner Studio Nos2Angle  (N/A)
Nos2PWLo_f    db     0T    ; +pw at low rpm           ;* 164 Tuner Studio Nos2PWLo   (N/A)
...
bc_kpa_f:
	db      100T,100T,100T,100T,100T,100T                        ;* 0 Tuner Studio bcBins1   (N/A)
...
RPMRANGEbc_f:
	db      10T,20T,30T,40T,50T,70T   ; RPMRANGEbc[0-5]           ;* 36 Tuner Studio bcrpmBins1 (N/A)
TPSRANGEbc_f:
	db      51T,77T,102T,127T,179T,230T   ; TPSRANGEbc[0-5]       ;* 42 Tuner Studio bctpsBins1 (N/A)

;boost controller, duty cycle target rpm vs tps

bc_dc_f:
	db      50T,50T,50T,50T,50T,50T                               ;* 48 Tuner Studio bcBins2    (N/A)
...
RPMRANGEbc_f2:
	db      10T,20T,30T,40T,50T,70T   ; RPMRANGEbc[0-5]           ;* 84 Tuner Studio bcrpmBins2 (N/A)
TPSRANGEbc_f2:
	db      51T,77T,102T,127T,179T,230T   ; TPSRANGEbc[0-5]       ;* 90 Tuner Studio bctpsBins2 (N/A)


; Second boost target table, switched over on input if selected.
bc3_kpa_f:
	db      100T,100T,100T,100T,100T,100T                         ;* 96 Tuner Studio bcBins3   (N/A)
	db      100T,120T,120T,120T,120T,120T
	db      100T,120T,120T,120T,120T,120T
	db      100T,120T,120T,120T,120T,120T
	db      100T,120T,120T,120T,120T,120T
...
RPMRANGE3bc_f:
	db      10T,20T,30T,40T,50T,70T   ; RPMRANGE2bc[0-5]         ;* 132 Tuner Studio bcrpmBins3 (N/A)
TPSRANGE3bc_f:
        db      51T,77T,102T,127T,179T,230T   ; TPSRANGE2bc[0-5] ;* 138 Tuner Studio bctpsBins3 (N/A)
...
idle_dc_lo	      db      50T	; Idle duty cycle at lower temp for 2-wire               ;* 0 Tuner Studio idle_dc_lo (= %00110010, $32, 50T)
				                ; Fielding Idle control *1  MIn = 0 Max = 100
idlePeriod_f	  db      25T	; idle period in ignition events                         ;* 1 Tuner Studio idleperiod (N/A)
;idlekickrpm_f	  db      07T	; low rpm to trigger kick up of pwm idle duty
;idlekickdc_f	  db      55T	; default duty cycle
idlecrankdc_f	  db      50T	; cranking idle dc kg                                    ;* 2 Tuner Studio idlecrankdc (= %00110010, $32, 50T
idledelayclock_f  db	  01T	; idle dashpot settling delay kg                         ;* 3 Tuner Studio idledelayclock (N/A

idledashdc_f	  db      45T	; dashpot default duty                                   ;* 4 Tuner Studio idledashdc (N/A)
idlemindc_f       db      27T	; minimum duty cycle                                     ;* 5 Tuner Studio idlemindc  (N/A)
idle_dc_hi        db      00T   ; rmd Idle duty cycle at upper temp for 2-wire (warmup)  ;* 6 Tuner Studio idle_dc_hi (= %00000000, $0, 0T))
;idlekpa1_f        db      100T  ; rmd
;idlekpa2_f        db      100T  ; rmd
;ikpamin1_f        db      30T   ; rmd
;ikpamin2_f        db      30T   ; rmd
ictlrpm1_f        db      3T	; idle deviation rpmx10 kg                               ;* 7 Tuner Studio ict1rpm1    (N/A) 
ictlrpm2_f        db      10T	; idle deviation rpmx10 kg                               ;* 8 Tuner Studio ict1rpm2    (N/A)
Ideadbnd_f        db      03T	; idle deadband range kg                                 ;* 9 Tuner Studio Ideadbnd    (N/A)
Idashdelay_f      db      0T	; AIC closure delay ign events kg                        ;* 10 Tuner Studio Idashdelay (N/A)
idlefreq_f        db      100T  ; rmd                                                    ;* 11 Tuner Studio idlefreq   (= %01100100, $64, 100T))
;delay2rpm_f       db      20T   ; rmd
idlestartclk_f    db      10T	; startup decay timer ign events kg                      ;* 12 Tuner Studio idlestartclk (N/A)
idlePeriod2_f     db      10T   ; rmd                                                    ;* 13 Tuner Studio idlePeriod2  (N/A)
...
slowIdleTemp_f	  db      234T	; Feilding 2-Wire Idle control Slow Idle                 ;* 18 Tuner Studio slowIdleTemp  (= %11001000, $C8, 200T)
				                ; upper temperature F -40
fastIdle_f	      db      110T	; Fast Idle RPM (RPM*10 100-2550 rpm range)              ;* 19 Tuner Studio fastIdleRPM  (N/A)
slowIdle_f	      db       65T	; Slow Idle RPM (RPM*10 100-2550 rpm range)              ;* 20 Tuner Studio slowIdleRPM  (N/A)
idleThresh_f	  db       30T	; TPS Raw value for Idle mode to kick in.                ;* 21 Tuner Studio idleThresh   (N/A)                                              
...
CrankRPM_f	    db      $03	 ; Maximum RPM for cranking (rpm*100)                        ;* 43 Tuner Studio CrankRPM (= 300T)
...
cltMATcorr_f:                   
                ...
                db      98T  ; IAT Air Density Correction 110 = correction * 110%        ;* 48 NOT USED              (N/A)
                db      96T                                                              ;* 49 NOT USED              (N/A)
                db      94T                                                              ;* 50 NOT USED              (N/A)
                db      92T                                                              ;* 51 NOT USED              (N/A)
                db      90T                                                              ;* 52 NOT USED              (N/A)
                db      88T  ; (53)                                                      ;* 53 NOT USED              (N/A)

RPMReduLo_f     db      10T  ; lowest rpm to reduce correction by (54)                   ;* 54 Tuner Studio RPMRedLo (N/A)
RPMReduHi_f     db      30T  ; Highest rpm when correction is back to normal             ;* 55 Tuner Studio RPMRedHi (N/A)

CltMATRange:
    db      200T    ; 160F   User defined Temp settings for Clt Related Air Dens (56)    ;* 56 Tuner Studio IATCTemp (N/A)
    db      207T    ; etc.                                                               ;* 57 NOT USED              (N/A) 
    db      216T    ;                                                                    ;* 58 NOT USED              (N/A)
    db      225T    ;                                                                    ;* 59 NOT USED              (N/A)
    db      234T    ;                                                                    ;* 60 NOT USED              (N/A)
    db      243T    ;                                                                    ;* 61 NOT USED              (N/A)
    db      252T    ; 212F (62)                                                          ;* 62 NOT USED              (N/A)

;rotary leading trailing split timing 6x6 table
split_f:   ; (63)
	db      29T,88T,88T,88T,88T,88T                                                      ;* 63 Tuner Studio splitTable (N/A)
