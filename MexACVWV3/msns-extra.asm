...
;* By Robert Hiebert, September 13 2023
;*
...
;                    VE TABLE 1
$MACRO ve1x   	; gets a VE byte from page1 or RAM. On entry X contains index.
				; Returns byte in A
    lda    page
    cmp    #01T
    bne    ve1xf
...
;                   VE TABLE 2
$MACRO ve2x		  ; gets a VE byte from page2 or RAM. On entry X contains index.
				  ; Returns byte in A
    lda    page
    cmp    #02T
    bne    ve2xf
...
;                  SPARK TABLE 1
$MACRO ve3x   	  ; gets a ST byte from page3 or RAM. On entry X contains index.
				  ; Returns byte in A
    lda    page
    cmp    #03T
    bne    ve3xf
...
;                  SPARK TABLE 2
$MACRO ve4x		 ; gets a ST byte from page4 or RAM. On entry X contains index.
				 ; Returns byte in A
    lda    page
    cmp    #04T
    bne    ve4xf
...
;                   VE TABLE 3
$MACRO ve5x		 ; gets a VE byte from page5 or RAM. On entry X contains index.
				 ; Returns byte in A
    lda    page
    cmp    #05T
    bne    ve5xf
...
;                  AFR TABLE 1 for VE1
$MACRO AFR1X	 ; gets an AFR byte from page6 or RAM. On entry X contains index.
				 ; Returns byte in A
    lda    page
    cmp    #06T
    bne    ve6xf
...
;                 AFR TABLE 2 for VE3
$MACRO AFR2X     ; gets an AFR byte from page7 or RAM. On entry X contains index.
				 ; Returns byte in A
    lda    page
    cmp    #07T
    bne    ve7xf
...
; "send" the data after changes.
    ...
    lda      #$ff
    sta      page		; select invalid page to make sure we run from flash
...
;real angle. Shouldn't happen if angles set correctly.
    lda     TriggAngle_f          ;* TriggAngle_f = 253 (253 * .352 = 89 degrees)
    cmp     #57T			      ; check for next cyl mode
    bhi     init_crang		      ; trigger angle > 20, continue
    bset    nextcyl,EnhancedBits4

init_crang:
    lda     CrankAngle_f                   ;* CrankAngle_f = 56 (56 - 28.4 = 27.6 * .352 = 9.71 (10 degrees)
    sta     SparkAngle                     ;* SparkAngle now = 56 (10 degrees)
    lda     TriggAngle_f                   ;* TriggAngle_f = 253 (253 * .352 = 89 degrees)
    sub     CrankAngle_f                   ;* CrankAngle_f = 56 (56 - 28.4 = 27.6 * .352 = 9.71 (10 degrees)
    add     #28T			; - - 10deg    ;* 253 - 56 = 197 + 28 = 225 (225 * .352 = 79.2 (80 degrees)
    sta     DelayAngle                     ;* DelayAngle now = 225 (80 degrees)
    lda     SparkHoldCyc_f                 ;* SparkHold_f = 0
    sta     wheelcount		; (HoldSpark)  ;* Wheelcount now = 0
...
CalcRunningParameters:
    ...
    brset   OneShotBArro,EnhancedBits2,bEnd_of_Baro	; Only do this once
							          ; as we may change the baro value
...
CalcRPM:  ; 50% re-written in 026i with aim of better odd-fire averaging
...
dorpmCalc: ; tmp12,13,14 used to hold average iTime or avg iTime
...
rpmCalcSlow: ; need to divide period (tmp12,13,14) by 100 to obtain period time in 0.1ms
    ldx    #100T
    clrh
    lda    tmp12
    div				; A rem H = (H:A) / X
...
Table_ASEStuff:
    ...
    jsr     LinInterp		; tmp6 contains amount of ase
					        ; enrichment in percent for this temperature
    clr     liX1
    lda     AWC_f1
    sta     liX2
...
; Add in accel enrichment
    lda     TPSAQ_f1	; start out using first element - will determine actual next
					    ; time around
Store_TEA1:
...
AE_COMP_SHOOT_AMT:          ; First, the amount based on cold temperatures
    lda     warmcor
...
TAE_CHK_TIME:  ; Check if acceleration done
...
Decay_AE_Aw:
    ...
    lda     Decay_Accel
    sta     liY1		; Load in the actual maximum PW we calculated fo the Accel to
					    ; interpolate from
    lda     TPSaclk
...
CHK_O2_LAG:  ; Check if exceeded lag time - if so then we can modify EGOcorr
    lda     EGOcount
    cmp     EGOcountcmp_f
    blo     EGOALL_DONEJMP
...
O2_IS_RICH:                 ; rich o2 - lean out EGOcorr
    lda     #100T
...
O2_IS_LEAN:                 ; lean o2 - richen EGOcorr
    lda     #100T
...
LoadContribDone:                ; NORMAL KPA stuff now
...
checkRPMsettings:                  ; Do all the rpm related stuff here.
...
cutChannels:
    ...
    mov     tmp1,pwcalc1
    mov     tmp2,pwcalc2
    bra     spark_lookup					 ; In fuel cut mode so return with zeros

checkRevsOk:
    brset   OverRun,EnhancedBits,cutChannels ; If Over run fuel cut on cut fuel
    mov       tmp1,pwcalc1
    mov       tmp2,pwcalc2
...
TRIM_DONE:
    ...
    lda     CrankAngle_f		; Update spark angle for User Interface  ;* CrankAngle_f = 56 (56 - 28.4 = 27.6 * .352 = 9.71 (10 degrees)
...
CALC_DELAY:
    ...
    stx     SparkAngle                ;* SparkAngle now = ST lookup + 1 degree
    lda     TriggAngle_f              ;* TriggAngle_f = 253 (253 * .352 = 89 degrees)
    sub     SparkAngle                ;* (253 - (ST lookup + 1 degree)
    add     #28T                      ;* ((253 - (ST lookup + 1 degree)) + 10 degrees
    sta     DelayAngle                ;* DelayAngle now = (((253 - ST lookup + 1 degree)) + 10 degrees)
...
idle_loopcold:                  ; determine duty cycle by linear interpolation
    lda     fastIdletemp_f
    sta     liX1
    lda     slowIdleTemp_f
    sta     liX2
...
; test code
; check if 0.1ms code has executed since we got here last. Major problem if it hasn't.
...
$MACRO DwellRail          ; check if we've gone too low
    lda     dwelltmpXop
    beq     dwlnwchk
    bit     #$80
...
$MACRO DwellDiv   ; store result. Convert us to 0.1ms
...
dwlldend:        ; check for high speed when dwell and period may be close
    lda     dwelltmpHms
    bne     save_dwell
    lda     dwelltmpLms
...
; we've now calculated target dwell period used by dwell and duty cycle
ms_dwell:
...
;add off the accel factor (-ve)    dt = dt-1 + (-ddt) (predicted next period)
    ...
    sta     dwelltmpXop
        ...
        SubDwell        ; subtract dwell macro

cd1_start:    ;check to see if value we _would_ store in dwelldelay1 is negative
              ; ie. top bit set
    lda     dwelltmpXop
    bmi     cd_1rail   ; if pos ok, else set to zero  ?? is BMI correct?

        DwellDiv       ; divide dwell macro
...
cd_2:     ;  dd2 = dd1 +dt-1 + ac + ac
    ...
    sta     dwelltmpXop
        ...
        DwellRail       ; check if negative or less than mindischarge macro

        DwellDiv        ; convert microseconds to 0.1ms units macro
...
wdwell2op:   ; first off always store a 360deg dwell delay
...
wd2end360:     ; we've now set the 360deg wait, see if we can delay off previous spark (180deg)
    lda     dwelldelay1
    bne     wd2ok
    lda     dwelldelay1+1
    cmp     #2
...
; late. a jsr would be nice.
    ...
    sei				; no ints while we are stealing this variable
...
lsspk_inv:
    brset    crank,engine,NoCoilNeg1

        COILNEG				; charge coil for inverted macro
...
;  hopefully this will not be a problem
    ...
    bit    #M_SC1TimCrnk		; Check if spark on time or IRQ return (SparkConfig1 already in A)   ;* M_SC1TimCrnk 0 = trigger return
    beq    IRQ_SPARK            ;                                                                                    1 = time based
...
REARM_IRQ:    ; Also do tacho output in here to give 50% output duty
...
stall_cont:
    lda      TriggAngle_f		; Calculate crank delay angle   ;* TriggAngle_f = 253 (253 * .352 = 89 degrees)
    sub      CrankAngle_f                                       ;* CrankAngle_f = 56 (56 - 28.4 = 27.6 * .352 = 9.71 (10 degrees)
    add      #28T			; - -10 deg                         ;* 253 - 56 = 197 +28 = 225 (225 * .352 = 79.2 (80 degrees)
    sta      DelayAngle                                         ;* DelayAngle now = 225 (80 degrees)

pass_store:
    lda      CrankAngle_f		    ; Update spark angle for user interface  ;* CrankAngle_f = 56 (56 - 28.4 = 27.6 * .352 = 9.71 (10 degrees)
    sta      SparkAngle                                                      ;* SparkAngle now = 56 (10 degrees)
    lda      SparkHoldCyc_f		    ; Hold spark after stall                 ;* SparkHold_f = 0
    sta      wheelcount		        ; (HoldSpark)                            ;* wheelcount now = 0
...
RTC_DONE:      ; now check that we haven't already missed the target
    sei
    lda     T2CNTL ; unlatch any previous read
    lda     T2CNTH
    sta     itmp00
...
NoCoilPos2:
    bclr    TOIE,T2SC1	; Disable interrupts
        ...
        CalcDwellspk		; Set spark on time macro
...
w_high_fast:    ;* 1us timer T2 already read at start of handler
    lda     T2CurrL
...
w_decode:   ;now try to decode pattern
...
; see if our tooth matches the user input trigger point
    lda     wheelcount
	cmp    trig1_f        ; Tooth 1 (90 degrees BTDC)  trigger point for run coila delay to on
	beq    MatchT1
	cmp    trig3_f        ; Tooth 8 ( 20 degrees BTDC) trigger point for crank coila on
	beq    MatchT8
	cmp    trig3ret_f     ; Tooth 9 (10 degrees BTDC)  trigger point for crank coila off
	beq    MatchT9
	cmp    trig5_f        ; Tooth 11 (10 degrees ATDC) trigger point for Inj1 start PW
	beq    MatchT11
	cmp    trig2_f        ; Tooth 19 (90 degrees BBDC) trigger point for run coilb delay to on
	beq    MatchT19
	cmp    trig4_f        ; Tooth 26 (20 degrees BBDC) trigger point for crank coilb on
	beq    MatchT26
	cmp    trig4ret_f     ; Tooth 27 (10 degrees BBDC) trigger point for crank coilb off
	beq    MatchT27
	cmp    trig6_f        ; Tooth 29 (10 degrees ABDC) trigger point for Inj2 start PW
	beq    MatchT29
    jmp    ret_w          ; No match so just make current T2 counter value tooth last and get out of here

MatchT1:
    clr    coilsel
    bset   coilabit,coilsel
    bset   output2,porta           ; Tacho on
    jmp    w_store2                ; Update all

MatchT8:
    brset  crank,engine,MatchT8OK  ; If not cranking just update T2 counter values last
    jmp    ret_w                   ; Make current T2 counter values last and get out of here

MatchT8OK:
    bset   coila,portc		       ; Cranking so energise coila and update T2 counter values last
    jmp    ret_w                   ; Make current T2 counter values last and get out of here

MatchT9:
    bclr   coila,portc		       ; Cranking so de-energise coila (fire spark) and update T2 counter values last
    jmp    ret_w                   ; Make current T2 counter values last and get out of here

MatchT11:                          ; Make current T2 counter values last and schedule injection
	lda    T2CurrH
    sta    T2PrevH
    lda    T2CurrL
    sta    T2PrevL
    jmp    SchedInj                ; Schedule injection
...
MatchT26:
    brset  crank,engine,MatchT26OK ; If not cranking just update time stamps
    jmp    ret_w                   ; Make current T2 counter values last and get out of here

MatchT26OK:
    bset   coilb,portc		       ; Cranking so energise coilb and update T2 counter values last
    jmp    ret_w                   ; Make current T2 counter values last and get out of here

MatchT27:
    bclr   coilb,portc		       ; Cranking so de-energise coilb (fire spark) and update T2 counter values last
	jmp    ret_w                   ; Make current T2 counter values last and get out of here

MatchT29:                          ; Make current T2 counter values last and schedule injection
    lda    T2CurrH
    sta    T2PrevH
    lda    T2CurrL
    sta    T2PrevL
...
ret_w:                             ; Make current T2 counter values last and get out of here
    lda     T2CurrH
    sta     T2PrevH
    lda     T2CurrL
    sta     T2PrevL
...
lost_sync_w:	; we found too many teeth after the missing one, start syncing again
				; also do holdoff. This should be rare, but if we lost sync that
...
; NEW
    ...
    sta     avgtoothh
    sta     avgtoothl
;NEW worth killing the dwell timers to avoid dwells starting
...
TPS_COUNTER:
    brclr    taeIgnCount,feature1,EGOBUMP	; Are we in Cycle counter mode for TPS Accel?
...
spk_mult:               ; Calculate time for delay angle
...
; low speed dwell
; a copy of some of Calcdwell, but simplified...uses SparkTempH/L for temporary space
...
setit:           ; Add total highres spark delay time to timer value from IRQ
...
w_dec_notlogt:
    ...
    cmp     #29T
    beq     SchedInj
    bra     squirtDone2  ;* Only schedule injection on tooth ll and 29
...
MODE_A:         ; Big A
    mov     #$16,txgoal		; 22T ; B&G mode ($17) For Megaview use
    bra     MODE_AA_cont
...
MODE_V:
    ...
    mov     #PAGESIZE,txgoal  ; no. of bytes to send back
					          ; (was $7e) was 201 now 213 for 12x12 NOW 201 again:-)
    lda     page
    cbeqa   #$F0,MODE_V_F0
    cbeqa   #$F1,MODE_V_F1
...
NotTPSLAst_Yet:
    cpx     #35T
    bne     inac_cont
    tsx     ; send stack ;* transfer stack pointer to index register
    txa
    jmp     CONT_TX
...
load_table:
    ...
    pulh
    incx
    cpx     #PAGESIZE+1		; copy 256 bytes reduced to 200 Increased to 212 for 12x12
...
;MSNS           equ      1    Megasquirtnspark (TS msnsCyl)
;MSNEON         equ      2    MS neon decoder (TS neonCrank)
;WHEEL          equ      4    generalised decoder 36-1, 60-2 etc (TS wheelon)
;WHEEL2         equ      8    If in WHEEL mode then WHEEL2 is 0 = -1  1 = -2 (TS onetwo)
;EDIS           equ      $10  edis (TS edis)
;DUALEDIS       equ      $20  if in EDIS mode then this allows two edis
;                             modules (for edis4 on V8, edis6 on V12)
;TFI            equ      $40  Ford TFI system (TS tfi)
;HEI7           equ      $80  GM 7 pin HEI (TS hei7)

outputpins_f    db      %00110010
;               bits=    76543210
;               equ 1       FIDLE for Idle Air Valve || spark output (as per MSnS) (TS fidleUse)
;               equ 2       LED17 for squirt led     || coila output (TS LED17Use)
;             (TS led18Use)(TS led182Use)
;                   bit 2    bit 3
...
;REUSE_LED19    equ $10     LED19 for accel led || coilb output (TS LED19Use)
;X2_FAN         equ $20     X2 = water Inj pulsed out || fan control (TS X2use)
...
;Mulitplex Ign  equ $80     NORMAL || toyota DLI ignition (TS toy_dli)
; multiplex

SRevLimRPM	    db      240T; Standard RPM limit for spark retard (rpm*100) (TS SoftRevLim)
SRevLimAngle	db      56T	; Spark retard for above +10 degrees (TS SoftLimRetard)
				            ; 10 = 10 + 10 i.e. -10 start point.
				            ; So 56T = 10BTDC )
SRevLimHTime	db      50T	; Time in 1/10Sec in Soft limit till (TS SoftLimMax)
				            ; hard limit cuts in *0.1
SRevLimCTime	db      10T	; NOT USED
RevLimit_f	    db      250T; Hard Rev limiter (rpm*100) (TS HardRevLim)
Out1Lim		    db      0T	; Output1 On point in RAW except for
				            ; TEMPS then its in F -40, so 200F switch
				            ; on point = 240F

Out1Source	    db      0T	; Output 1 source, index from secl, (TS out1src)
				            ; Standard Out1source
...
Out2Source	    db      0T	; Same as Out1Source (TS out2src)

feature1_f	    db      %01001000
;wd_2trig       equ 1        wheel decoder 2nd trigger i/p - new in 023c9 (TS wd_2trig)
;               spare
;whlsim         equ 4        Wheel simulator          off      on (TS whlsim)
;taeIgnCount	equ 8        Acceleration Timer    Seconds^ || Engine Cycles (TS taeIgnCount)
; NOT USE       equ $10      NOT USED NOW 023
;hybridAlphaN	equ $20      Hybrid Alpha N           OFF^ || OFF (TS hybridAN)
;CrankingPW2	equ $40      Fire PW2 during Cranking?   YES^ || NO (TS inj2cr)
;Nitrous	    equ $80      Nitrous system              OFF || ON (TS nitrousUse)
                                                       ;NOT allowed with W Inj

feature2_f	    db      %00000000  ; more features
;BoostControl	equ 1        Boost Controller           OFF || ON (TS boostUse)
;ShiftLight	    equ 2        Shift Lights               OFF || ON (TS shiftUse)
;LaunchControl	equ 4        launch Control             OFF || ON (TS launchUse)
;wasPWMidle	    equ 8
;                            only if outputpins_f bit 1 = 0
;out3sparkd	    equ $10      Output 3             Output 3  || Spark D (TS out3sparkd)
;min_dwell	    equ $20
;dwellduty50	equ $40
;config_error	equ $80    this is only set if non-sense combination
;                          of options - don't run.

whlsimcnt       db      04T ; How many outputs when simulating wheel (TS whlsimcnt)
...
Trig1_f		    db       1T	 ; wheel decoding (TS trig1)
Trig2_f		    db       19T ;  "             (TS trig2)
Trig3_f		    db       8T	 ;  "             (TS trig3)
Trig4_f		    db       26T ;  "             (TS trig4)
Trig1ret_f	    db       0T	 ;  "             (TS trig1ret)
Trig2ret_f	    db       0T  ;  "             (TS trig2ret)
Trig3ret_f	    db       9T	 ;  "             (TS trig3ret)
Trig4ret_f	    db       27T ;  "             (TS trig4ret)
...
alt_i1ge        equ %00100000  ; (TS inj1g)
alt_i2ge        equ %01000000  ; (TS inj2g)
trig2fallb      equ 1          ; 0 = rising edge trigger, 1 = falling edge
trig2risefallb  equ 2          ; 0 = rising or falling, 1 = rise and falling edge

latency_f       db        0T    ; "known" latency in spark input to output (TS latency)
spare1_2_f      db        0T
spare1_3_f      db        0T
spare1_4_f      db        0T
spare1_5_f      db        0T

EgoLimitKPa_f	db      255T  ; MAP KPa Point to change Ego +- limit (39) (TS EgoLimKPA)
EgoLim2_f	    db      05T	  ; New Ego limit when MAP KPa above (TS EgoLim2)
				              ; EgoLimitKPa_f
...
SparkCutNum_f	db      3T	  ; Rev Limiter Hard cut spark cut number to (TS HCutSpark)
				              ; remove sparks from SparkCutBase_f

feature3_f	    db      %10110001  ; (46)
KPaTpsOpenb     equ 1          ; (TS OLoopEgo)
VarLaunchb      equ 2
CltIatIgnitionb equ 4          ; (TS CltAdv)
WaterInjb       equ 8
Fuel_SparkHardb equ $10        ; Fuel or Spark cut for Rev limiter
FuelSparkCutb   equ $20        ; Fuel or spark cut for Rev limiter
KnockDetb       equ $40        ; (TS KnockOn)
TargetAFRb      equ $80        ; (TS AfrTar)
...
kpaO2_f		    db      120T    ; KPa Open loop setpoint for no O2 correction (TS KpaOn)
tpsO2_f		    db      255T    ; TPS Open Loop setpoint for no O2 correction Raw ADC (TS tpsOn)
...
OverRunOnb      equ $40         ; (TS OverRunC)
KpaDotSetb      equ $80         ; (TS AccDecEnrich)
...
EfanOnTemp_f	db      234T    ; X2 or LED 17 electric fan output on temp (TS efanontemp)
				                ; F-40
EfanOffTemp_f	db      185T    ; X2 or LED 17 electric fan output off temp (TS efanofftemp)
				                ; F-40

feature5_f	    db      %00110011  ; Yet another feature bit (76)
Fuel_SparkHLCb  equ 1       ; Fuel or Spark cut for Launch (TS HcutType)
FuelSparkLCb    equ 2       ; Fuel or Spark cut for Launch (TS HcutType)
stagedb:        equ 4       ; Roger Enns Staged Mode   xxxx00xx = Staged Off  xxxx01xx = RPM Based (TS IStagedMode)
stagedModeb:    equ 8       ; Roger Enns Staged Mode   xxxx10xx = MAP Based   xxxx11xx = TPS Based
stagedeither:   equ $0c     ; either staging
BoostCutb:      equ $10     ; Over boost Cut type, option2 or spark cut (TS OvrBCutBNum)
BoostCut2b:     equ $20     ; Option2 for Over boost Cut type, fuel only or both fuel and spark  (TS OvrBCutBNum)
NosLagSystemb:  equ $40     ; Nos Anti-lag System used
SparkTable2b:   equ $80     ; Second Spark Table (ST STTable2On)


SparkCutNLC_f	db      03T	; Launch control spark cut, this is the (TS HCutLCSpark)
				            ; amount of sparks to remove from
				            ; SparkCutBase_f when in Launch hard cut
SparkCutBase_f	db      06T	; Base number to cut sparks from MS (TS SparkCutBase)
				            ; = MT value - 1
...
BarroHi_f	    db      116T; Barometric Correction Max Limit in KPa (82) (TS BarroHi)
BarroLow_f	    db      67T	; Barometric Correction Lower Limit in KPa (TS BarroLow)
...
NumTeeth_f	    db      36T	; Number of teeth for wheel decoder (TS numteeth)
MAPThresh_f	    db      30T	; MAP dot threshold for Accel Decel
				            ; Enrichments *10 (KPa/Sec)

feature6_f	    db      %00001000  ; More feature bits (92)
VETable3b       equ 1       ; Use VE table 3 (TS UseVE3)
TargetAFR3b     equ 2       ; Use Target AFR for VE3
falsetrigb      equ 4       ; 0=Enable false trigger protection, 1=disable    ;* (TS falsetrig_dis not used)
wheel_oldb      equ 8       ; 0= new(025) wheel decoder or 1=old(024s9) style ;* (TS wheel_old)
dualdizzyb      equ $10     ; (TS dualdizzy)
TractionCb      equ $20     ; Traction control system on (TS TractionOn)
BoostDirb       equ $40     ; Direction for boost control output
NoDecelBoostb   equ $80     ; No decelleration when in boost (TS DecelBoost)
...
RPMthresh_f	    db      2T	; Threshhold for RPM change for traction (TS RPMThresh)
...
;dwellcont	    equ 2   Real (crude) dwell control (TS dwellcont)
...
;WheelSensor	equ 8   Traction control    RPM Based || VSS (TS TractMode)
				      ; only if Traction ON
				      ; see TractionCb:feature6 bit 6
;AlphaTarAFR	equ $10  speed density for target afr tables || Alpha-N (TS KpaTPSTar)
				      ; only if Target AFR tables ON
				      ; see TargetAFRb:feature3 bit 8
;TPSTargetAFR	equ $20    0=  Use Target AFR all the while || 1=Only when (TS TPSTar)
...
dwellcrank_f	db       45T ; cranking dwell in 0.1ms (TS dwellcrank)(NOT USED)
dwellrun_f	    db       30T ; running  dwell in 0.1ms  (111) (TS dwellrun)
...
feature8_f	    db      %01000000  ; More feature bits (116)
;spare 1
;spare 2
BoostTable3b:   equ 4      ; Use boost table 3 when switch table input on
spkeopb         equ 8      ; Enable spark E output (instead of shiftlight)(TS spkeop)
spkfopb         equ $10    ; Enable spark F output (instead of knock in)(TS spkfop)
DecelMAPb:      equ $20    ; Use MAP for Decel
InterpAcelb:    equ $40    ; Interpole the accel enrichments down to a setpoint (TS DecayAccel)
Out1_Out3b:     equ $80    ; Output3 only if output1 is on. (TS Out3_Out1)
...
Out3Source_f	db      00T	; Same as Out1Source (TS out3src bits0:4)

Out3Lim_f	    db      00T	; Output 3 On/Off Limit as per standard MSnS (TS out3lim)
TimerOut3_f	    db      00T	; Output 3 OFF delay timer in Seconds (134) *1 (TS out3Timer)
...
mindischg_f	    db      05T	; minimum discharge period for dwell (TS mindischg)
				            ; control in mSec *0.1

;pwm idle was here

tachconf_f      db      4T  ; tach output config (159) (TS tachconf tachconf bits0:2)tachconfdiv bit7
Trig5_f		    db      11T ; wheel decoding (160) (TS trig5)
Trig6_f		    db      29T ;  "                   (TS trig6)
Trig5ret_f	    db      0T  ;  "                   (TS trig5ret)
Trig6ret_f	    db      0T	;  "                   (TS trig6ret)
...
TPSACOLD_f1	    db      90T	; TPSACOLD (ms to add in when cold) *0.1 (TS taeCold4)
TPSTHRESH_f1	db      03T	; TPSTHRESH for Accel enrichment when in (TS tpsThresh)
				            ; TPS mode *0.1953125
TPSASYNC_f1	    db      02T	; TPSASYNC (accel enrich time in 1/10 (TS taeTime4)
				            ; second increments) or in Enfgine Cycles.
TPSDQ_f1	    db      100T; TPSDQ   (195) (TS tdePct4) (TS taeColdM4)
...
RpmAEBased:     equ $10     ; RPM Based Accel Enrichment (TS RPMBasedAE)
MassAirFlwb:    equ $20     ; Using Mass AirFlow meter instead of a MAP sensor. (TS AFMUse)
NoAirFactorb:   equ $40     ; If using MAF do we use Air Density in fueling cals?
ConsBarCorb:    equ $80     ; Constant Bar Cor using MAP on X7 (TS ConsBaroCor)
...
VE_f1:
	...
	db      84T,84T,87T,88T,89T,91T,92T,94T,96T,97T,98T,100T ; VE (11,0-11)

EGOTEMP_f	    db      200T; EGOTEMP (TS egoTemp1)
EGOCOUNTCMP_f	db      16T	; EGOCOUNTCMP (TS egoCount1)
EGODELTA_f	    db      0T	; EGODELTA (TS egoDelta1)
EGOLIMIT_f	    db      15T	; EGOLIMIT (TS egoLimit1)
REQ_FUEL_f1	    db      104T; REQFUEL (148) (TS reqFuel1)
DIVIDER_f1	    db      4T	; DIVIDER (TS divider1)
Alternate_f1	db      1T	; alternate or simult (single table ONLY)(TS alternate1)
INJOPEN_f1	    db      9T	; INJOPEN (TS injOpen)
INJOCFUEL_f1	db      0T	; INJOCFUEL  NOT USED NOW !!!!!
				            ; Kept to fill hole
INJPWM_f1	    db      75T	; INJPWM
INJPWMT_f1	    db      255T; INJPWMT
BATTFAC_f1	    db      20T	; BATTFAC (TS battFac1)
rpmk_f1		    db      $05	; RPMK[0]
		        db      $DC	; RPMK[1]
RPMRANGEVE_f1:
	            db      5T	; RPMRANGEVE[0]                             ;* 158 Tuner Studio rpmBins1
	            db      9T	; RPMRANGEVE[1]                             ;* 159
	            db      12T	; RPMRANGEVE[2]                             ;* 160
	            db      16T	; RPMRANGEVE[3]                             ;* 161
	            db      20T	; RPMRANGEVE[4]                             ;* 162
	            db      23T	; RPMRANGEVE[5]                             ;* 163
	            db      27T	; RPMRANGEVE[6]                             ;* 164
	            db      30T	; RPMRANGEVE[7]                             ;* 165
	            db      34T	; RPMRANGEVE[8]                             ;* 166
	            db      38T	; RPMRANGEVE[9]                             ;* 167
	            db      41T	; RPMRANGEVE[10]                            ;* 168
	            db      45T	; RPMRANGEVE[11]
KPARANGEVE_f1:
	            db      20T	; KPARANGEVE[0]                             ;* 170 Tuner Studio mapBins1
	            db      27T	; KPARANGEVE[1]                             ;* 171
	            db      35T	; KPARANGEVE[2]                             ;* 172
       	        db      42T	; KPARANGEVE[3]                             ;* 173
	            db      49T	; KPARANGEVE[4]                             ;* 174
	            db      56T	; KPARANGEVE[5]                             ;* 175
	            db      64T	; KPARANGEVE[6]                             ;* 176
	            db      71T	; KPARANGEVE[7]                             ;* 177
	            db      78T	; KPARANGEVE[8]                             ;* 178
	            db      85T	; KPARANGEVE[9]                             ;* 179
	            db      93T	; KPARANGEVE[10]                            ;* 180
	            db      100T; KPARANGEVE[11]                            ;* 181


config11_f1	    db      49T	; Config11 (originally 113T for 8 cyl) (182)TS Config1 (= %00110001, $31, 49T)
;  Bit 0-1 = MAP Type
;            00 = MPX4115AP
;            01 = MPX4250AP
;            10 = MPXH6300A
...
;            1010 = 11 cylinders
;            1011 = 12 cylinders
;* TS mapSensor1 bit0 (= 0)
;* TS mapType1 bit0:1 (= 1 (250 KPA))
;* TS twoStroke1 bit2 (= 0 Four_Stroke)
;* bit3 Not Used (= 0)
;* TS nCylinders1 bit4 (= 1 Four Cyl)
;* TS nCylinders1 bit5 (= 1 Four Cyl)
;* TS nCylinders1 bit6 (= 0)
;* TS nCylinders1 bit7 (= 0)
config12_f1	    db      58T	; Config12 (originally 112T for 8 injectors)TS Config2 (= %00111010, $3A, 58T)
...
;            1010 = 11 Injectors
;            1011 = 12 Injectors
;* TS cltType1 bit0 (= 0 unknown1)
;* TS cltType1 bit1 (= 1 unknown2)
;* TS matType1 bit2 (= 0 unknown1)
;* TS matType1 bit3 (= 1 unknown2)
;* TS nInjectors1 bit4 (= 1 Four Cyl)
;* TS nInjectors1 bit5 (= 1 Four Cyl)
;* TS nInjectors1 bit6 (= 0)
;* TS nInjectors1 bit7 (= 0)
config13_f1	    db      10T	; Config13 (TS Config3 (= %00001010, $A, 10T))
...
;            1 = Enrichment On
;* TS engineType1 bit0 (= 0 Even Fire
;* TS egoType1 bit1    (= 1 Wide Band)
;* TS algorithm1 bit2  (= 0 Speed Density)
;* TS baroCorr1 bit3   (= 1 Barocor on)
;* bit4 Not Used (= 0)
;* bit5 Not Used (= 0)
;* bit6 Not Used (= 0)
;* bit7 Not Used (= 0)

EGOrpm_f	    db      60T	; RPMOXLIMIT (TS egoRPM1)
FASTIDLEbg_f	db      234T;
O2targetV_f	    db      26T	; VOLTOXTARGET (187) (TS egoSwitch1)
feature14_f1    db      %00000000  ; (188)  ; allows EGOigncount to be on page1 (TS egoIgnCount)
...
ST_f1: 				; (ST - 28.4) * .352 = Degrees BTDC
	...
	db    77T,82T,82T,88T,94T,99T,105T,108T,114T,114T,114T,114T   ;(6,0-11)
	db    74T,77T,77T,82T,88T,94T,99T,105T,111T,111T,111T,111T    ;(7,0-11)
	db    68T,71T,74T,80T,85T,88T,94T,102T,108T,108T,108T,108T    ;(8,0-11)
	db    65T,65T,68T,74T,80T,82T,88T,99T,105T,105T,105T,105T     ;(9,0-11)
	db    62T,62T,62T,68T,74T,80T,85T,94T,99T,99T,99T,99T         ;(a,0-11)
	db    57T,57T,57T,62T,68T,74T,80T,91T,97T,97T,97T,97T         ;(b,0-11) (143)

RPMRANGEST_f1:
	db      05T,09T,12T,16T,20T,23T,27T,30T,34T,38T,41T,45T
				; RPMRANGEST[0-11]

KPARANGEST_f1:
	db      20T,27T,35T,42T,49T,56T,64T,71T,78T,85T,93T,100T  ; (last byte 167)
				; KPARANGEST[0-b]

;; org $d3a8  ; stick them up out of the way at known values
TriggAngle_f	db      253T; TriggAngle (60 deg)     (168)  *0.352  ;* Tuner Studio TrigAng = 253 (253 * .352 = 89 degrees)
FixedAngle_f	db      0T	; FixedAngle   *0.352  -28.4   min -10 Max 80 (TS FixAng)
				            ; THIS MUST BE -10 (0) for non fixed angle
TrimAngle_f	    db      1T	; TrimAngle (NOT cleared on startup)  *0.352 (TS Trim)
CrankAngle_f	db      56T	; Cranking advance (10deg)  *0.352 -28.4 Tuner Studio CrankAng = 56 (56 - 28.4 = 27.6 * .352 = 9.71 (10 degrees)
				            ;  min -10 max 80
SparkHoldCyc_f	db      0T	; SparkHoldCyc (hold spark x cycles on  ;* Tuner Studio IgHold = 0
				            ; stall and restart)
...
M_SC1LngTrg     equ     $01 ; Spark config 1 (0) Long trigger +22.5 (TS Trig_plus)
M_SC1XLngTrg    equ     $02 ; Spark config 1 (1) Extra Long trigger +45 (TS Trig_plus)
M_SC1TimCrnk    equ     $04 ; Spark config 1 (2) Time based cranking (not trigger return) (TS TrigCrank)
M_SC1InvSpark   equ     $08 ; Spark config 1 (3) Invert spark output (TS iginv)
M_SC1oddfire    equ     $10 ; Spark config 1 (4) Oddfire ignition (TS oddfire)

IdleAdvance_f	db	    0T	; IdleAdvance *0.342 -28.4 min -10 max 80 (ST IdleAdv0
IdleTPSThresh_f	db	    0T	; below this TPS value idle advance (ST IdleAdvTPS)
IdleRPMThresh_f	db	    0T	; below this RPM value idle advance (0 disables) (ST IdleAdvRPM)
IdleCLTThresh_f db	    0T	; below this CLT value don't use idle advance (ST IdleAdvClt)
IdleDelayTime_f db	    1T	; wait this long before using the idle advance (ST IdleDelayTime)
...
KPARANGEST_f2:
    db      30T,40T,50T,60T,70T,80T,90T,100T,110T,120T,130T,140T
...
AWC_f1  	 db      250T; After Start Warmup Time (TS ASETime)
feature10_f5 db      %00000000  ; (179)
aseIgnCountb equ 1       ;  AFTER START Enrichment Seconds || Engine Cycles^ (TS aseIgnCount)
ASEHoldb:    equ 2       ; Hold ASE from decaying for a period of time determined by TimFixASE_f  (TS ASEFixDE)
MAPHoldb:    equ 4       ; Fix MAP value during Fixed ASE timer (TS VEFixAASE)
...
KPARANGEAFR_f1:
	db      20T	    ; KPARANGEAFR1[0]                            ;* 72 Tuner Studio mapBins6
...
flash_table7_end:
                ...
                org     $E800
flash_table8:
idle_dc_lo	    db      50T ; Idle duty cycle at lower temp for 2-wire (TC idle_dc_lo)
				            ; Fielding Idle control *1  MIn = 0 Max = 100
idlePeriod_f	db      25T ; idle period in ignition events
idlecrankdc_f	db      50T ; cranking idle dc kg (TS idlecrankdc)
...
idle_dc_hi      db      00T ; rmd Idle duty cycle at upper temp for 2-wire (warmup) (TS idle_dc_hi)
...
idlefreq_f      db      100T; rmd  (TS idlefreq)
...
feature13_f     db      3T
PWMidleb        equ    $01  ; pwm idle on vs B&G on/off (TS pwmIdle)
idle_warmupb    equ    $02  ; pwm idle warmup open loop (TS pwmidlewhen)
idle_clb        equ    $04  ; pwm idle closed loop       (TS pwmidlewhen)
cltMAPb:        equ    $08  ; Use Correction table in the Air Density factor
CltMATCheckb:   equ    $10  ; Correction table MAT or IAT based

FASTIDLEtemp_f	db      105T; Feilding 2-Wire Idle control Fast Idle (TS fastIdleTemp)
				            ; lower temperature F -40
slowIdleTemp_f	db      200T; Feilding 2-Wire Idle control Slow Idle (TS slowIdleTemp)
...
AlwaysPrimeb:     equ 1    ; Only fire pump if Prime pulse ON | Prime pump every time (TS AlwaysPrime)
PrimeLateb        equ 2    ; Fire prime pulse after 2 seconds (TS PrimeLate)
PrimeTwiceb       equ 4    ; Fire the Prime Pulses Twice (TS PrimeLate)
NoPrimePb:        equ 8    ; Use Priming Table or Use Prime Pulse(TS PrimeTBox)
cltcrankb:        equ $10  ; use coolant temp for crank pulsewidth (TS crankpwip)
matcrankb:        equ $20  ; use inlet air temp for crank pulsewidth. Both means average (TS crankpwip)
ExCrFuelb:        equ $40  ; Look at TPS to see if we trigger extra fuel during cranking? (TS ExFuelCrank)

CrankRPM_f	    db      3T	; Maximum RPM for cranking (rpm*100) (TS CrankRPM)
tpsflood_f	    db      243T; Throttle position for floodclear mode in (TS tpsflood)
				            ; RAW ADC
